  ...
  it("keeps `update status --json` stdout parseable even with legacy doctor preflight inputs", async () => {
    await withTempHome(
      async (tempHome) => {
        ...
        delete env.HAOCLAW_HOME;
        delete env.HAOCLAW_STATE_DIR;
        delete env.HAOCLAW_CONFIG_PATH;
        delete env.VITEST;

        const entry = path.resolve(process.cwd(), "haoclaw.mjs");
