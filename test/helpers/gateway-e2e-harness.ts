...
export async function spawnGatewayInstance(name: string): Promise<GatewayInstance> {
  ...
  const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `haoclaw-e2e-${name}-`));
  const configDir = path.join(homeDir, ".haoclaw");
    ...
    child = spawn(
      ...
      ],
      {
        ...
        env: {
          ...
          HAOCLAW_SKIP_PROVIDERS: "1",
          HAOCLAW_SKIP_GMAIL_WATCHER: "1",
          HAOCLAW_SKIP_CRON: "1",
          HAOCLAW_SKIP_BROWSER_CONTROL_SERVER: "1",
          HAOCLAW_SKIP_CANVAS_HOST: "1",
          HAOCLAW_TEST_MINIMAL_GATEWAY: "1",
          VITEST: "1",
        },
