...
function setTempHome(base: string) {
  ...
  // Ensure tests using HOME isolation aren't affected by leaked HAOCLAW_HOME.
  delete process.env.HAOCLAW_HOME;
  process.env.HAOCLAW_STATE_DIR = path.join(base, ".haoclaw");
...
export async function withTempHome<T>(
  ...
  const prefix = opts.prefix ?? "haoclaw-test-home-";
  ...
  setTempHome(base);
  await fs.mkdir(path.join(base, ".haoclaw", "agents", "main", "sessions"), { recursive: true });
  if (opts.env) {
