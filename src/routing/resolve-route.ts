...
function listAgents(cfg: HaoclawConfig) {
  const agents = cfg.agents?.list;
  return Array.isArray(agents) ? agents : [];
}

type AgentLookupCache = {
  agentsRef: HaoclawConfig["agents"] | undefined;
  byNormalizedId: Map<string, string>;
  fallbackDefaultAgentId: string;
};

const agentLookupCacheByCfg = new WeakMap<HaoclawConfig, AgentLookupCache>();

function resolveAgentLookupCache(cfg: HaoclawConfig): AgentLookupCache {
...
export function pickFirstExistingAgentId(cfg: HaoclawConfig, agentId: string): string {
...
type EvaluatedBindingsCache = {
  bindingsRef: HaoclawConfig["bindings"];
...
const evaluatedBindingsCacheByCfg = new WeakMap<HaoclawConfig, EvaluatedBindingsCache>();
const MAX_EVALUATED_BINDINGS_CACHE_KEYS = 2000;
const resolvedRouteCacheByCfg = new WeakMap<
  HaoclawConfig,
  {
    bindingsRef: HaoclawConfig["bindings"];
    agentsRef: HaoclawConfig["agents"];
    sessionRef: HaoclawConfig["session"];
...
function resolveRouteCacheForConfig(cfg: HaoclawConfig): Map<string, ResolvedAgentRoute> {
