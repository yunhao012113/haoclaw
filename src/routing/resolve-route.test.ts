...
describe("resolveAgentRoute", () => {
  const resolveDiscordGuildRoute = (cfg: HaoclawConfig) =>
    resolveAgentRoute({
      cfg,
      channel: "discord",
      accountId: "default",
  ...
  test("defaultAgentId is used when no binding matches", () => {
    const cfg: HaoclawConfig = {
      agents: {
        list: [{ id: "home", default: true, workspace: "~/haoclaw-home" }],
      },
    };
    const route = resolveAgentRoute({
      cfg,
...
describe("role-based agent routing", () => {
  type DiscordBinding = NonNullable<HaoclawConfig["bindings"]>[number];
