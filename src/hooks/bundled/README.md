...
This directory contains hooks that ship with Haoclaw. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
Hooks can be configured in `~/.haoclaw/haoclaw.json`:
...
2. Restart gateway: `pkill -9 -f 'haoclaw.*gateway' && pnpm haoclaw gateway`
3. Enable the hook: `haoclaw hooks enable my-hook`
4. Trigger the event (e.g., send `/new` command)
5. Check gateway logs for hook execution

## Documentation

Full documentation: https://docs.haoclaw.ai/automation/hooks
