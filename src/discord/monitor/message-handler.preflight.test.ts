...
type DiscordConfig = NonNullable<
  import("../../config/config.js").HaoclawConfig["channels"]
>["discord"];
...
} as import("../../config/config.js").HaoclawConfig;
...
function createPreflightArgs(params: {
  cfg: import("../../config/config.js").HaoclawConfig;
...
describe("preflightDiscordMessage", () => {
  ...
  it("allows bot messages with explicit mention when allowBots=mentions", async () => {
    ...
    const message = createMessage({
      ...
      channelId,
      content: "hi <@haoclaw-bot>",
      mentionedUsers: [{ id: "haoclaw-bot" }],
      author: {
        id: "relay-bot-1",
        bot: true,
        username: "Relay",
  ...
  it("uses attachment content_type for guild audio preflight mention detection", async () => {
    transcribeFirstAudioMock.mockResolvedValue("hey haoclaw");
