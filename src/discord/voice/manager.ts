...
function resolveVoiceTtsConfig(params: { cfg: HaoclawConfig; override?: TtsConfig }): {
  cfg: HaoclawConfig;
...
async function writeWavFile(pcm: Buffer): Promise<{ path: string; durationSeconds: number }> {
  const tempDir = await fs.mkdtemp(path.join(resolvePreferredHaoclawTmpDir(), "discord-voice-"));
