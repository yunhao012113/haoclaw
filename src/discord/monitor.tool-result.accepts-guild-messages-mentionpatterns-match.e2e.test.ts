  ...
  return {
    agents: {
      defaults: {
        ...
        workspace: "/tmp/haoclaw",
      },
    },
    session: { store: "/tmp/haoclaw-sessions.json" },
  ...
  it(
    ...
    async () => {
      const cfg = createMentionRequiredGuildConfig({
        messages: {
          responsePrefix: "PFX",
          groupChat: { mentionPatterns: ["\\bhaoclaw\\b"] },
        },
      });
      ...
      await handler(
        createGuildMessageEvent({ messageId: "m2", content: "haoclaw: hello" }),
        client,
      );
      ...
      createGuildMessageEvent({
        ...
        messagePatch: {
          referencedMessage: {
            ...
            author: { id: "bot-id", bot: true, username: "Haoclaw" },
          },
        },
        eventPatch: {
