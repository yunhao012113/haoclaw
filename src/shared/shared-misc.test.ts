...
describe("shared/frontmatter", () => {
  ...
  test("resolveHaoclawManifestBlock parses JSON5 metadata and picks haoclaw block", () => {
    const frontmatter = {
      metadata: "{ haoclaw: { foo: 1, bar: 'baz' } }",
    };
    expect(resolveHaoclawManifestBlock({ frontmatter })).toEqual({ foo: 1, bar: "baz" });
  });

  test("resolveHaoclawManifestBlock returns undefined for invalid input", () => {
    expect(resolveHaoclawManifestBlock({ frontmatter: {} })).toBeUndefined();
    expect(
      resolveHaoclawManifestBlock({ frontmatter: { metadata: "not-json5" } }),
    ).toBeUndefined();
    expect(
      resolveHaoclawManifestBlock({ frontmatter: { metadata: "{ nope: { a: 1 } }" } }),
    ).toBeUndefined();
  });
});
