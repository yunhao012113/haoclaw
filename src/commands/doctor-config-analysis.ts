...
import { HaoclawSchema } from "../config/zod-schema.js";
...
export function stripUnknownConfigKeys(config: HaoclawConfig): {
  config: HaoclawConfig;
  removed: string[];
} {
  const parsed = HaoclawSchema.safeParse(config);
  if (parsed.success) {
...
export function noteOpencodeProviderOverrides(cfg: HaoclawConfig): void {
