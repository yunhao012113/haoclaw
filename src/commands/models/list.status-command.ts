...
export async function modelsStatusCommand(
  ...
  const agentDir = agentId ? resolveAgentDir(cfg, agentId) : resolveHaoclawAgentDir();
  ...
  if (missingProvidersInUse.length > 0) {
    ...
    for (const provider of missingProvidersInUse) {
      const hint =
        provider === "anthropic"
          ? `Run \`claude setup-token\`, then \`${formatCliCommand("haoclaw models auth setup-token")}\` or \`${formatCliCommand("haoclaw configure")}\`.`
          : `Run \`${formatCliCommand("haoclaw configure")}\` or set an API key env var.`;
