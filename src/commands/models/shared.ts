...
export async function loadValidConfigOrThrow(): Promise<HaoclawConfig> {
...
export async function updateConfig(
  mutator: (cfg: HaoclawConfig) => HaoclawConfig,
...
export function resolveModelTarget(params: { raw: string; cfg: HaoclawConfig }): {
  provider: string;
  model: string;
} {
...
export function buildAllowlistSet(cfg: HaoclawConfig): Set<string> {
...
export function resolveKnownAgentId(params: {
  ...
  if (!knownAgents.includes(agentId)) {
    throw new Error(
      `Unknown agent id "${raw}". Use "${formatCliCommand("haoclaw agents list")}" to see configured agents.`,
    );
  }
  return agentId;
}
