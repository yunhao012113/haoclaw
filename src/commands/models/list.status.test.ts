...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveHaoclawAgentDir: vi.fn().mockReturnValue("/tmp/haoclaw-agent"),
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/haoclaw-agent"),
    ...
    resolveAuthStorePathForDisplay: vi
      .fn()
      .mockReturnValue("/tmp/haoclaw-agent/auth-profiles.json"),
...
vi.mock("../../agents/agent-paths.js", () => ({
  resolveHaoclawAgentDir: mocks.resolveHaoclawAgentDir,
}));
...
async function withAgentScopeOverrides<T>(
  ...
  try {
    ...
    if (originalAgentDir) {
      ...
      mocks.resolveAgentDir.mockReturnValue("/tmp/haoclaw-agent");
    }
  }
}
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(mocks.resolveHaoclawAgentDir).toHaveBeenCalled();
    expect(payload.defaultModel).toBe("anthropic/claude-opus-4-5");
    expect(payload.configPath).toBe("/tmp/haoclaw-dev/haoclaw.json");
    expect(payload.auth.storePath).toBe("/tmp/haoclaw-agent/auth-profiles.json");
    ...
    await withAgentScopeOverrides(
      {
        ...
        agentDir: "/tmp/haoclaw-agent-custom",
      },
      async () => {
        ...
        expect(payload.agentDir).toBe("/tmp/haoclaw-agent-custom");
