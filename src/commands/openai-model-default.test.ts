...
function expectPrimaryModelChanged(
  applied: { changed: boolean; next: HaoclawConfig },
  primary: string,
) {
...
type SharedDefaultModelCase = {
  apply: (cfg: HaoclawConfig) => { changed: boolean; next: HaoclawConfig };
  defaultModel: string;
  overrideConfig: HaoclawConfig;
  alreadyDefaultConfig: HaoclawConfig;
};
...
describe("applyDefaultModelChoice", () => {
  it("ensures allowlist entry exists when returning an agent override", async () => {
    ...
    const applied = await applyDefaultModelChoice({
      ...
      applyProviderConfig: (config: HaoclawConfig) => config,
      applyDefaultConfig: (config: HaoclawConfig) => config,
      noteAgentModel,
...
describe("shared default model behavior", () => {
  it("sets defaults when model is unset", () => {
    for (const testCase of SHARED_DEFAULT_MODEL_CASES) {
      const cfg: HaoclawConfig = { agents: { defaults: {} } };
