...
function resolveAgentName(cfg: HaoclawConfig, agentId: string) {
...
function resolveAgentModel(cfg: HaoclawConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: HaoclawConfig): AgentSummary[] {
