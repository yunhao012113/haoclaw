...
const ensureHaoclawModelsJson = vi.fn().mockResolvedValue(undefined);
const resolveHaoclawAgentDir = vi.fn().mockReturnValue("/tmp/haoclaw-agent");
...
vi.mock("../agents/models-config.js", () => ({
  ensureHaoclawModelsJson,
}));

vi.mock("../agents/agent-paths.js", () => ({
  resolveHaoclawAgentDir,
}));
...
beforeEach(() => {
  ...
  ensureHaoclawModelsJson.mockClear();
...
describe("models list/status", () => {
  ...
  it("loadModelRegistry does not persist models.json as a side effect", async () => {
    ...
    expect(ensureHaoclawModelsJson).not.toHaveBeenCalled();
  });
  ...
  it("modelsListCommand persists using the write snapshot config when provided", async () => {
    ...
    expect(ensureHaoclawModelsJson).toHaveBeenCalled();
    expect(ensureHaoclawModelsJson.mock.calls[0]?.[0]).toEqual(resolvedConfig);
  });
