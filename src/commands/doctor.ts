...
import { resolveHaoclawPackageRoot } from "../infra/haoclaw-root.js";
...
function resolveMode(cfg: HaoclawConfig): "local" | "remote" {
...
export async function doctorCommand(
  ...
  intro("Haoclaw doctor");

  const root = await resolveHaoclawPackageRoot({
  ...
  let cfg: HaoclawConfig = configResult.cfg;
  ...
  if (!cfg.gateway?.mode) {
    const lines = [
      ...
      `Fix: run ${formatCliCommand("haoclaw configure")} and set Gateway mode (local/remote).`,
      `Or set directly: ${formatCliCommand("haoclaw config set gateway.mode local")}`,
    ];
    if (!fs.existsSync(configPath)) {
      lines.push(`Missing config: run ${formatCliCommand("haoclaw setup")} first.`);
    }
    note(lines.join("\n"), "Gateway");
  }
  if (resolveMode(cfg) === "local" && hasAmbiguousGatewayAuthModeConfig(cfg)) {
    note(
      [
        ...
        `Set token mode: ${formatCliCommand("haoclaw config set gateway.auth.mode token")}`,
        `Set password mode: ${formatCliCommand("haoclaw config set gateway.auth.mode password")}`,
      ].join("\n"),
      "Gateway auth",
    );
  }
  ...
  if (shouldWriteConfig) {
    ...
    runtime.log(`Run "${formatCliCommand("haoclaw doctor --fix")}" to apply changes.`);
  }
