...
function pruneAuthProfiles(
  cfg: HaoclawConfig,
  profileIds: Set<string>,
): { next: HaoclawConfig; changed: boolean } {
...
export async function maybeRemoveDeprecatedCliAuthProfiles(
  ...
  if (deprecated.has(CLAUDE_CLI_PROFILE_ID)) {
    lines.push(
      `- ${CLAUDE_CLI_PROFILE_ID} (Anthropic): use setup-token → ${formatCliCommand("haoclaw models auth setup-token")}`,
    );
  }
...
function formatAuthIssueHint(issue: AuthIssue): string | null {
  ...
  if (issue.provider === "anthropic" && issue.profileId === CLAUDE_CLI_PROFILE_ID) {
    return `Deprecated profile. Use ${formatCliCommand("haoclaw models auth setup-token")} or ${formatCliCommand(
      "haoclaw configure",
    )}.`;
  }
  if (issue.provider === "openai-codex" && issue.profileId === CODEX_CLI_PROFILE_ID) {
    return `Deprecated profile. Use ${formatCliCommand(
      "haoclaw models auth login --provider openai-codex",
    )} or ${formatCliCommand("haoclaw configure")}.`;
  }
  return `Re-auth via \`${formatCliCommand("haoclaw configure")}\` or \`${formatCliCommand("haoclaw onboard")}\`.`;
}
