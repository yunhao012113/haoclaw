...
describe("readServiceStatusSummary", () => {
  it("marks Haoclaw-managed services as installed", async () => {
    const summary = await readServiceStatusSummary(
      createService({
        isLoaded: vi.fn(async () => true),
        readCommand: vi.fn(async () => ({ programArguments: ["haoclaw", "gateway", "run"] })),
    ...
    expect(summary.managedByHaoclaw).toBe(true);
  ...
  it("marks running unmanaged services as externally managed", async () => {
    ...
    expect(summary.installed).toBe(true);
    expect(summary.managedByHaoclaw).toBe(false);
