...
const SEARCH_TOOL = "https://docs.haoclaw.ai/mcp.SearchHaoclaw";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.haoclaw.ai");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('haoclaw docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.haoclaw.ai/");
      runtime.log(`Search: ${formatCliCommand('haoclaw docs "your query"')}`);
    }
    return;
  }
