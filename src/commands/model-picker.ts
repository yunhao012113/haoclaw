...
type PromptDefaultModelResult = { model?: string; config?: HaoclawConfig };
...
function resolveConfiguredModelRaw(cfg: HaoclawConfig): string {
  return resolveAgentModelPrimaryValue(cfg.agents?.defaults?.model) ?? "";
}

function resolveConfiguredModelKeys(cfg: HaoclawConfig): string[] {
...
export function applyPrimaryModel(cfg: HaoclawConfig, model: string): HaoclawConfig {
...
export function applyModelAllowlist(cfg: HaoclawConfig, models: string[]): HaoclawConfig {
  const defaults = cfg.agents?.defaults;
