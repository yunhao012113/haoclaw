...
describe("writeOAuthCredentials", () => {
  ...
  it("writes auth-profiles.json under HAOCLAW_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("haoclaw-oauth-");
  ...
  it("writes OAuth credentials to all sibling agent dirs when syncSiblingAgents=true", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "haoclaw-oauth-sync-"));
    ...
    process.env.HAOCLAW_AGENT_DIR = kidAgentDir;
  ...
  it("writes OAuth credentials only to target dir by default", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "haoclaw-oauth-nosync-"));
    process.env.HAOCLAW_STATE_DIR = tempStateDir;
  ...
  it("syncs siblings from explicit agentDir outside HAOCLAW_STATE_DIR", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "haoclaw-oauth-external-"));
    process.env.HAOCLAW_STATE_DIR = tempStateDir;

    // Create standard-layout agents tree *outside* HAOCLAW_STATE_DIR
...
describe("setMinimaxApiKey", () => {
  ...
  it("writes to HAOCLAW_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("haoclaw-minimax-", { agentSubdir: "custom-agent" });
    lifecycle.setStateDir(env.stateDir);
