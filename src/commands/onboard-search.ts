...
function rawKeyValue(config: HaoclawConfig, provider: SearchProvider): unknown {
...
export function hasExistingKey(config: HaoclawConfig, provider: SearchProvider): boolean {
...
function applyProviderOnly(config: HaoclawConfig, provider: SearchProvider): HaoclawConfig {
  return {
    ...config,
    tools: {
      ...config.tools,
...
function preserveDisabledState(original: HaoclawConfig, result: HaoclawConfig): HaoclawConfig {
...
export async function setupSearch(
  ...
  const choice = await prompter.select<PickerValue>({
    ...
    options: [
      ...options,
      {
        ...
        label: "Skip for now",
        hint: "Configure later with haoclaw configure --section web",
      },
    ],
  ...
  if (useSecretRefMode) {
    ...
    await prompter.note(
      [
        "Secret references enabled — Haoclaw will store a reference instead of the API key.",
