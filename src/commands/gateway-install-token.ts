...
import { readConfigFileSnapshot, writeConfigFile, type HaoclawConfig } from "../config/config.js";
...
function formatAmbiguousGatewayAuthModeReason(): string {
  return [
    ...
    `Set ${formatCliCommand("haoclaw config set gateway.auth.mode token")} or ${formatCliCommand("haoclaw config set gateway.auth.mode password")}.`,
  ].join(" ");
}
