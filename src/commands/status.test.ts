...
beforeAll(() => {
  envSnapshot = captureEnv(["HAOCLAW_PROFILE"]);
  process.env.HAOCLAW_PROFILE = "isolated";
});
...
vi.mock("../infra/haoclaw-root.js", () => ({
  resolveHaoclawPackageRoot: vi.fn().mockResolvedValue("/tmp/haoclaw"),
}));
...
vi.mock("../infra/update-check.js", () => ({
  checkUpdateStatus: vi.fn().mockResolvedValue({
    ...
    deps: {
      manager: "pnpm",
      status: "ok",
      lockfilePath: "/tmp/haoclaw/pnpm-lock.yaml",
      markerPath: "/tmp/haoclaw/node_modules/.modules.yaml",
    },
...
vi.mock("../daemon/service.js", () => ({
  resolveGatewayService: () => ({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/ai.haoclaw.gateway.plist",
    }),
  }),
}));
vi.mock("../daemon/node-service.js", () => ({
  resolveNodeService: () => ({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/ai.haoclaw.node.plist",
    }),
  }),
}));
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines otherwise", async () => {
    ...
    expect(
      logs.some(
        (line) =>
          line.includes("haoclaw status --all") ||
          line.includes("haoclaw --profile isolated status --all"),
      ),
    ).toBe(true);
  });

  it("shows gateway auth when reachable", async () => {
    await withEnvVar("HAOCLAW_GATEWAY_TOKEN", "abcd1234", async () => {
