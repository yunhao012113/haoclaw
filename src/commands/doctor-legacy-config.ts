...
export function normalizeCompatibilityConfigValues(cfg: HaoclawConfig): {
  config: HaoclawConfig;
  changes: string[];
} {
  const changes: string[] = [];
  let next: HaoclawConfig = cfg;
  ...
  const seedMissingDefaultAccountsFromSingleAccountBase = () => {
    ...
    next = {
      ...next,
      channels: nextChannels as HaoclawConfig["channels"],
    };
  };
  ...
  const normalizeBrowserSsrFPolicyAlias = () => {
    ...
    next = {
      ...next,
      browser: migratedBrowser as HaoclawConfig["browser"],
    };
    changes.push(
