...
type EnvSnapshot = {
  HOME?: string;
  HAOCLAW_HOME?: string;
  HAOCLAW_STATE_DIR?: string;
  HAOCLAW_OAUTH_DIR?: string;
};

function captureEnv(): EnvSnapshot {
  return {
    HOME: process.env.HOME,
    HAOCLAW_HOME: process.env.HAOCLAW_HOME,
    HAOCLAW_STATE_DIR: process.env.HAOCLAW_STATE_DIR,
    HAOCLAW_OAUTH_DIR: process.env.HAOCLAW_OAUTH_DIR,
  };
}
...
function setupSessionState(cfg: HaoclawConfig, env: NodeJS.ProcessEnv, homeDir: string) {
  const agentId = "main";
...
async function runStateIntegrity(cfg: HaoclawConfig) {
...
async function runStateIntegrityText(cfg: HaoclawConfig): Promise<string> {
...
describe("doctor state integrity oauth dir checks", () => {
  ...
  beforeEach(() => {
    ...
    tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "haoclaw-doctor-state-integrity-"));
    process.env.HOME = tempHome;
    process.env.HAOCLAW_HOME = tempHome;
    process.env.HAOCLAW_STATE_DIR = path.join(tempHome, ".haoclaw");
    delete process.env.HAOCLAW_OAUTH_DIR;
    fs.mkdirSync(process.env.HAOCLAW_STATE_DIR, { recursive: true, mode: 0o700 });
  ...
  it("prompts for oauth dir when HAOCLAW_OAUTH_DIR is explicitly configured", async () => {
    process.env.HAOCLAW_OAUTH_DIR = path.join(tempHome, ".oauth");
    const cfg: HaoclawConfig = {};
    const confirmSkipInNonInteractive = await runStateIntegrity(cfg);
    expect(confirmSkipInNonInteractive).toHaveBeenCalledWith(OAUTH_PROMPT_MATCHER);
    expect(stateIntegrityText()).toContain("CRITICAL: OAuth dir missing");
  ...
  it("prints haoclaw-only verification hints when recent sessions are missing transcripts", async () => {
    const cfg: HaoclawConfig = {};
    ...
    expect(text).toMatch(/haoclaw sessions --store ".*sessions\.json"/);
    expect(text).toMatch(/haoclaw sessions cleanup --store ".*sessions\.json" --dry-run/);
    expect(text).toMatch(
      /haoclaw sessions cleanup --store ".*sessions\.json" --enforce --fix-missing/,
    );
