...
export async function statusCommand(
  ...
  const daemonValue = (() => {
    ...
    const installedPrefix = daemon.managedByHaoclaw ? "installed · " : "";
  ...
  const nodeDaemonValue = (() => {
    ...
    const installedPrefix = nodeDaemon.managedByHaoclaw ? "installed · " : "";
  ...
  runtime.log(theme.heading("Haoclaw status"));
  runtime.log("");
  ...
  if (pairingRecovery) {
    ...
    if (pairingRecovery.requestId) {
      runtime.log(
        theme.muted(
          `Recovery: ${formatCliCommand(`haoclaw devices approve ${pairingRecovery.requestId}`)}`,
        ),
      );
    }
    runtime.log(theme.muted(`Fallback: ${formatCliCommand("haoclaw devices approve --latest")}`));
    runtime.log(theme.muted(`Inspect: ${formatCliCommand("haoclaw devices list")}`));
  }
  ...
  runtime.log(theme.muted(`Full report: ${formatCliCommand("haoclaw security audit")}`));
  runtime.log(theme.muted(`Deep probe: ${formatCliCommand("haoclaw security audit --deep")}`));
  ...
  runtime.log("");
  runtime.log("FAQ: https://docs.haoclaw.ai/faq");
  runtime.log("Troubleshooting: https://docs.haoclaw.ai/troubleshooting");
  runtime.log("");
  ...
  runtime.log(`  Need to share?      ${formatCliCommand("haoclaw status --all")}`);
  runtime.log(`  Need to debug live? ${formatCliCommand("haoclaw logs --follow")}`);
  if (gatewayReachable) {
    runtime.log(`  Need to test channels? ${formatCliCommand("haoclaw status --deep")}`);
  } else {
    runtime.log(`  Fix reachability first: ${formatCliCommand("haoclaw gateway probe")}`);
  }
}
