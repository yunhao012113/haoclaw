...
async function discoverOpenAiCompatibleModel(params: {
  baseUrl: string;
  apiKey?: string;
  runtime: RuntimeEnv;
}): Promise<string> {
  ...
  try {
    ...
    throw new Error(
      `Could not reach ${url} to auto-detect a model. Pass --model explicitly or verify the endpoint. (${error instanceof Error ? error.message : String(error)})`,
    );
  }

  if (!response.ok) {
    throw new Error(
      `Model auto-detection failed at ${url} (HTTP ${response.status}). Pass --model explicitly or verify the endpoint exposes /models.`,
    );
  }

  const payload = (await response.json()) as OpenAiModelsResponse;
  const firstModel = payload.data?.find((entry) => typeof entry.id === "string" && entry.id.trim());
  if (!firstModel?.id) {
    throw new Error(
      `No models were returned from ${url}. Pass --model explicitly or verify the endpoint catalog.`,
    );
  }

  params.runtime.log(`Auto-detected model: ${firstModel.id}`);
  return firstModel.id;
}

async function buildSimpleOnboardOptions(
  opts: SetupOptions | undefined,
  runtime: RuntimeEnv,
): Promise<OnboardOptions> {
  ...
  if (customProviderRequested && !baseUrl) {
    throw new Error("Simple setup with a custom API endpoint requires --base-url.");
  }
  ...
  if (customProviderRequested) {
    if (!model) {
      if ((provider ?? "openai-compatible") === "anthropic-compatible") {
        throw new Error(
          "Anthropic-compatible setup still requires --model. Automatic model discovery is only available for OpenAI-compatible endpoints.",
        );
      }
      model = await discoverOpenAiCompatibleModel({
        baseUrl,
        apiKey,
        runtime,
      });
...
export async function setupCommand(opts?: SetupOptions, runtime: RuntimeEnv = defaultRuntime) {
  if (hasSimpleSetupInputs(opts)) {
    const onboardingOpts = await buildSimpleOnboardOptions(opts, runtime);
