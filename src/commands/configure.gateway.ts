...
export async function promptGatewayConfig(
    ...
    if (tokenInputMode === "ref") {
      const envVar = guardCancel(
        await text({
          ...
          initialValue: "HAOCLAW_GATEWAY_TOKEN",
          placeholder: "HAOCLAW_GATEWAY_TOKEN",
          validate: (value) => {
            const candidate = String(value ?? "").trim();
            if (!isValidEnvSecretRefId(candidate)) {
              return "Use an env var name like HAOCLAW_GATEWAY_TOKEN.";
            }
      ...
      note(`Validated ${envVarName}. Haoclaw will store a token SecretRef.`, "Gateway token");
    } else {
  ...
  if (authMode === "trusted-proxy") {
    note(
      [
        "Trusted proxy mode: Haoclaw trusts user identity from a reverse proxy.",
        ...
        "Docs: https://docs.haoclaw.ai/gateway/trusted-proxy-auth",
      ].join("\n"),
      "Trusted Proxy Auth",
    );
