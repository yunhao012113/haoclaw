...
import type { HaoclawConfig, GatewayBindMode } from "../config/config.js";
...
function collectImplicitHeartbeatDirectPolicyWarnings(cfg: HaoclawConfig): string[] {
...
export async function noteSecurityWarnings(cfg: HaoclawConfig) {
  const warnings: string[] = [];
  const auditHint = `- Run: ${formatCliCommand("haoclaw security audit --deep")}`;

  if (cfg.approvals?.exec?.enabled === false) {
    warnings.push(
      "- Note: approvals.exec.enabled=false disables approval forwarding only.",
      "  Host exec gating still comes from ~/.haoclaw/exec-approvals.json.",
      `  Check local policy with: ${formatCliCommand("haoclaw approvals get --gateway")}`,
    );
  }
  ...
  const saferRemoteAccessLines = [
    ...
    "  Docs: https://docs.haoclaw.ai/gateway/remote",
  ];
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("haoclaw configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("haoclaw config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("haoclaw doctor --fix")} to generate a token`,
      ...
      warnings.push(
        ...
        `  Fix: ${formatCliCommand("haoclaw config set gateway.bind loopback")}`,
    ...
    if (dmScope === "main" && isMultiUserDm) {
      warnings.push(
        `- ${params.label} DMs: multiple senders share the main session; run: ` +
          formatCliCommand('haoclaw config set session.dmScope "per-channel-peer"') +
