...
export const resolveHaoclawPackageRoot = vi.fn().mockResolvedValue(null) as unknown as MockFn;
...
vi.mock("../plugins/loader.js", () => ({
  loadHaoclawPlugins: () => ({ plugins: [], diagnostics: [] }),
}));
...
beforeEach(() => {
  ...
  resolveHaoclawPackageRoot.mockReset().mockResolvedValue(null);
  ...
  setStdinTty(true);
  originalStateDir = process.env.HAOCLAW_STATE_DIR;
  originalUpdateInProgress = process.env.HAOCLAW_UPDATE_IN_PROGRESS;
  process.env.HAOCLAW_UPDATE_IN_PROGRESS = "1";
  tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "haoclaw-doctor-state-"));
...
afterEach(() => {
  ...
  if (originalStateDir === undefined) {
    ...
    process.env.HAOCLAW_STATE_DIR = originalStateDir;
  }
  if (originalUpdateInProgress === undefined) {
    delete process.env.HAOCLAW_UPDATE_IN_PROGRESS;
  } else {
    process.env.HAOCLAW_UPDATE_IN_PROGRESS = originalUpdateInProgress;
  }
  if (tempStateDir) {
