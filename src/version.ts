...
declare const __HAOCLAW_VERSION__: string | undefined;
const CORE_PACKAGE_NAME = "haoclaw";
...
export function resolveRuntimeServiceVersion(
  ...
  return (
    firstNonEmpty(
      env["HAOCLAW_VERSION"],
      runtimeVersion,
      env["HAOCLAW_SERVICE_VERSION"],
      env["npm_package_version"],
    ) ?? fallback
  );
}

// Single source of truth for the current Haoclaw version.
// - Embedded/bundled builds: injected define or env var.
// - Dev/npm builds: package.json.
export const VERSION = resolveBinaryVersion({
  moduleUrl: import.meta.url,
  injectedVersion: typeof __HAOCLAW_VERSION__ === "string" ? __HAOCLAW_VERSION__ : undefined,
  bundledVersion: process.env.HAOCLAW_BUNDLED_VERSION,
});
