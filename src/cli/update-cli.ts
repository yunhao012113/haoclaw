...
export function registerUpdateCli(program: Command) {
  const update = program
    .command("update")
    .description("Update Haoclaw and inspect update channel status")
    ...
    .addHelpText("after", () => {
      const examples = [
        ["haoclaw update", "Update a source checkout (git)"],
        ["haoclaw update --channel beta", "Switch to beta channel (git + npm)"],
        ["haoclaw update --channel dev", "Switch to dev channel (git + npm)"],
        ["haoclaw update --tag beta", "One-off update to a dist-tag or version"],
        ["haoclaw update --dry-run", "Preview actions without changing anything"],
        ["haoclaw update --no-restart", "Update without restarting the service"],
        ["haoclaw update --json", "Output result as JSON"],
        ["haoclaw update --yes", "Non-interactive (accept downgrade prompts)"],
        ["haoclaw update wizard", "Interactive update wizard"],
        ["haoclaw --update", "Shorthand for haoclaw update"],
      ] as const;
...
${theme.heading("Switch channels:")}
  ...
  - Run haoclaw update status to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.haoclaw.ai/cli/update")}`;
    })
    .action(async (opts) => {
      try {
        await updateCommand({
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["haoclaw update status", "Show channel + version status."],
          ["haoclaw update status --json", "JSON output."],
          ["haoclaw update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
          "- Shows current update channel (stable/beta/dev) and source",
        )}\n${theme.muted("- Includes git tag/branch/SHA for source checkouts")}\n\n${theme.muted(
          "Docs:",
        )} ${formatDocsLink("/cli/update", "docs.haoclaw.ai/cli/update")}`,
    )
    .action(async (opts, command) => {
      try {
