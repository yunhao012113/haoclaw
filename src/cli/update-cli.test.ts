...
import type { HaoclawConfig, ConfigFileSnapshot } from "../config/types.haoclaw.js";
...
vi.mock("../infra/haoclaw-root.js", () => ({
  resolveHaoclawPackageRoot: vi.fn(),
}));
...
const { resolveHaoclawPackageRoot } = await import("../infra/haoclaw-root.js");
...
describe("update-cli", () => {
  const fixtureRoot = "/tmp/haoclaw-update-tests";
  let fixtureCount = 0;
  ...
  const baseConfig = {} as HaoclawConfig;
  const baseSnapshot: ConfigFileSnapshot = {
    path: "/tmp/haoclaw-config.json",
    exists: true,
    raw: "{}",
    parsed: {},
    resolved: baseConfig,
  ...
  const mockPackageInstallStatus = (root: string) => {
    vi.mocked(resolveHaoclawPackageRoot).mockResolvedValue(root);
  ...
  const setupNonInteractiveDowngrade = async () => {
    const tempDir = createCaseDir("haoclaw-update");
    setTty(false);
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    vi.mocked(resolveHaoclawPackageRoot).mockResolvedValue(process.cwd());
    ...
    readPackageName.mockResolvedValue("haoclaw");
    ...
    prepareRestartScript.mockResolvedValue("/tmp/haoclaw-restart-test.sh");
    runRestartScript.mockResolvedValue(undefined);
    inspectPortUsage.mockResolvedValue({
      port: 18789,
      status: "busy",
      listeners: [{ pid: 4242, command: "haoclaw-gateway" }],
      hints: [],
    });
  ...
  it("updateStatusCommand prints table output", async () => {
    ...
    expect(logs.join("\n")).toContain("Haoclaw update status");
  });
    ...
    },
    {
      ...
      prepare: async () => {
        vi.mocked(readConfigFileSnapshot).mockResolvedValue({
          ...baseSnapshot,
          config: { update: { channel: "beta" } } as HaoclawConfig,
        });
      },
  ...
  it("honors --tag override", async () => {
    ...
    vi.mocked(resolveHaoclawPackageRoot).mockResolvedValue(tempDir);
    vi.mocked(runGatewayUpdate).mockResolvedValue(
      makeOkUpdateResult({
        mode: "npm",
      }),
  ...
  it("updateCommand refreshes service env from updated install root when available", async () => {
    const root = createCaseDir("haoclaw-updated-root");
  ...
  it("updateCommand continues after doctor sub-step and clears update flag", async () => {
    ...
    try {
      await withEnvAsync({ HAOCLAW_UPDATE_IN_PROGRESS: undefined }, async () => {
        vi.mocked(runGatewayUpdate).mockResolvedValue(makeOkUpdateResult());
        vi.mocked(runDaemonRestart).mockResolvedValue(true);
        vi.mocked(doctorCommand).mockResolvedValue(undefined);
        vi.mocked(defaultRuntime.log).mockClear();
        ...
        expect(process.env.HAOCLAW_UPDATE_IN_PROGRESS).toBeUndefined();
  ...
  it("updateWizardCommand offers dev checkout and forwards selections", async () => {
    const tempDir = createCaseDir("haoclaw-update-wizard");
    await withEnvAsync({ HAOCLAW_GIT_DIR: tempDir }, async () => {
      setTty(true);
