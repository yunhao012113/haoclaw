...
describe("skills-cli (e2e)", () => {
  ...
  beforeAll(() => {
    ...
    tempWorkspaceDir = fs.mkdtempSync(path.join(os.tmpdir(), "haoclaw-skills-test-"));
    tempBundledDir = fs.mkdtempSync(path.join(os.tmpdir(), "haoclaw-bundled-skills-test-"));
    process.env.HAOCLAW_BUNDLED_SKILLS_DIR = tempBundledDir;
  });
