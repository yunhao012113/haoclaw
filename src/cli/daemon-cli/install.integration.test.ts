...
describe("runDaemonInstall integration", () => {
  ...
  beforeAll(async () => {
    ...
    tempHome = await makeTempWorkspace("haoclaw-daemon-install-int-");
    configPath = path.join(tempHome, "haoclaw.json");
    process.env.HOME = tempHome;
    process.env.HAOCLAW_STATE_DIR = tempHome;
  ...
  it("auto-mints token when no source exists without embedding it into service env", async () => {
    ...
    expect(installEnv?.HAOCLAW_GATEWAY_TOKEN).toBeUndefined();
  });
});
