...
const serviceReadCommand = vi.fn(async (_env?: NodeJS.ProcessEnv) => ({
  ...
  environment: {
    HAOCLAW_STATE_DIR: "/tmp/haoclaw-daemon",
    HAOCLAW_CONFIG_PATH: "/tmp/haoclaw-daemon/haoclaw.json",
  },
}));
...
const resolveStateDir = vi.fn(
  (env: NodeJS.ProcessEnv) => env.HAOCLAW_STATE_DIR ?? "/tmp/haoclaw-cli",
);
const resolveConfigPath = vi.fn((env: NodeJS.ProcessEnv, stateDir: string) => {
  return env.HAOCLAW_CONFIG_PATH ?? `${stateDir}/haoclaw.json`;
});
...
vi.mock("../../config/config.js", () => ({
  createConfigIO: ({ configPath }: { configPath: string }) => {
    const isDaemon = configPath.includes("/haoclaw-daemon/");
    return {
...
describe("gatherDaemonStatus", () => {
  ...
  beforeEach(() => {
    ...
    process.env.HAOCLAW_STATE_DIR = "/tmp/haoclaw-cli";
    process.env.HAOCLAW_CONFIG_PATH = "/tmp/haoclaw-cli/haoclaw.json";
  ...
  it("keeps remote probe auth strict when remote token is missing", async () => {
    ...
    process.env.HAOCLAW_GATEWAY_PASSWORD = "env-password"; // pragma: allowlist secret
