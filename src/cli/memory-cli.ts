    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["haoclaw memory status", "Show index and provider status."],
          ["haoclaw memory status --deep", "Probe embedding provider readiness."],
          ["haoclaw memory index --force", "Force a full reindex."],
          ['haoclaw memory search "meeting notes"', "Quick search using positional query."],
          [
            'haoclaw memory search --query "deployment" --max-results 20',
            "Limit results for focused troubleshooting.",
          ],
          ["haoclaw memory status --json", "Output machine-readable JSON (good for scripts)."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/memory", "docs.haoclaw.ai/cli/memory")}\n`,
    );
