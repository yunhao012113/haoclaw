...
const coreEntries: CoreCliEntry[] = [
  {
    commands: [
      {
        name: "deploy",
        description: "One-click deploy for provider setup, gateway start, and health check",
        hasSubcommands: false,
      },
    ],
    register: async ({ program }) => {
      const mod = await import("./register.deploy.js");
      mod.registerDeployCommand(program);
    },
  },
  {
    commands: [
  ...
  },
  {
    commands: [
      {
        name: "browser",
        description: "Manage Haoclaw's dedicated browser (Chrome/Chromium)",
        hasSubcommands: true,
      },
    ],
    register: async ({ program }) => {
