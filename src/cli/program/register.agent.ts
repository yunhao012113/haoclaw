...
${formatHelpExamples([
  ['haoclaw agent --to +15555550123 --message "status update"', "Start a new session."],
  ['haoclaw agent --agent ops --message "Summarize logs"', "Use a specific agent."],
  [
    'haoclaw agent --session-id 1234 --message "Summarize inbox" --thinking medium',
    "Target a session with explicit thinking level.",
  ],
  [
    'haoclaw agent --to +15555550123 --message "Trace logs" --verbose on --json',
    "Enable verbose logging and JSON output.",
  ],
  ['haoclaw agent --to +15555550123 --message "Summon reply" --deliver', "Deliver reply."],
  [
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/agent", "docs.haoclaw.ai/cli/agent")}`,
    )
    .action(async (opts) => {
  ...
  const agents = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agents", "docs.haoclaw.ai/cli/agents")}\n`,
    );
...
${theme.heading("Examples:")}
${formatHelpExamples([
  ['haoclaw agents set-identity --agent main --name "Haoclaw" --emoji "🐙"', "Set name + emoji."],
  ["haoclaw agents set-identity --agent main --avatar avatars/haoclaw.png", "Set avatar path."],
  [
    "haoclaw agents set-identity --workspace ~/.haoclaw/workspace --from-identity",
    "Load from IDENTITY.md.",
  ],
  [
    "haoclaw agents set-identity --identity-file ~/.haoclaw/workspace/IDENTITY.md --agent main",
