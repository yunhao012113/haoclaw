...
const buildGatewayInstallPlan = vi.fn(
  async (params: { port: number; token?: string; env?: NodeJS.ProcessEnv }) => ({
    ...
    environment: {
      HAOCLAW_GATEWAY_PORT: String(params.port),
      ...(params.token ? { HAOCLAW_GATEWAY_TOKEN: params.token } : {}),
    },
  }),
);
...
describe("daemon-cli coverage", () => {
  ...
  beforeEach(() => {
    ...
    process.env.HAOCLAW_STATE_DIR = "/tmp/haoclaw-cli-state";
    process.env.HAOCLAW_CONFIG_PATH = "/tmp/haoclaw-cli-state/haoclaw.json";
  ...
  it("derives probe URL from service args + env (json)", async () => {
    ...
    serviceReadCommand.mockResolvedValueOnce({
      ...
      environment: {
        HAOCLAW_PROFILE: "dev",
        HAOCLAW_STATE_DIR: "/tmp/haoclaw-daemon-state",
        HAOCLAW_CONFIG_PATH: "/tmp/haoclaw-daemon-state/haoclaw.json",
        HAOCLAW_GATEWAY_PORT: "19001",
      },
      sourcePath: "/tmp/ai.haoclaw.gateway.plist",
    });
