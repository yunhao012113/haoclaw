    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["haoclaw nodes status", "List known nodes with live status."],
          ["haoclaw nodes pairing pending", "Show pending node pairing requests."],
          ['haoclaw nodes run --node <id> --raw "uname -a"', "Run a shell command on a node."],
          ["haoclaw nodes camera snap --node <id>", "Capture a photo from a node camera."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/nodes", "docs.haoclaw.ai/cli/nodes")}\n`,
    );
