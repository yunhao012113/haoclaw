...
describe("runCommandWithTimeout", () => {
  ...
  it("merges custom env with base env and drops undefined values", async () => {
    const resolved = resolveCommandEnv({
      argv: ["node", "script.js"],
      baseEnv: {
        HAOCLAW_BASE_ENV: "base",
        HAOCLAW_TO_REMOVE: undefined,
      },
      env: {
        HAOCLAW_TEST_ENV: "ok",
      },
    ...
    expect(resolved.HAOCLAW_BASE_ENV).toBe("base");
    expect(resolved.HAOCLAW_TEST_ENV).toBe("ok");
    expect(resolved.HAOCLAW_TO_REMOVE).toBeUndefined();
  });
