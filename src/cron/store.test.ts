...
const { makeStorePath } = createCronStoreHarness({ prefix: "haoclaw-cron-store-" });
...
describe("resolveCronStorePath", () => {
  ...
  it("uses HAOCLAW_HOME for tilde expansion", () => {
    vi.stubEnv("HAOCLAW_HOME", "/srv/haoclaw-home");
    vi.stubEnv("HOME", "/home/other");

    const result = resolveCronStorePath("~/cron/jobs.json");
    expect(result).toBe(path.resolve("/srv/haoclaw-home", "cron", "jobs.json"));
  });
});
