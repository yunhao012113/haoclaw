...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeHelper(fn, { prefix: "haoclaw-cron-submodel-" });
}

async function writeSessionStore(home: string) {
  const dir = path.join(home, ".haoclaw", "sessions");
...
function makeCfg(
  home: string,
  storePath: string,
  overrides: Partial<HaoclawConfig> = {},
): HaoclawConfig {
  const base: HaoclawConfig = {
    agents: {
      defaults: {
...
async function runSubagentModelCase(params: {
  home: string;
  cfgOverrides?: Partial<HaoclawConfig>;
  ...
  it.each([
    {
      ...
      } satisfies Partial<HaoclawConfig>,
