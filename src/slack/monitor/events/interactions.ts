...
// Prefix for Haoclaw-generated action IDs to scope our handler
const HAOCLAW_ACTION_PREFIX = "haoclaw:";
...
export function registerSlackInteractionEvents(params: { ctx: SlackMonitorContext }) {
  ...
  const modalMatcher = new RegExp(`^${HAOCLAW_ACTION_PREFIX}`);

  // Handle Haoclaw modal submissions with callback_ids scoped by our prefix.
  registerModalLifecycleHandler({
