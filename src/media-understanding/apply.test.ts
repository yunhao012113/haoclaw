...
const TEMP_MEDIA_PREFIX = "haoclaw-media-";
...
function createGroqAudioConfig(): HaoclawConfig {
  return {
    tools: {
      media: {
        audio: {
...
function createMediaDisabledConfig(): HaoclawConfig {
  return {
    tools: {
      media: {
        audio: { enabled: false },
...
function createMediaDisabledConfigWithAllowedMimes(allowedMimes: string[]): HaoclawConfig {
  return {
...
async function setupAudioAutoDetectCase(stdout: string): Promise<{
  ...
  const cfg: HaoclawConfig = { tools: { media: { audio: {} } } };
...
describe("applyMediaUnderstanding", () => {
  ...
  beforeAll(async () => {
    const baseDir = resolvePreferredHaoclawTmpDir();
  ...
  it("skips audio auto-detect when no supported binaries or provider keys are available", async () => {
    ...
    await withMediaAutoDetectEnv(
      {
        PATH: emptyBinDir,
        HAOCLAW_AGENT_DIR: isolatedAgentDir,
