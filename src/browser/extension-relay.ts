...
const RELAY_AUTH_HEADER = "x-haoclaw-relay-token";
  ...
  const initPromise = (async (): Promise<ChromeExtensionRelayServer> => {
    ...
    const routeCdpCommand = async (cmd: CdpCommand): Promise<unknown> => {
      switch (cmd.method) {
        case "Browser.getVersion":
          return {
            protocolVersion: "1.3",
            product: "Chrome/Haoclaw-Extension-Relay",
            revision: "0",
            userAgent: "Haoclaw-Extension-Relay",
            jsVersion: "V8",
          };
    ...
    const server = createServer((req, res) => {
      ...
      if (
        ...
        const payload: Record<string, unknown> = {
          Browser: "Haoclaw/extension-relay",
    ...
    try {
      ...
      if (
        ...
        (await probeAuthenticatedHaoclawRelay({
