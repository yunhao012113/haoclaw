import type { BrowserConfig, BrowserProfileConfig, HaoclawConfig } from "../config/config.js";
...
import {
  ...
  DEFAULT_HAOCLAW_BROWSER_ENABLED,
...
function normalizeHexColor(raw: string | undefined) {
  ...
  if (!value) {
    return DEFAULT_HAOCLAW_BROWSER_COLOR;
  }
...
/**
 * Ensure the default "haoclaw" profile exists in the profiles map.
...
function ensureDefaultProfile(
  ...
  if (!result[DEFAULT_HAOCLAW_BROWSER_PROFILE_NAME]) {
    result[DEFAULT_HAOCLAW_BROWSER_PROFILE_NAME] = {
 ...
 *
 * Note: this is an Haoclaw browser profile (routing config), not a Chrome user profile.
...
function ensureDefaultChromeExtensionProfile(
  ...
  // (legacy single-profile configs may use controlPort+1 for haoclaw/haoclaw CDP).
...
export function resolveBrowserConfig(
  ...
  rootConfig?: HaoclawConfig,
): ResolvedBrowserConfig {
  const enabled = cfg?.enabled ?? DEFAULT_HAOCLAW_BROWSER_ENABLED;
  ...
  const defaultProfile =
    ...
    (profiles[DEFAULT_BROWSER_DEFAULT_PROFILE_NAME]
      ? DEFAULT_BROWSER_DEFAULT_PROFILE_NAME
      : profiles[DEFAULT_HAOCLAW_BROWSER_PROFILE_NAME]
        ? DEFAULT_HAOCLAW_BROWSER_PROFILE_NAME
        : "chrome");
...
export function resolveProfile(
  ...
  let cdpUrl = "";
  const driver = profile.driver === "extension" ? "extension" : "haoclaw";
