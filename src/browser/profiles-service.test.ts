...
vi.mock("./chrome.js", () => ({
  resolveHaoclawUserDataDir: vi.fn(() => "/tmp/haoclaw-test/haoclaw/user-data"),
}));

import { loadConfig, writeConfigFile } from "../config/config.js";
import { resolveHaoclawUserDataDir } from "./chrome.js";
...
describe("BrowserProfilesService", () => {
  ...
  it("deletes local profiles and moves data to Trash", async () => {
    ...
    const tempDir = fs.mkdtempSync(path.join("/tmp", "haoclaw-profile-"));
    const userDataDir = path.join(tempDir, "work", "user-data");
    fs.mkdirSync(path.dirname(userDataDir), { recursive: true });
    vi.mocked(resolveHaoclawUserDataDir).mockReturnValue(userDataDir);
