...
describe("browser config", () => {
  it("defaults to enabled with loopback defaults and octopus-orange color", () => {
    ...
    expect(profile?.name).toBe("haoclaw");
    expect(profile?.driver).toBe("haoclaw");
    expect(profile?.cdpPort).toBe(18800);
    expect(profile?.cdpUrl).toBe("http://127.0.0.1:18800");

    const haoclaw = resolveProfile(resolved, "haoclaw");
    expect(haoclaw?.driver).toBe("haoclaw");
    expect(haoclaw?.cdpPort).toBe(18800);
    expect(haoclaw?.cdpUrl).toBe("http://127.0.0.1:18800");
  ...
  it("derives default ports from HAOCLAW_GATEWAY_PORT when unset", () => {
    withEnv({ HAOCLAW_GATEWAY_PORT: "19001" }, () => {
      const resolved = resolveBrowserConfig(undefined);
      ...
      const haoclaw = resolveProfile(resolved, "haoclaw");
      expect(haoclaw?.cdpPort).toBe(19012);
      expect(haoclaw?.cdpUrl).toBe("http://127.0.0.1:19012");
    });
  });

  it("derives default ports from gateway.port when env is unset", () => {
    withEnv({ HAOCLAW_GATEWAY_PORT: undefined }, () => {
      ...
      const haoclaw = resolveProfile(resolved, "haoclaw");
      expect(haoclaw?.cdpPort).toBe(19022);
      expect(haoclaw?.cdpUrl).toBe("http://127.0.0.1:19022");
    });
  });
  ...
  it("supports overriding the local CDP auto-allocation range start", () => {
    ...
    expect(haoclaw?.cdpPort).toBe(19000);
    expect(haoclaw?.cdpUrl).toBe("http://127.0.0.1:19000");
  });
  ...
  it("treats non-loopback cdpUrl as remote", () => {
    ...
    const profile = resolveProfile(resolved, "haoclaw");
  ...
  it("does not add the built-in chrome extension profile if the derived relay port is already used", () => {
    const resolved = resolveBrowserConfig({
      profiles: {
        haoclaw: { cdpPort: 18792, color: "#FF4500" },
      },
    });
    expect(resolveProfile(resolved, "chrome")).toBe(null);
    expect(resolved.defaultProfile).toBe("haoclaw");
  });
  ...
  describe("default profile preference", () => {
    it("defaults to haoclaw profile when defaultProfile is not configured", () => {
      const resolved = resolveBrowserConfig({
        headless: false,
        noSandbox: false,
      });
    ...
    it("keeps haoclaw default when headless=true", () => {
      const resolved = resolveBrowserConfig({
        headless: true,
      });
      expect(resolved.defaultProfile).toBe("haoclaw");
    ...
    it("keeps haoclaw default when noSandbox=true", () => {
      const resolved = resolveBrowserConfig({
        noSandbox: true,
      });
      expect(resolved.defaultProfile).toBe("haoclaw");
    ...
    it("keeps haoclaw default when both headless and noSandbox are true", () => {
      const resolved = resolveBrowserConfig({
        headless: true,
        noSandbox: true,
      });
