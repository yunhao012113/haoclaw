...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".haoclaw-profile-decorated");
}
...
/**
 * Best-effort profile decoration (name + octopus-orange). Chrome preference keys
 * vary by version; we keep this conservative and idempotent.
 */
export function decorateHaoclawProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string },
) {
  const desiredName = opts?.name ?? DEFAULT_HAOCLAW_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_HAOCLAW_BROWSER_COLOR).toUpperCase();
