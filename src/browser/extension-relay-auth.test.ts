...
import {
  probeAuthenticatedHaoclawRelay,
...
async function probeRelay(baseUrl: string, relayAuthToken: string): Promise<boolean> {
  return await probeAuthenticatedHaoclawRelay({
    baseUrl,
    relayAuthHeader: "x-haoclaw-relay-token",
    relayAuthToken,
  });
}
...
describe("extension-relay-auth", () => {
  ...
  it("accepts authenticated haoclaw relay probe responses", async () => {
