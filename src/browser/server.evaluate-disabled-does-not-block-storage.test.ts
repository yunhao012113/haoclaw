  ...
  return {
    ...
    loadConfig: () => ({
      browser: {
        ...
        profiles: {
          haoclaw: { cdpPort: testPort + 1, color: "#FF4500" },
        },
      },
    }),
...
describe("browser control evaluate gating", () => {
  beforeEach(async () => {
    ...
    prevGatewayPort = process.env.HAOCLAW_GATEWAY_PORT;
    process.env.HAOCLAW_GATEWAY_PORT = String(testPort - 2);
    prevGatewayToken = process.env.HAOCLAW_GATEWAY_TOKEN;
    prevGatewayPassword = process.env.HAOCLAW_GATEWAY_PASSWORD;
