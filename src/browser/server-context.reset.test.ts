...
function localHaoclawProfile(): Parameters<typeof createProfileResetOps>[0]["profile"] {
  return {
    name: "haoclaw",
...
function createLocalHaoclawResetOps(
  params: Omit<Parameters<typeof createProfileResetOps>[0], "profile">,
) {
  return createProfileResetOps({ profile: localHaoclawProfile(), ...params });
}
...
function createStatelessResetOps(profile: Parameters<typeof createProfileResetOps>[0]["profile"]) {
  return createProfileResetOps({
    ...
    resolveHaoclawUserDataDir: (name: string) => `/tmp/${name}`,
  });
}
...
describe("createProfileResetOps", () => {
  ...
  it("stops local browser, closes playwright connection, and trashes profile dir", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "haoclaw-reset-"));
    const profileDir = path.join(tempRoot, "haoclaw");
    ...
    const ops = createLocalHaoclawResetOps({
      getProfileState,
      stopRunningBrowser,
      isHttpReachable,
      resolveHaoclawUserDataDir: () => profileDir,
    });
  ...
  it("forces playwright disconnect when loopback cdp is occupied by non-owned process", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "haoclaw-reset-no-own-"));
    const profileDir = path.join(tempRoot, "haoclaw");
    fs.mkdirSync(profileDir, { recursive: true });
