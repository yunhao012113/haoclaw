...
export function resolveHaoclawUserDataDir(profileName = DEFAULT_HAOCLAW_BROWSER_PROFILE_NAME) {
...
export async function launchHaoclawChrome(
  ...
  const userDataDir = resolveHaoclawUserDataDir(profile.name);
  fs.mkdirSync(userDataDir, { recursive: true });

  const needsDecorate = !isProfileDecorated(
    userDataDir,
    profile.name,
    (profile.color ?? DEFAULT_HAOCLAW_BROWSER_COLOR).toUpperCase(),
  );
  ...
  if (needsDecorate) {
    try {
      decorateHaoclawProfile(userDataDir, {
        name: profile.name,
        color: profile.color,
      });
      log.info(`🦞 haoclaw browser profile decorated (${profile.color})`);
    } catch (err) {
      log.warn(`haoclaw browser profile decoration failed: ${String(err)}`);
    }
  }
  ...
  try {
    ...
    log.warn(`haoclaw browser clean-exit prefs failed: ${String(err)}`);
  }
  ...
  log.info(
    `🦞 haoclaw browser started (${exe.kind}) profile "${profile.name}" on 127.0.0.1:${profile.cdpPort} (pid ${pid})`,
  );
...
export async function stopHaoclawChrome(
