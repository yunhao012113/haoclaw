...
import { planHaoclawModelsJson } from "./models-config.plan.js";
...
function resolveModelsConfigInput(config?: HaoclawConfig): HaoclawConfig {
...
export async function ensureHaoclawModelsJson(
  config?: HaoclawConfig,
  agentDirOverride?: string,
): Promise<{ agentDir: string; wrote: boolean }> {
  const cfg = resolveModelsConfigInput(config);
  const agentDir = agentDirOverride?.trim() ? agentDirOverride.trim() : resolveHaoclawAgentDir();
  ...
  return await withModelsJsonWriteLock(targetPath, async () => {
    ...
    const plan = await planHaoclawModelsJson({
      cfg,
      agentDir,
      env,
