...
} from "./haoclaw-tools.subagents.test-harness.js";
import {
...
describe("haoclaw-tools: subagents steer failure", () => {
  beforeEach(() => {
    resetSubagentRegistryForTests();
    callGatewayMock.mockClear();
    const storePath = path.join(
      os.tmpdir(),
      `haoclaw-subagents-steer-${Date.now()}-${Math.random().toString(16).slice(2)}.json`,
    );
    setSubagentsConfigOverride({
      session: {
        mainKey: "main",
