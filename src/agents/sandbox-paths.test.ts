...
import { resolvePreferredHaoclawTmpDir } from "../infra/tmp-haoclaw-dir.js";
...
async function withOutsideHardlinkInHaoclawTmp<T>(
  params: {
...
describe("resolveSandboxedMediaSource", () => {
  ...
  it.each([
    ...
    {
      name: "absolute paths under host tmp outside haoclaw tmp root",
      media: path.join(os.tmpdir(), "outside-haoclaw", "passwd"),
      expected: /sandbox/i,
    },
    {
  ...
  it("rejects symlinked Haoclaw tmp paths escaping tmp root", async () => {
  ...
  it("rejects hardlinked Haoclaw tmp paths to outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInHaoclawTmp(
      {
        openClawTmpDir,
  ...
  it("rejects symlinked Haoclaw tmp paths to hardlinked outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInHaoclawTmp(
