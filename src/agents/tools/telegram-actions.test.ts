...
describe("handleTelegramAction", () => {
  ...
  function reactionConfig(reactionLevel: "minimal" | "extensive" | "off" | "ack"): HaoclawConfig {
    return {
      channels: { telegram: { botToken: "tok", reactionLevel } },
    } as HaoclawConfig;
  }

  function telegramConfig(overrides?: Record<string, unknown>): HaoclawConfig {
    return {
      channels: {
        telegram: {
          botToken: "tok",
  ...
  it("rejects sticker actions when disabled by default", async () => {
    const cfg = { channels: { telegram: { botToken: "tok" } } } as HaoclawConfig;
    await expect(
...
describe("handleTelegramAction per-account gating", () => {
  ...
  async function expectAccountStickerSend(cfg: HaoclawConfig, accountId = "media") {
    await handleTelegramAction(
