...
describe("web_search kimi provider", () => {
  ...
  it("runs the Kimi web_search tool flow and echoes tool results", async () => {
      ...
      if (idx === 1) {
        return new Response(
          JSON.stringify({
                  ...
                  tool_calls: [
                    {
                      ...
                      function: {
                        name: "$web_search",
                        arguments: JSON.stringify({ q: "haoclaw" }),
                      },
                    },
                  ],
                },
            ...
            search_results: [
              { title: "Haoclaw", url: "https://haoclaw.ai/docs", content: "docs" },
            ],
          }),
    ...
    const result = await tool?.execute?.("call-1", { query: "latest haoclaw release" });
    ...
    expect(JSON.parse(toolMessage?.content ?? "{}")).toMatchObject({
      search_results: [{ url: "https://haoclaw.ai/docs" }],
    });
    ...
    expect(details.citations).toEqual(["https://haoclaw.ai/docs"]);
