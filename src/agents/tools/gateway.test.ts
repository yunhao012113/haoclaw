...
describe("gateway tool defaults", () => {
  const envSnapshot = {
    haoclaw: process.env.HAOCLAW_GATEWAY_TOKEN,
  ...
  afterAll(() => {
    if (envSnapshot.haoclaw === undefined) {
      delete process.env.HAOCLAW_GATEWAY_TOKEN;
    } else {
      process.env.HAOCLAW_GATEWAY_TOKEN = envSnapshot.haoclaw;
    }
  ...
  it("uses HAOCLAW_GATEWAY_TOKEN for allowlisted local overrides", () => {
  ...
  it("does not leak local env/config tokens to remote overrides", () => {
    process.env.HAOCLAW_GATEWAY_TOKEN = "local-env-token";
