...
type AgentEntry = NonNullable<NonNullable<HaoclawConfig["agents"]>["list"]>[number];
...
export function listAgentEntries(cfg: HaoclawConfig): AgentEntry[] {
...
export function listAgentIds(cfg: HaoclawConfig): string[] {
...
export function resolveDefaultAgentId(cfg: HaoclawConfig): string {
  const agents = listAgentEntries(cfg);
  if (agents.length === 0) {
    return DEFAULT_AGENT_ID;
  }
...
function resolveAgentEntry(cfg: HaoclawConfig, agentId: string): AgentEntry | undefined {
...
export function resolveAgentModelPrimary(cfg: HaoclawConfig, agentId: string): string | undefined {
...
export function resolveAgentWorkspaceDir(cfg: HaoclawConfig, agentId: string) {
  const id = normalizeAgentId(agentId);
...
export function resolveAgentDir(cfg: HaoclawConfig, agentId: string) {
  const id = normalizeAgentId(agentId);
