...
function parseInstallSpec(input: unknown): SkillInstallSpec | undefined {
  const parsed = parseHaoclawManifestInstallBase(input, ["brew", "node", "go", "uv", "download"]);
  if (!parsed) {
    return undefined;
  }
  const { raw } = parsed;
  const spec = applyHaoclawManifestInstallCommonFields<SkillInstallSpec>(
    {
...
): HaoclawSkillMetadata | undefined {
