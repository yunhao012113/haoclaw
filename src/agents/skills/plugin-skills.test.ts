    ...
    plugins: [
      {
        ...
        manifestPath: path.join(params.acpxRoot, "haoclaw.plugin.json"),
      },
      {
        id: "helper",
        name: "Helper",
        ...
        manifestPath: path.join(params.helperRoot, "haoclaw.plugin.json"),
      },
    ],
  };
}
    ...
    plugins: [
      {
        ...
        manifestPath: path.join(params.pluginRoot, "haoclaw.plugin.json"),
      },
    ],
  };
}
...
async function setupAcpxAndHelperRegistry() {
  const workspaceDir = await tempDirs.make("haoclaw-");
  const acpxRoot = await tempDirs.make("haoclaw-acpx-plugin-");
  const helperRoot = await tempDirs.make("haoclaw-helper-plugin-");
...
async function setupPluginOutsideSkills() {
  const workspaceDir = await tempDirs.make("haoclaw-");
  const pluginRoot = await tempDirs.make("haoclaw-plugin-");
  const outsideDir = await tempDirs.make("haoclaw-outside-");
