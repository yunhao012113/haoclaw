 ...
 * A common mistake — caused by the similarity with the `haoclaw.json`
...
function normalizeRawCredentialEntry(raw: Record<string, unknown>): Partial<AuthProfileCredential> {
  ...
  // mode → type alias (haoclaw.json uses "mode"; auth-profiles.json uses "type")
...
function loadAuthProfileStoreForAgent(
  ...
  const forceReadOnly = process.env.HAOCLAW_AUTH_STORE_READONLY === "1";
