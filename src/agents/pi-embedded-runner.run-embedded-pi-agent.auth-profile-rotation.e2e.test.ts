...
vi.mock("./models-config.js", async (importOriginal) => {
  ...
  return {
    ...mod,
    ensureHaoclawModelsJson: vi.fn(async () => ({ wrote: false })),
  };
});
...
const makeConfig = (opts?: { fallbacks?: string[]; apiKey?: string }): HaoclawConfig =>
  ({
    agents: {
      defaults: {
        model: {
...
const makeAgentOverrideOnlyFallbackConfig = (agentId: string): HaoclawConfig =>
  ({
    agents: {
      defaults: {
        model: {
...
const makeCopilotConfig = (): HaoclawConfig =>
  ({
    models: {
      providers: {
        "github-copilot": {
...
async function withTimedAgentWorkspace<T>(
  ...
  vi.useFakeTimers();
  try {
    const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "haoclaw-agent-"));
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "haoclaw-workspace-"));
    const now = Date.now();
