...
test("exec sets HAOCLAW_SHELL in pty mode", async () => {
  const tool = createExecTool({ allowBackground: false, security: "full", ask: "off" });
  const result = await tool.execute("toolcall-haoclaw-shell", {
    command: "node -e \"process.stdout.write(process.env.HAOCLAW_SHELL || '')\"",
    pty: true,
  });
