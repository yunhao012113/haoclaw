...
const GOOGLE_LIVE_SINGLE_KEY = "HAOCLAW_LIVE_GEMINI_KEY";
...
const PROVIDER_API_KEY_CONFIG: Record<string, Omit<ProviderApiKeyConfig, "fallbackVars">> = {
  anthropic: {
    liveSingle: "HAOCLAW_LIVE_ANTHROPIC_KEY",
    listVar: "HAOCLAW_LIVE_ANTHROPIC_KEYS",
  ...
  openai: {
    liveSingle: "HAOCLAW_LIVE_OPENAI_KEY",
...
function resolveProviderApiKeyConfig(provider: string): ProviderApiKeyConfig {
  ...
  const liveSingle = custom?.liveSingle ?? `HAOCLAW_LIVE_${base}_KEY`;
