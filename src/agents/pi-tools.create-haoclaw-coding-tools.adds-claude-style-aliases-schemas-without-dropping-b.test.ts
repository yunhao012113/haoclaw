...
const defaultTools = createHaoclawCodingTools({ senderIsOwner: true });
...
describe("createHaoclawCodingTools", () => {
  ...
  it("enforces apply_patch availability and canonical names across model/provider constraints", () => {
    ...
    const enabledConfig: HaoclawConfig = {
      tools: {
        exec: {
    ...
    const openAiTools = createHaoclawCodingTools({
    ...
    const anthropicTools = createHaoclawCodingTools({
      config: enabledConfig,
    ...
    const allowModelsConfig: HaoclawConfig = {
      tools: {
        exec: {
    ...
    const allowed = createHaoclawCodingTools({
    ...
    const denied = createHaoclawCodingTools({
      config: allowModelsConfig,
      modelProvider: "openai",
      modelId: "gpt-5-mini",
    });
    ...
    const oauthTools = createHaoclawCodingTools({
      modelProvider: "anthropic",
      modelAuthMode: "oauth",
    });
