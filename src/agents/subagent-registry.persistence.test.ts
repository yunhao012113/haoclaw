...
describe("subagent registry persistence", () => {
  const envSnapshot = captureEnv(["HAOCLAW_STATE_DIR"]);
  ...
  const writePersistedRegistry = async (
    ...
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "haoclaw-subagent-"));
  ...
  it("uses isolated temp state when HAOCLAW_STATE_DIR is unset in tests", async () => {
    ...
    expect(registryPath).toContain(path.join(os.tmpdir(), "haoclaw-test-state"));
  });
});
