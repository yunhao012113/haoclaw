...
describe("subagent announce formatting", () => {
  ...
  beforeAll(async () => {
    ...
    // See: https://github.com/haoclaw/haoclaw/issues/31298
    previousFastTestEnv = process.env.HAOCLAW_TEST_FAST;
  ...
  afterAll(() => {
    if (previousFastTestEnv === undefined) {
      delete process.env.HAOCLAW_TEST_FAST;
      return;
    }
    process.env.HAOCLAW_TEST_FAST = previousFastTestEnv;
  });

  beforeEach(() => {
    // HAOCLAW_TEST_FAST is set in beforeAll before module import
  ...
  it("sends instructional message to main agent with status and findings", async () => {
    ...
    expect(msg).toContain("Haoclaw runtime context (internal):");
