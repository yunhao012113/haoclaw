...
function buildCoreToolGroupMap() {
  ...
  const haoclawTools = CORE_TOOL_DEFINITIONS.filter((tool) => tool.includeInHaoclawGroup).map(
    (tool) => tool.id,
  );
  return {
    "group:haoclaw": haoclawTools,
