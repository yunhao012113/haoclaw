...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "haoclaw.sandbox=1");
  args.push("--label", `haoclaw.sessionKey=${params.scopeKey}`);
  args.push("--label", `haoclaw.createdAtMs=${createdAtMs}`);
  if (params.configHash) {
    args.push("--label", `haoclaw.configHash=${params.configHash}`);
  }
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  return await readDockerContainerLabel(containerName, "haoclaw.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`haoclaw sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`haoclaw sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("haoclaw sandbox recreate --all");
}
