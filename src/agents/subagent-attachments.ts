...
function resolveAttachmentLimits(config: HaoclawConfig): AttachmentLimits {
...
export async function materializeSubagentAttachments(params: {
  ...
  const absRootDir = path.join(childWorkspaceDir, ".haoclaw", "attachments");
  const relDir = path.posix.join(".haoclaw", "attachments", attachmentId);
