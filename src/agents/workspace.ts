...
export function resolveDefaultAgentWorkspaceDir(
  ...
  const profile = env.HAOCLAW_PROFILE?.trim();
  if (profile && profile.toLowerCase() !== "default") {
    return path.join(home, ".haoclaw", `workspace-${profile}`);
  }
  return path.join(home, ".haoclaw", "workspace");
}
...
const WORKSPACE_STATE_DIRNAME = ".haoclaw";
