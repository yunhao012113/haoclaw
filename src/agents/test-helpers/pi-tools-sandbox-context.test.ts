...
describe("createPiToolsSandboxContext", () => {
  it("provides stable defaults for pi-tools sandbox tests", () => {
    ...
    expect(sandbox.containerName).toBe("haoclaw-sbx-test");
    expect(sandbox.containerWorkdir).toBe("/workspace");
    expect(sandbox.docker.image).toBe("haoclaw-sandbox:bookworm-slim");
    expect(sandbox.docker.containerPrefix).toBe("haoclaw-sbx-");
