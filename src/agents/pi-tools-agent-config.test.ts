...
describe("Agent-specific tool filtering", () => {
  ...
  async function withApplyPatchEscapeCase(
    ...
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "haoclaw-pi-tools-"));
...
*** End Patch`;
  ...
  function createMainSessionTools(cfg: HaoclawConfig) {
    return createHaoclawCodingTools({
      config: cfg,
      sessionKey: "agent:main:main",
  ...
  function createMainAgentConfig(params: {
    tools: NonNullable<HaoclawConfig["tools"]>;
    agentTools?: NonNullable<NonNullable<HaoclawConfig["agents"]>["list"]>[number]["tools"];
  }): HaoclawConfig {
    return {
      tools: params.tools,
      agents: {
  ...
  it("should allow different tool policies for different agents", () => {
    ...
    const mainTools = createHaoclawCodingTools({
      config: cfg,
      sessionKey: "agent:main:main",
    ...
    const familyTools = createHaoclawCodingTools({
      config: cfg,
  ...
  it("should apply group tool policy overrides (group-specific beats wildcard)", () => {
    ...
    const trustedTools = createHaoclawCodingTools({
      config: cfg,
  ...
  it("should apply per-sender tool policies for group tools", () => {
    ...
    const aliceTools = createHaoclawCodingTools({
      config: cfg,
    ...
    const bobTools = createHaoclawCodingTools({
      config: cfg,
      sessionKey: "agent:main:whatsapp:group:family",
      senderId: "bob",
  ...
  it("should not let default sender policy override group tools", () => {
    ...
    const adminTools = createHaoclawCodingTools({
      config: cfg,
  ...
  it("should apply agent-specific exec host defaults over global defaults", async () => {
    ...
    const helperTools = createHaoclawCodingTools({
      config: cfg,
