...
(() => {
  ...
  const hasIOS = () =>
    !!(
      ...
      window.webkit.messageHandlers.haoclawCanvasA2UIAction
    );
  const hasAndroid = () =>
    !!(
      (window.haoclawCanvasA2UIAction &&
        typeof window.haoclawCanvasA2UIAction.postMessage === "function")
    );
  const hasHelper = () => typeof window.haoclawSendUserAction === "function";
  statusEl.innerHTML =
    "Bridge: " +
  ...
  window.addEventListener("haoclaw:a2ui-action-status", onStatus);

  function send(name, sourceComponentId) {
    if (!hasHelper()) {
      log("No action bridge found. Ensure you're viewing this on an iOS/Android Haoclaw node canvas.");
      return;
    }
    const sendUserAction =
      typeof window.haoclawSendUserAction === "function"
        ? window.haoclawSendUserAction
        : undefined;
...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.HAOCLAW_SKIP_CANVAS_HOST)) {
    return true;
  }
  if (isTruthyEnvValue(process.env.HAOCLAW_SKIP_CANVAS_HOST)) {
    return true;
    ...
    try {
      ...
      if (!opened) {
        if (urlPath === "/" || urlPath.endsWith("/")) {
          ...
          res.end(
            `<!doctype html><meta charset="utf-8" /><title>Haoclaw Canvas</title><pre>Missing file.\nCreate ${rootDir}/index.html</pre>`,
          );
          return true;
        }
        res.statusCode = 404;
