...
describe("canvas host state dir defaults", () => {
  it("uses HAOCLAW_STATE_DIR for the default canvas root", async () => {
    await withStateDirEnv("haoclaw-canvas-state-", async ({ stateDir }) => {
      ...
      try {
        ...
        expect(indexContents).toContain("Haoclaw Canvas");
      } finally {
        await handler.close();
      }
    });
