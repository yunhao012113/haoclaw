...
describe("buildServiceEnvironment", () => {
  it("sets minimal PATH and gateway vars", () => {
    ...
    expect(env.HAOCLAW_GATEWAY_PORT).toBe("18789");
    expect(env.HAOCLAW_GATEWAY_TOKEN).toBeUndefined();
    expect(env.HAOCLAW_SERVICE_MARKER).toBe("haoclaw");
    expect(env.HAOCLAW_SERVICE_KIND).toBe("gateway");
    expect(typeof env.HAOCLAW_SERVICE_VERSION).toBe("string");
    expect(env.HAOCLAW_SYSTEMD_UNIT).toBe("haoclaw-gateway.service");
    expect(env.HAOCLAW_WINDOWS_TASK_NAME).toBe("Haoclaw Gateway");
    if (process.platform === "darwin") {
      expect(env.HAOCLAW_LAUNCHD_LABEL).toBe("ai.haoclaw.gateway");
    }
  });
  ...
  it("uses profile-specific unit and label", () => {
    const env = buildServiceEnvironment({
      env: { HOME: "/home/user", HAOCLAW_PROFILE: "work" },
      port: 18789,
    });
    expect(env.HAOCLAW_SYSTEMD_UNIT).toBe("haoclaw-gateway-work.service");
    expect(env.HAOCLAW_WINDOWS_TASK_NAME).toBe("Haoclaw Gateway (work)");
    if (process.platform === "darwin") {
      expect(env.HAOCLAW_LAUNCHD_LABEL).toBe("ai.haoclaw.work");
    }
  });
...
describe("buildNodeServiceEnvironment", () => {
  ...
  it("passes through HAOCLAW_GATEWAY_TOKEN for node services", () => {
    const env = buildNodeServiceEnvironment({
      env: { HOME: "/home/user", HAOCLAW_GATEWAY_TOKEN: " node-token " },
    });
    expect(env.HAOCLAW_GATEWAY_TOKEN).toBe("node-token");
  });

  it("maps legacy CLAWDBOT_GATEWAY_TOKEN to HAOCLAW_GATEWAY_TOKEN for node services", () => {
    const env = buildNodeServiceEnvironment({
      env: { HOME: "/home/user", CLAWDBOT_GATEWAY_TOKEN: " legacy-token " },
    });
    expect(env.HAOCLAW_GATEWAY_TOKEN).toBe("legacy-token");
  });

  it("prefers HAOCLAW_GATEWAY_TOKEN over legacy CLAWDBOT_GATEWAY_TOKEN", () => {
    const env = buildNodeServiceEnvironment({
      env: {
        HOME: "/home/user",
        HAOCLAW_GATEWAY_TOKEN: "haoclaw-token",
        CLAWDBOT_GATEWAY_TOKEN: "legacy-token",
      },
    });
    expect(env.HAOCLAW_GATEWAY_TOKEN).toBe("haoclaw-token");
  });

  it("omits HAOCLAW_GATEWAY_TOKEN when both token env vars are empty", () => {
    const env = buildNodeServiceEnvironment({
      env: {
        HOME: "/home/user",
...
describe("resolveGatewayStateDir", () => {
  it("uses the default state dir when no overrides are set", () => {
    ...
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".haoclaw"));
  });

  it("appends the profile suffix when set", () => {
    const env = { HOME: "/Users/test", HAOCLAW_PROFILE: "rescue" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".haoclaw-rescue"));
  });

  it("treats default profiles as the base state dir", () => {
    const env = { HOME: "/Users/test", HAOCLAW_PROFILE: "Default" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".haoclaw"));
  });

  it("uses HAOCLAW_STATE_DIR when provided", () => {
    const env = { HOME: "/Users/test", HAOCLAW_STATE_DIR: "/var/lib/haoclaw" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/var/lib/haoclaw"));
  });

  it("expands ~ in HAOCLAW_STATE_DIR", () => {
    const env = { HOME: "/Users/test", HAOCLAW_STATE_DIR: "~/haoclaw-state" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/Users/test/haoclaw-state"));
  });

  it("preserves Windows absolute paths without HOME", () => {
    const env = { HAOCLAW_STATE_DIR: "C:\\State\\haoclaw" };
    expect(resolveGatewayStateDir(env)).toBe("C:\\State\\haoclaw");
  });
});
