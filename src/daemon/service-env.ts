...
export function buildServiceEnvironment(params: {
  ...
  return {
    ...
    HAOCLAW_PROFILE: profile,
    HAOCLAW_GATEWAY_PORT: String(port),
    HAOCLAW_LAUNCHD_LABEL: resolvedLaunchdLabel,
    HAOCLAW_SYSTEMD_UNIT: systemdUnit,
    HAOCLAW_WINDOWS_TASK_NAME: resolveGatewayWindowsTaskName(profile),
    HAOCLAW_SERVICE_MARKER: GATEWAY_SERVICE_MARKER,
    HAOCLAW_SERVICE_KIND: GATEWAY_SERVICE_KIND,
...
export function buildNodeServiceEnvironment(params: {
  ...
  const sharedEnv = resolveSharedServiceEnvironmentFields(env, platform);
  const gatewayToken =
    env.HAOCLAW_GATEWAY_TOKEN?.trim() || env.CLAWDBOT_GATEWAY_TOKEN?.trim() || undefined;
  return {
    ...buildCommonServiceEnvironment(env, sharedEnv),
    HAOCLAW_GATEWAY_TOKEN: gatewayToken,
...
function buildCommonServiceEnvironment(
  ...
  const serviceEnv: Record<string, string | undefined> = {
    ...
    HAOCLAW_STATE_DIR: sharedEnv.stateDir,
    HAOCLAW_CONFIG_PATH: sharedEnv.configPath,
  };
...
function resolveSharedServiceEnvironmentFields(
  ...
  const stateDir = env.HAOCLAW_STATE_DIR;
  const configPath = env.HAOCLAW_CONFIG_PATH;
