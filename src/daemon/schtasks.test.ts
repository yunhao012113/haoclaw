...
describe("resolveTaskScriptPath", () => {
  it.each([
    {
      name: "uses default path when HAOCLAW_PROFILE is unset",
      env: { USERPROFILE: "C:\\Users\\test" },
      expected: path.join("C:\\Users\\test", ".haoclaw", "gateway.cmd"),
    },
    {
      name: "uses profile-specific path when HAOCLAW_PROFILE is set to a custom value",
      env: { USERPROFILE: "C:\\Users\\test", HAOCLAW_PROFILE: "jbphoenix" },
      expected: path.join("C:\\Users\\test", ".haoclaw-jbphoenix", "gateway.cmd"),
    },
    {
      name: "prefers HAOCLAW_STATE_DIR over profile-derived defaults",
      env: {
        USERPROFILE: "C:\\Users\\test",
        HAOCLAW_PROFILE: "rescue",
        HAOCLAW_STATE_DIR: "C:\\State\\haoclaw",
      },
      expected: path.join("C:\\State\\haoclaw", "gateway.cmd"),
    },
    {
      name: "falls back to HOME when USERPROFILE is not set",
      env: { HOME: "/home/test", HAOCLAW_PROFILE: "default" },
      expected: path.join("/home/test", ".haoclaw", "gateway.cmd"),
    },
  ])("$name", ({ env, expected }) => {
...
describe("readScheduledTaskCommand", () => {
  async function withScheduledTaskScript(
    ...
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "haoclaw-schtasks-test-"));
    try {
    ...
    await withScheduledTaskScript(
      {
        scriptLines: [
          "@echo off",
          "rem Haoclaw Gateway",
          "cd /d C:\\Projects\\haoclaw",
          "set NODE_ENV=production",
          "set HAOCLAW_PORT=18789",
      ...
      async (env) => {
        ...
        expect(result).toEqual({
          ...
          workingDirectory: "C:\\Projects\\haoclaw",
          environment: {
            NODE_ENV: "production",
            HAOCLAW_PORT: "18789",
          },
    ...
    await withScheduledTaskScript(
      {
        scriptLines: [
          "@echo off",
          '"C:\\Program Files\\nodejs\\node.exe" C:\\Users\\test\\AppData\\Roaming\\npm\\node_modules\\haoclaw\\dist\\index.js gateway --port 18789',
        ],
      },
      async (env) => {
        const result = await readScheduledTaskCommand(env);
        expect(result).toEqual({
          programArguments: [
            "C:\\Program Files\\nodejs\\node.exe",
            "C:\\Users\\test\\AppData\\Roaming\\npm\\node_modules\\haoclaw\\dist\\index.js",
            "gateway",
            "--port",
            "18789",
          ],
    ...
    await withScheduledTaskScript(
      {
        scriptLines: [
          "@echo off",
          '"\\\\fileserver\\Haoclaw Share\\node.exe" "\\\\fileserver\\Haoclaw Share\\dist\\index.js" gateway --port 18789',
        ],
      },
      async (env) => {
        const result = await readScheduledTaskCommand(env);
        expect(result).toEqual({
          programArguments: [
            "\\\\fileserver\\Haoclaw Share\\node.exe",
            "\\\\fileserver\\Haoclaw Share\\dist\\index.js",
            "gateway",
            "--port",
            "18789",
          ],
  ...
  it("reads script from HAOCLAW_STATE_DIR override", async () => {
    await withScheduledTaskScript(
      {
        env: (tmpDir) => ({ HAOCLAW_STATE_DIR: path.join(tmpDir, "custom-state") }),
