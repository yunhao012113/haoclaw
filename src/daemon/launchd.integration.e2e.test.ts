...
describeLaunchdIntegration("launchd integration", () => {
  ...
  beforeAll(async () => {
    ...
    homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `haoclaw-launchd-int-${testId}-`));
    env = {
      HOME: homeDir,
      HAOCLAW_LAUNCHD_LABEL: `ai.haoclaw.launchd-int-${testId}`,
      HAOCLAW_LOG_PREFIX: `gateway-launchd-int-${testId}`,
    };
  });
