...
function auditGatewayToken(
  ...
  issues.push({
    ...
    message: "Gateway service embeds HAOCLAW_GATEWAY_TOKEN and should be reinstalled.",
    detail: "Run `haoclaw gateway install --force` to remove embedded service token.",
    level: "recommended",
  });
  ...
  issues.push({
    ...
    message:
      "Gateway service HAOCLAW_GATEWAY_TOKEN does not match gateway.auth.token in haoclaw.json",
...
export function readEmbeddedGatewayToken(command: GatewayServiceCommand): string | undefined {
  ...
  if (command.environmentValueSources?.HAOCLAW_GATEWAY_TOKEN === "file") {
    return undefined;
  }
  return command.environment?.HAOCLAW_GATEWAY_TOKEN?.trim() || undefined;
}
...
export function checkTokenDrift(params: {
  ...
  if (configToken && serviceToken !== configToken) {
    return {
      ...
      detail: "Run `haoclaw gateway install --force` to sync the token.",
      level: "recommended",
    };
  }
