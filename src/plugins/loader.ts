...
import { resolveHaoclawPackageRootSync } from "../infra/haoclaw-root.js";
...
import { discoverHaoclawPlugins } from "./discovery.js";
...
function listPluginSdkExportedSubpaths(params: { modulePath?: string } = {}): string[] {
  const modulePath = params.modulePath ?? fileURLToPath(import.meta.url);
  const packageRoot = resolveHaoclawPackageRootSync({
...
const resolvePluginSdkScopedAliasMap = (): Record<string, string> => {
  ...
  for (const subpath of listPluginSdkExportedSubpaths()) {
    ...
    if (resolved) {
      aliasMap[`haoclaw/plugin-sdk/${subpath}`] = resolved;
    }
  }
  return aliasMap;
};
...
function resolvePluginModuleExport(moduleExport: unknown): {
  definition?: HaoclawPluginDefinition;
  register?: HaoclawPluginDefinition["register"];
} {
  const resolved =
    moduleExport &&
  ...
  if (typeof resolved === "function") {
    return {
      register: resolved as HaoclawPluginDefinition["register"],
    };
  }
  if (resolved && typeof resolved === "object") {
    const def = resolved as HaoclawPluginDefinition;
...
export function loadHaoclawPlugins(options: PluginLoadOptions = {}): PluginRegistry {
  ...
  const discovery = discoverHaoclawPlugins({
    workspaceDir: options.workspaceDir,
  ...
  const getJiti = () => {
    ...
    const aliasMap = {
      ...(pluginSdkAlias ? { "haoclaw/plugin-sdk": pluginSdkAlias } : {}),
  ...
  for (const candidate of discovery.candidates) {
    ...
    let mod: HaoclawPluginModule | null = null;
    try {
      mod = getJiti()(safeSource) as HaoclawPluginModule;
    } catch (err) {
      recordPluginError({
        logger,
        registry,
