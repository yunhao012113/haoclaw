...
const MISSING_EXTENSIONS_ERROR =
  'package.json missing haoclaw.extensions; update the plugin package to include haoclaw.extensions (for example ["./dist/index.js"]). See https://docs.haoclaw.ai/help/troubleshooting#plugin-install-fails-with-missing-haoclaw-extensions';

export const PLUGIN_INSTALL_ERROR_CODE = {
  INVALID_NPM_SPEC: "invalid_npm_spec",
  MISSING_HAOCLAW_EXTENSIONS: "missing_haoclaw_extensions",
  EMPTY_HAOCLAW_EXTENSIONS: "empty_haoclaw_extensions",
...
function ensureHaoclawExtensions(params: { manifest: PackageManifest }):
  | {
      ok: true;
      entries: string[];
    }
  ...
  if (resolved.status === "missing") {
    return {
      ...
      code: PLUGIN_INSTALL_ERROR_CODE.MISSING_HAOCLAW_EXTENSIONS,
    };
  }
  if (resolved.status === "empty") {
    return {
      ok: false,
      error: "package.json haoclaw.extensions is empty",
      code: PLUGIN_INSTALL_ERROR_CODE.EMPTY_HAOCLAW_EXTENSIONS,
    };
  }
  return {
    ok: true,
...
async function installPluginFromPackageDir(
  ...
  const extensionsResult = ensureHaoclawExtensions({
    manifest,
  });
  ...
  // Prefer the canonical `id` from haoclaw.plugin.json over the npm package name.
  // This avoids a latent key-mismatch bug: if the manifest id (e.g. "memory-cognee")
  // differs from the npm package name (e.g. "cognee-haoclaw"), the plugin registry
  ...
  try {
    ...
    if (scanSummary.critical > 0) {
      ...
      logger.warn?.(
        `Plugin "${pluginId}" has ${scanSummary.warn} suspicious code pattern(s). Run "haoclaw security audit --deep" for details.`,
      );
    }
  } catch (err) {
    logger.warn?.(
      `Plugin "${pluginId}" code safety scan failed (${String(err)}). Installation continues; run "haoclaw security audit --deep" after install.`,
    );
  }
...
export async function installPluginFromNpmSpec(params: {
  ...
  const flowResult = await installFromNpmSpecArchiveWithInstaller({
    tempDirPrefix: "haoclaw-npm-pack-",
    spec,
    timeoutMs,
