...
const fixtureRoot = fs.mkdtempSync(path.join(os.tmpdir(), "haoclaw-plugin-"));
let tempDirIndex = 0;
const prevBundledDir = process.env.HAOCLAW_BUNDLED_PLUGINS_DIR;
...
function writePlugin(params: {
  ...
  fs.writeFileSync(
    path.join(dir, "haoclaw.plugin.json"),
    JSON.stringify(
      {
        id: params.id,
...
function loadBundledMemoryPluginRegistry(options?: {
  ...
  if (!options && cachedBundledMemoryDir) {
    process.env.HAOCLAW_BUNDLED_PLUGINS_DIR = cachedBundledMemoryDir;
    return loadHaoclawPlugins({
      cache: false,
      ...
      JSON.stringify(
        {
          ...
          haoclaw: { extensions: [`./${pluginFilename}`] },
        },
        null,
        2,
      ),
  ...
  process.env.HAOCLAW_BUNDLED_PLUGINS_DIR = bundledDir;
...
function setupBundledTelegramPlugin() {
  ...
  process.env.HAOCLAW_BUNDLED_PLUGINS_DIR = cachedBundledTelegramDir;
}

function expectTelegramLoaded(registry: ReturnType<typeof loadHaoclawPlugins>) {
...
function useNoBundledPlugins() {
  process.env.HAOCLAW_BUNDLED_PLUGINS_DIR = "/nonexistent/bundled/plugins";
}
...
function loadRegistryFromSinglePlugin(params: {
  ...
  options?: Omit<Parameters<typeof loadHaoclawPlugins>[0], "cache" | "workspaceDir" | "config">;
}) {
...
afterEach(() => {
  if (prevBundledDir === undefined) {
    delete process.env.HAOCLAW_BUNDLED_PLUGINS_DIR;
  } else {
    process.env.HAOCLAW_BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
});
...
describe("loadHaoclawPlugins", () => {
  it("disables bundled plugins by default", () => {
    ...
    const registry = loadHaoclawPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
  ...
  it("re-initializes global hook runner when serving registry from cache", () => {
    ...
    const first = loadHaoclawPlugins(options);
    expect(getGlobalHookRunner()).not.toBeNull();

    resetGlobalHookRunner();
    expect(getGlobalHookRunner()).toBeNull();

    const second = loadHaoclawPlugins(options);
  ...
  it("skips importing bundled memory plugins that are disabled by memory slot", () => {
    ...
    fs.writeFileSync(
      path.join(memoryADir, "haoclaw.plugin.json"),
      JSON.stringify(
        {
          id: "memory-a",
          kind: "memory",
    ...
    fs.writeFileSync(
      path.join(memoryBDir, "haoclaw.plugin.json"),
      JSON.stringify(
        {
          id: "memory-b",
          kind: "memory",
  ...
  it("prefers bundled plugin over auto-discovered global duplicate ids", () => {
    ...
    withEnv({ HAOCLAW_STATE_DIR: stateDir, CLAWDBOT_STATE_DIR: undefined }, () => {
  ...
  it("supports legacy plugins importing monolithic plugin-sdk root", async () => {
    ...
    const plugin = writePlugin({
      ...
      body: `module.exports = {
  id: "legacy-root-import",
  configSchema: (require("haoclaw/plugin-sdk").emptyPluginConfigSchema)(),
  register() {},
};`,
    });
    ...
    const script = `
      import { loadHaoclawPlugins } from ${JSON.stringify(loaderModuleUrl)};
      const registry = loadHaoclawPlugins({
        cache: false,
    ...
    execFileSync(process.execPath, ["--import", "tsx", "--input-type=module", "-e", script], {
      ...
      env: {
        ...process.env,
        HAOCLAW_BUNDLED_PLUGINS_DIR: "/nonexistent/bundled/plugins",
      },
      encoding: "utf-8",
      stdio: "pipe",
    });
