...
function throwGatewayAuthResolutionError(reason: string): never {
  throw new Error(
    [
      reason,
      "Fix: set HAOCLAW_GATEWAY_TOKEN/HAOCLAW_GATEWAY_PASSWORD, pass --token/--password,",
...
export async function resolveGatewayConnection(
  ...
  const envToken = trimToUndefined(env.HAOCLAW_GATEWAY_TOKEN);
  const envPassword = trimToUndefined(env.HAOCLAW_GATEWAY_PASSWORD);
