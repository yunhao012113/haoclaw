...
describe("resolveInitialTuiAgentId", () => {
  const cfg: HaoclawConfig = {
    agents: {
      list: [
        { id: "main", workspace: "/tmp/haoclaw" },
        { id: "ops", workspace: "/tmp/haoclaw/projects/ops" },
      ],
    },
  };
  ...
  it("infers agent from cwd when session is not agent-prefixed", () => {
    expect(
      resolveInitialTuiAgentId({
        ...
        initialSessionInput: "",
        cwd: "/tmp/haoclaw/projects/ops/src",
      }),
    ).toBe("ops");
  });
...
describe("resolveGatewayDisconnectState", () => {
  it("returns pairing recovery guidance when disconnect reason requires pairing", () => {
    ...
    expect(state.activityStatus).toBe("pairing required: run haoclaw devices list");
    expect(state.pairingHint).toContain("haoclaw devices list");
  });
