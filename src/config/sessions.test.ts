...
describe("sessions", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "haoclaw-sessions-suite-"));
  });
  ...
  const withStateDir = <T>(stateDir: string, fn: () => T): T =>
    withEnv({ HAOCLAW_STATE_DIR: stateDir }, fn);
  ...
  it("builds discord display name with guild+channel slugs", () => {
    expect(
      buildGroupDisplayName({
        ...
        space: "friends-of-haoclaw",
        id: "123",
        key: "discord:group:123",
      }),
    ).toBe("discord:friends-of-haoclaw#general");
  });
  ...
  it("derives session transcripts dir from HAOCLAW_STATE_DIR", () => {
    const dir = resolveSessionTranscriptsDir(
      { HAOCLAW_STATE_DIR: "/custom/state" } as NodeJS.ProcessEnv,
      () => "/home/ignored",
    );
  ...
  it("resolves cross-agent paths when HAOCLAW_STATE_DIR differs from stored paths", () => {
