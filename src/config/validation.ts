...
import type { HaoclawConfig, ConfigValidationIssue } from "./types.js";
...
function validateIdentityAvatar(config: HaoclawConfig): ConfigValidationIssue[] {
...
function validateGatewayTailscaleBind(config: HaoclawConfig): ConfigValidationIssue[] {
...
export function validateConfigObjectRaw(
  raw: unknown,
): { ok: true; config: HaoclawConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = HaoclawSchema.safeParse(raw);
  ...
  const duplicates = findDuplicateAgentDirs(validated.data as HaoclawConfig);
  ...
  const avatarIssues = validateIdentityAvatar(validated.data as HaoclawConfig);
  if (avatarIssues.length > 0) {
    return { ok: false, issues: avatarIssues };
  }
  const gatewayTailscaleBindIssues = validateGatewayTailscaleBind(validated.data as HaoclawConfig);
  if (gatewayTailscaleBindIssues.length > 0) {
    return { ok: false, issues: gatewayTailscaleBindIssues };
  }
  return {
    ok: true,
    config: validated.data as HaoclawConfig,
  };
}
