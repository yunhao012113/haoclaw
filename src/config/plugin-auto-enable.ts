...
function isWhatsAppConfigured(cfg: HaoclawConfig): boolean {
...
function isGenericChannelConfigured(cfg: HaoclawConfig, channelId: string): boolean {
  const entry = resolveChannelConfig(cfg, channelId);
  return recordHasKeys(entry);
}
...
function collectModelRefs(cfg: HaoclawConfig): string[] {
  const refs: string[] = [];
...
function isProviderConfigured(cfg: HaoclawConfig, providerId: string): boolean {
...
function collectCandidateChannelIds(cfg: HaoclawConfig): string[] {
...
function isPluginExplicitlyDisabled(cfg: HaoclawConfig, pluginId: string): boolean {
...
function isPluginDenied(cfg: HaoclawConfig, pluginId: string): boolean {
...
function registerPluginEntry(cfg: HaoclawConfig, pluginId: string): HaoclawConfig {
  const builtInChannelId = normalizeChatChannelId(pluginId);
  if (builtInChannelId) {
    const channels = cfg.channels as Record<string, unknown> | undefined;
