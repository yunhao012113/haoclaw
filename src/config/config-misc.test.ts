...
import { buildWebSearchProviderConfig, withTempHome, writeHaoclawConfig } from "./test-helpers.js";
import { HaoclawSchema } from "./zod-schema.js";

describe("$schema key in config (#14998)", () => {
  it("accepts config with $schema string", () => {
    const result = HaoclawSchema.safeParse({
      $schema: "https://haoclaw.ai/config.json",
    });
    expect(result.success).toBe(true);
    if (result.success) {
      expect(result.data.$schema).toBe("https://haoclaw.ai/config.json");
    }
  });
  ...
  it("rejects non-string $schema", () => {
    const result = HaoclawSchema.safeParse({ $schema: 123 });
...
describe("cron webhook schema", () => {
  it("accepts cron.webhookToken and legacy cron.webhook", () => {
    const res = HaoclawSchema.safeParse({
      cron: {
        enabled: true,
...
describe("config strict validation", () => {
  ...
  it("does not mark resolved-only gateway.bind aliases as auto-migratable legacy", async () => {
    await withTempHome(async (home) => {
      await writeHaoclawConfig(home, {
        gateway: { bind: "${HAOCLAW_BIND}" },
      });

      const prev = process.env.HAOCLAW_BIND;
      process.env.HAOCLAW_BIND = "0.0.0.0";
      try {
        const snap = await readConfigFileSnapshot();
        expect(snap.valid).toBe(false);
        ...
        if (prev === undefined) {
          delete process.env.HAOCLAW_BIND;
        } else {
          process.env.HAOCLAW_BIND = prev;
        }
      }
    });
  });
