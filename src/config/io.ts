...
import type { HaoclawConfig, ConfigFileSnapshot, LegacyConfigIssue } from "./types.js";
import {
  validateConfigObjectRawWithPlugins,
  validateConfigObjectWithPlugins,
} from "./validation.js";
import { compareHaoclawVersions } from "./version.js";
...
function formatConfigValidationFailure(pathLabel: string, issueMessage: string): string {
  ...
  return [
    ...
    "",
    "Fix with:",
    `  haoclaw config set ${allowPath} '["*"]'`,
    "",
    "Or switch policy:",
    `  haoclaw config set ${policyPath} "pairing"`,
  ].join("\n");
}
...
function unsetPathForWrite(
  root: HaoclawConfig,
  pathSegments: string[],
): { changed: boolean; next: HaoclawConfig } {
...
function coerceConfig(value: unknown): HaoclawConfig {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return {};
  }
  return value as HaoclawConfig;
}
...
function stampConfigVersion(cfg: HaoclawConfig): HaoclawConfig {
...
function warnIfConfigFromFuture(cfg: HaoclawConfig, logger: Pick<typeof console, "warn">): void {
  const touched = cfg.meta?.lastTouchedVersion;
  if (!touched) {
    return;
  }
  const cmp = compareHaoclawVersions(VERSION, touched);
  if (cmp === null) {
    return;
  }
  if (cmp < 0) {
    logger.warn(
      `Config was last written by a newer Haoclaw (${touched}); current version is ${VERSION}.`,
    );
  }
}
...
function resolveConfigForRead(
  ...
  if (resolvedIncludes && typeof resolvedIncludes === "object" && "env" in resolvedIncludes) {
    applyConfigEnvVars(resolvedIncludes as HaoclawConfig, env);
  }
...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  function loadConfig(): HaoclawConfig {
    try {
      ...
      const preValidationDuplicates = findDuplicateAgentDirs(resolvedConfig as HaoclawConfig, {
        env: deps.env,
  ...
  async function writeConfigFile(cfg: HaoclawConfig, options: ConfigWriteOptions = {}) {
    clearConfigCache();
    ...
    const outputConfigBase =
      envRefMap && changedPaths
        ? (restoreEnvRefsFromMap(cfgToWrite, "", envRefMap, changedPaths) as HaoclawConfig)
        : cfgToWrite;
    ...
    const logConfigOverwrite = () => {
      ...
      const shouldLogInVitest = deps.env.HAOCLAW_TEST_CONFIG_OVERWRITE_LOG === "1";
    ...
    const logConfigWriteAnomalies = () => {
      ...
      const isVitest = deps.env.VITEST === "true";
      const shouldLogInVitest = deps.env.HAOCLAW_TEST_CONFIG_WRITE_ANOMALY_LOG === "1";
      if (isVitest && !shouldLogInVitest) {
        return;
      }
    ...
    const auditRecordBase = {
      ...
      watchMode: deps.env.HAOCLAW_WATCH_MODE === "1",
      watchSession:
        typeof deps.env.HAOCLAW_WATCH_SESSION === "string" &&
        deps.env.HAOCLAW_WATCH_SESSION.trim().length > 0
          ? deps.env.HAOCLAW_WATCH_SESSION.trim()
          : null,
      watchCommand:
        typeof deps.env.HAOCLAW_WATCH_COMMAND === "string" &&
        deps.env.HAOCLAW_WATCH_COMMAND.trim().length > 0
          ? deps.env.HAOCLAW_WATCH_COMMAND.trim()
          : null,
      existsBefore: snapshot.exists,
...
// module scope. `HAOCLAW_CONFIG_PATH` (and friends) are expected to work even
...
let runtimeConfigSnapshot: HaoclawConfig | null = null;
let runtimeConfigSourceSnapshot: HaoclawConfig | null = null;
let runtimeConfigSnapshotRefreshHandler: RuntimeConfigSnapshotRefreshHandler | null = null;

function resolveConfigCacheMs(env: NodeJS.ProcessEnv): number {
  const raw = env.HAOCLAW_CONFIG_CACHE_MS?.trim();
  if (raw === "" || raw === "0") {
    return 0;
  }
  if (!raw) {
...
function shouldUseConfigCache(env: NodeJS.ProcessEnv): boolean {
  if (env.HAOCLAW_DISABLE_CONFIG_CACHE?.trim()) {
    return false;
  }
...
export function getRuntimeConfigSnapshot(): HaoclawConfig | null {
  return runtimeConfigSnapshot;
}

export function getRuntimeConfigSourceSnapshot(): HaoclawConfig | null {
...
export function loadConfig(): HaoclawConfig {
...
export async function readBestEffortConfig(): Promise<HaoclawConfig> {
...
export async function writeConfigFile(
  ...
  // edits to haoclaw.json remain visible (no stale snapshot).
}
