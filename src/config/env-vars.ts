...
function collectConfigEnvVarsByTarget(cfg?: HaoclawConfig): Record<string, string> {
...
export function collectConfigRuntimeEnvVars(cfg?: HaoclawConfig): Record<string, string> {
  return collectConfigEnvVarsByTarget(cfg);
}

export function collectConfigServiceEnvVars(cfg?: HaoclawConfig): Record<string, string> {
  return collectConfigEnvVarsByTarget(cfg);
}

/** @deprecated Use `collectConfigRuntimeEnvVars` or `collectConfigServiceEnvVars`. */
export function collectConfigEnvVars(cfg?: HaoclawConfig): Record<string, string> {
...
export function applyConfigEnvVars(
  ...
  for (const [key, value] of Object.entries(entries)) {
    ...
    // (e.g. process.env.HAOCLAW_GATEWAY_TOKEN = "${VAULT_TOKEN}") which downstream auth
