...
async function writePluginFixture(params: {
  ...
  await fs.writeFile(
    path.join(params.dir, "haoclaw.plugin.json"),
...
describe("config plugin validation", () => {
  ...
  const envSnapshot = {
    ...
    HAOCLAW_PLUGIN_MANIFEST_CACHE_MS: process.env.HAOCLAW_PLUGIN_MANIFEST_CACHE_MS,
  };

  const validateInSuite = (raw: unknown) => validateConfigObjectWithPlugins(raw);

  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "haoclaw-config-plugin-validation-"));
    ...
    process.env.HAOCLAW_STATE_DIR = path.join(suiteHome, ".haoclaw");
    process.env.HAOCLAW_PLUGIN_MANIFEST_CACHE_MS = "10000";
  ...
  afterAll(async () => {
    ...
    clearPluginManifestRegistryCache();
    if (envSnapshot.HAOCLAW_STATE_DIR === undefined) {
      delete process.env.HAOCLAW_STATE_DIR;
    } else {
      process.env.HAOCLAW_STATE_DIR = envSnapshot.HAOCLAW_STATE_DIR;
    }
    if (envSnapshot.HAOCLAW_PLUGIN_MANIFEST_CACHE_MS === undefined) {
      delete process.env.HAOCLAW_PLUGIN_MANIFEST_CACHE_MS;
    } else {
      process.env.HAOCLAW_PLUGIN_MANIFEST_CACHE_MS = envSnapshot.HAOCLAW_PLUGIN_MANIFEST_CACHE_MS;
    }
  });
