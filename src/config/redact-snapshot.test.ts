...
import type { ConfigFileSnapshot } from "./types.haoclaw.js";
import { HaoclawSchema } from "./zod-schema.js";

const { mapSensitivePaths } = __test__;
const mainSchemaHints = mapSensitivePaths(HaoclawSchema, "", {});
...
function makeSnapshot<TConfig extends Record<string, unknown>>(
  ...
  return {
    path: "/home/user/.haoclaw/config.json5",
    exists: true,
...
describe("redactConfigSnapshot", () => {
  ...
  it("does not redact passwordFile path fields", () => {
    const snapshot = makeSnapshot({
      channels: {
        irc: {
          passwordFile: "/etc/haoclaw/irc-password.txt",
          nickserv: {
            passwordFile: "/etc/haoclaw/nickserv-password.txt",
    ...
    expect(irc.passwordFile).toBe("/etc/haoclaw/irc-password.txt");
    expect(nickserv.passwordFile).toBe("/etc/haoclaw/nickserv-password.txt");
...
describe("realredactConfigSnapshot_real", () => {
  it("main schema redact works (samples)", () => {
    const schema = HaoclawSchema.toJSONSchema({
      target: "draft-07",
      unrepresentable: "any",
    });
    schema.title = "HaoclawConfig";
    const hints = mainSchemaHints;
