...
describe("sandbox browser binds config", () => {
  ...
  it("defaults browser network to dedicated sandbox network", () => {
    ...
    expect(resolved.network).toBe("haoclaw-sandbox-browser");
  });

  it("prefers agent browser network over global browser network", () => {
    const resolved = resolveSandboxBrowserConfig({
      scope: "agent",
      globalBrowser: { network: "haoclaw-sandbox-browser-global" },
      agentBrowser: { network: "haoclaw-sandbox-browser-agent" },
    });
    expect(resolved.network).toBe("haoclaw-sandbox-browser-agent");
  });
