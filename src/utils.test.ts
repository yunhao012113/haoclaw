...
describe("ensureDir", () => {
  it("creates nested directory", async () => {
    await withTempDirSync("haoclaw-test-", async (tmp) => {
...
describe("jidToE164", () => {
  ...
  it("maps @lid from authDir mapping files", () => {
    withTempDirSync("haoclaw-auth-", (authDir) => {
  ...
  it("falls back through lidMappingDirs in order", () => {
    withTempDirSync("haoclaw-lid-a-", (first) => {
      withTempDirSync("haoclaw-lid-b-", (second) => {
...
describe("resolveConfigDir", () => {
  it("prefers ~/.haoclaw when legacy dir is missing", async () => {
    const root = await fs.promises.mkdtemp(path.join(os.tmpdir(), "haoclaw-config-dir-"));
    try {
...
describe("resolveHomeDir", () => {
  it("prefers HAOCLAW_HOME over HOME", () => {
    vi.stubEnv("HAOCLAW_HOME", "/srv/haoclaw-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveHomeDir()).toBe(path.resolve("/srv/haoclaw-home"));
...
describe("shortenHomePath", () => {
  it("uses $HAOCLAW_HOME prefix when HAOCLAW_HOME is set", () => {
    vi.stubEnv("HAOCLAW_HOME", "/srv/haoclaw-home");
    vi.stubEnv("HOME", "/home/other");

    expect(shortenHomePath(`${path.resolve("/srv/haoclaw-home")}/.haoclaw/haoclaw.json`)).toBe(
      "$HAOCLAW_HOME/.haoclaw/haoclaw.json",
    );
...
describe("shortenHomeInString", () => {
  it("uses $HAOCLAW_HOME replacement when HAOCLAW_HOME is set", () => {
    vi.stubEnv("HAOCLAW_HOME", "/srv/haoclaw-home");
    vi.stubEnv("HOME", "/home/other");

    expect(
      shortenHomeInString(`config: ${path.resolve("/srv/haoclaw-home")}/.haoclaw/haoclaw.json`),
    ).toBe("config: $HAOCLAW_HOME/.haoclaw/haoclaw.json");
...
describe("resolveUserPath", () => {
  ...
  it("expands ~/ to home dir", () => {
    expect(resolveUserPath("~/haoclaw")).toBe(path.resolve(os.homedir(), "haoclaw"));
  });
  ...
  it("prefers HAOCLAW_HOME for tilde expansion", () => {
    vi.stubEnv("HAOCLAW_HOME", "/srv/haoclaw-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveUserPath("~/haoclaw")).toBe(path.resolve("/srv/haoclaw-home", "haoclaw"));
