...
describe("abort detection", () => {
  ...
  async function createAbortConfig(params?: {
    ...
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "haoclaw-abort-"));
  ...
  it("isAbortRequestText aligns abort command semantics", () => {
    ...
    expect(isAbortRequestText("stop haoclaw!!!")).toBe(true);
    ...
    expect(isAbortRequestText("/stop@haoclaw_bot", { botUsername: "haoclaw_bot" })).toBe(true);
    expect(isAbortRequestText("/Stop@haoclaw_bot", { botUsername: "haoclaw_bot" })).toBe(true);
