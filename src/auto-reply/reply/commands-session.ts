...
import { scheduleGatewaySigusr1Restart, triggerHaoclawRestart } from "../../infra/restart.js";
...
export const handleRestartCommand: CommandHandler = async (params, allowTextCommands) => {
  ...
  if (hasSigusr1Listener) {
    ...
    return {
      shouldContinue: false,
      reply: {
        text: "⚙️ Restarting Haoclaw in-process (SIGUSR1); back in a few seconds.",
      },
    };
  }
  const restartMethod = triggerHaoclawRestart();
  if (!restartMethod.ok) {
  ...
  return {
    shouldContinue: false,
    reply: {
      text: `⚙️ Restarting Haoclaw via ${restartMethod.method}; give me a few seconds to come back online.`,
    },
  };
};
