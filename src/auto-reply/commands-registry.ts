...
export function isCommandEnabled(cfg: HaoclawConfig, commandKey: string): boolean {
...
function resolveDefaultCommandContext(cfg?: HaoclawConfig): {
  provider: string;
  model: string;
} {
  const resolved = resolveConfiguredModelRef({
    cfg: cfg ?? ({} as HaoclawConfig),
...
export function getCommandDetection(_cfg?: HaoclawConfig): CommandDetection {
  const commands = getChatCommands();
...
export function maybeResolveTextAlias(raw: string, cfg?: HaoclawConfig) {
