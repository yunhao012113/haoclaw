...
export function resolveConfigDir(
  ...
  const newDir = path.join(resolveRequiredHomeDir(env, homedir), ".haoclaw");
  try {
...
function resolveHomeDisplayPrefix(): { home: string; prefix: string } | undefined {
  ...
  const explicitHome = process.env.HAOCLAW_HOME?.trim();
  if (explicitHome) {
    return { home, prefix: "$HAOCLAW_HOME" };
  }
  return { home, prefix: "~" };
}
...
// Configuration root; can be overridden via HAOCLAW_STATE_DIR.
