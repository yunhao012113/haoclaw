...
const GATEWAY_TEST_ENV_KEYS = [
  ...
  "HAOCLAW_STATE_DIR",
  "HAOCLAW_CONFIG_PATH",
  "HAOCLAW_SKIP_BROWSER_CONTROL_SERVER",
  "HAOCLAW_SKIP_GMAIL_WATCHER",
  "HAOCLAW_SKIP_CANVAS_HOST",
  "HAOCLAW_BUNDLED_PLUGINS_DIR",
  "HAOCLAW_SKIP_CHANNELS",
  "HAOCLAW_SKIP_PROVIDERS",
  "HAOCLAW_SKIP_CRON",
  "HAOCLAW_TEST_MINIMAL_GATEWAY",
] as const;
...
async function setupGatewayTestHome() {
  ...
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "haoclaw-gateway-home-"));
  process.env.HOME = tempHome;
  process.env.USERPROFILE = tempHome;
  process.env.HAOCLAW_STATE_DIR = path.join(tempHome, ".haoclaw");
  delete process.env.HAOCLAW_CONFIG_PATH;
}

function applyGatewaySkipEnv() {
  process.env.HAOCLAW_SKIP_BROWSER_CONTROL_SERVER = "1";
  process.env.HAOCLAW_SKIP_GMAIL_WATCHER = "1";
  process.env.HAOCLAW_SKIP_CANVAS_HOST = "1";
  process.env.HAOCLAW_SKIP_CHANNELS = "1";
  process.env.HAOCLAW_SKIP_PROVIDERS = "1";
  process.env.HAOCLAW_SKIP_CRON = "1";
  process.env.HAOCLAW_TEST_MINIMAL_GATEWAY = "1";
  process.env.HAOCLAW_BUNDLED_PLUGINS_DIR = tempHome
    ? path.join(tempHome, "haoclaw-test-no-bundled-extensions")
    : "haoclaw-test-no-bundled-extensions";
}
...
async function resetGatewayTestState(options: { uniqueConfigRoot: boolean }) {
  ...
  if (options.uniqueConfigRoot) {
    const suiteRoot = path.join(tempHome, ".haoclaw-test-suite");
    ...
    tempConfigRoot = path.join(tempHome, ".haoclaw-test");
    await fs.rm(tempConfigRoot, { recursive: true, force: true });
    await fs.mkdir(tempConfigRoot, { recursive: true });
  }
...
const CONNECT_CHALLENGE_NONCE_KEY = "__haoclawTestConnectChallengeNonce";
const CONNECT_CHALLENGE_TRACKED_KEY = "__haoclawTestConnectChallengeTracked";
...
export async function startServerWithClient(
  ...
  const envSnapshot = captureEnv(["HAOCLAW_GATEWAY_TOKEN"]);
  const prev = process.env.HAOCLAW_GATEWAY_TOKEN;
  ...
  if (fallbackToken === undefined) {
    ...
    process.env.HAOCLAW_GATEWAY_TOKEN = fallbackToken;
  }
...
function resolveDefaultTestDeviceIdentityPath(params: {
  ...
  const safe =
    `${params.clientId}-${params.clientMode}-${params.platform}-${params.deviceFamily ?? "none"}-${params.role}`
      ...
      .toLowerCase();
  const suiteRoot = process.env.HAOCLAW_STATE_DIR ?? process.env.HOME ?? os.tmpdir();
