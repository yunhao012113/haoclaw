...
const { createHaoclawTools } = await import("../agents/haoclaw-tools.js");
...
type SessionSendTool = ReturnType<typeof createHaoclawTools>[number];
...
function getSessionsSendTool(): SessionSendTool {
  ...
  const tool = createHaoclawTools().find((candidate) => candidate.name === "sessions_send");
  if (!tool) {
...
beforeAll(async () => {
  envSnapshot = captureEnv(["HAOCLAW_GATEWAY_PORT", "HAOCLAW_GATEWAY_TOKEN"]);
  gatewayPort = await getFreePort();
  testState.gatewayAuth = { mode: "token", token: gatewayToken };
  process.env.HAOCLAW_GATEWAY_PORT = String(gatewayPort);
  process.env.HAOCLAW_GATEWAY_TOKEN = gatewayToken;
...
describe("sessions_send label lookup", () => {
  it(
    ...
    async () => {
      ...
      if (!configPath) {
        throw new Error("HAOCLAW_CONFIG_PATH missing in gateway test environment");
      }
