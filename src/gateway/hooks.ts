...
export function resolveHooksConfig(cfg: HaoclawConfig): HooksConfigResolved | null {
...
function resolveKnownAgentIds(cfg: HaoclawConfig, defaultAgentId: string): Set<string> {
...
export function extractHookToken(req: IncomingMessage): string | undefined {
  ...
  const headerToken =
    typeof req.headers["x-haoclaw-token"] === "string"
      ? req.headers["x-haoclaw-token"].trim()
      : "";
  if (headerToken) {
    return headerToken;
  }
