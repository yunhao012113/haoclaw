...
let lastCreateHaoclawToolsContext: Record<string, unknown> | undefined;
...
vi.mock("../agents/haoclaw-tools.js", () => {
  ...
  const tools = [
    ...
    },
    {
      ...
      execute: async () => ({
        ok: true,
        route: {
          agentTo: lastCreateHaoclawToolsContext?.agentTo,
          agentThreadId: lastCreateHaoclawToolsContext?.agentThreadId,
        },
      }),
    },
    {
  ...
  return {
    createHaoclawTools: (ctx: Record<string, unknown>) => {
      lastCreateHaoclawToolsContext = ctx;
      return tools;
    },
  };
});
...
beforeEach(() => {
  ...
  pluginHttpHandlers = [];
  cfg = {};
  lastCreateHaoclawToolsContext = undefined;
});
...
describe("POST /tools/invoke", () => {
  it("invokes a tool and returns {ok:true,result}", async () => {
    ...
    expect(lastCreateHaoclawToolsContext?.allowMediaInvokeCommands).toBe(true);
  });
  ...
  it("propagates message target/thread headers into tools context for sessions_spawn", async () => {
    ...
    const res = await invokeTool({
      ...
      headers: {
        ...gatewayAuthHeaders(),
        "x-haoclaw-message-to": "channel:24514",
        "x-haoclaw-thread-id": "thread-24514",
      },
      tool: "sessions_spawn",
      sessionKey: "main",
    });
