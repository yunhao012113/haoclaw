...
import { createHaoclawTools } from "../agents/haoclaw-tools.js";
import {
...
export async function handleToolsInvokeHttpRequest(
  ...
  const messageChannel = normalizeMessageChannel(
    getHeader(req, "x-haoclaw-message-channel") ?? "",
  );
  const accountId = getHeader(req, "x-haoclaw-account-id")?.trim() || undefined;
  const agentTo = getHeader(req, "x-haoclaw-message-to")?.trim() || undefined;
  const agentThreadId = getHeader(req, "x-haoclaw-thread-id")?.trim() || undefined;
  ...
  const allTools = createHaoclawTools({
