...
function expectSecurityConnectError(
  ...
  expect(error.message).toContain("haoclaw doctor --fix");
...
describe("GatewayClient security checks", () => {
  ...
  it("allows ws:// to private addresses only with HAOCLAW_ALLOW_INSECURE_PRIVATE_WS=1", () => {
  ...
  it("allows ws:// hostnames with HAOCLAW_ALLOW_INSECURE_PRIVATE_WS=1", () => {
    process.env.HAOCLAW_ALLOW_INSECURE_PRIVATE_WS = "1";
    const onConnectError = vi.fn();
    const client = new GatewayClient({
      url: "ws://haoclaw-gateway.ai:18789",
      onConnectError,
    });
