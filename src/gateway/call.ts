...
export function buildGatewayConnectionDetails(
  ...
  const envUrlOverride = cliUrlOverride
    ? undefined
    : (trimToUndefined(process.env.HAOCLAW_GATEWAY_URL) ??
  ...
  const allowPrivateWs = process.env.HAOCLAW_ALLOW_INSECURE_PRIVATE_WS === "1";
  ...
  if (!isSecureWebSocketUrl(url, { allowPrivateWs })) {
    throw new Error(
      [
        ...
        allowPrivateWs
          ? undefined
          : "Break-glass (trusted private networks only): set HAOCLAW_ALLOW_INSECURE_PRIVATE_WS=1",
        "Doctor: haoclaw doctor --fix",
...
function gatewaySecretInputPathCanWin(params: {
  ...
  const sentinel = `__HAOCLAW_GATEWAY_SECRET_REF_PROBE_${params.path.replaceAll(".", "_")}__`;
