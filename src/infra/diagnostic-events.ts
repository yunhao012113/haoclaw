...
function getDiagnosticEventsState(): DiagnosticEventsGlobalState {
  const globalStore = globalThis as typeof globalThis & {
    __haoclawDiagnosticEventsState?: DiagnosticEventsGlobalState;
  };
  if (!globalStore.__haoclawDiagnosticEventsState) {
    globalStore.__haoclawDiagnosticEventsState = {
      seq: 0,
      listeners: new Set<(evt: DiagnosticEventPayload) => void>(),
      dispatchDepth: 0,
    };
  }
  return globalStore.__haoclawDiagnosticEventsState;
}

export function isDiagnosticsEnabled(config?: HaoclawConfig): boolean {
