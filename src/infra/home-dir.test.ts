...
describe("resolveEffectiveHomeDir", () => {
  it("prefers HAOCLAW_HOME over HOME and USERPROFILE", () => {
    const env = {
  ...
  it("expands HAOCLAW_HOME when set to ~", () => {
    const env = {
      HAOCLAW_HOME: "~/svc",
      HOME: "/home/alice",
    } as NodeJS.ProcessEnv;
...
describe("resolveRequiredHomeDir", () => {
  ...
  it("returns a fully resolved path for HAOCLAW_HOME", () => {
    const result = resolveRequiredHomeDir(
      { HAOCLAW_HOME: "/custom/home" } as NodeJS.ProcessEnv,
      () => "/fallback",
    );
    expect(result).toBe(path.resolve("/custom/home"));
  });

  it("returns cwd when HAOCLAW_HOME is tilde-only and no fallback home exists", () => {
    expect(
      resolveRequiredHomeDir({ HAOCLAW_HOME: "~" } as NodeJS.ProcessEnv, () => {
        throw new Error("no home");
      }),
    ).toBe(process.cwd());
  });
...
describe("expandHomePrefix", () => {
  it("expands tilde using effective home", () => {
    const value = expandHomePrefix("~/x", {
      env: { HAOCLAW_HOME: "/srv/haoclaw-home" } as NodeJS.ProcessEnv,
    });
    expect(value).toBe(`${path.resolve("/srv/haoclaw-home")}/x`);
  });
