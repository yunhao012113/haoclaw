 ...
 * returning control to the caller (typically `triggerHaoclawRestart` →
...
/**
 * Parse haoclaw gateway PIDs from lsof -Fpc stdout.
...
function parsePidsFromLsofOutput(stdout: string): number[] {
  ...
  for (const line of stdout.split(/\r?\n/).filter(Boolean)) {
    if (line.startsWith("p")) {
      if (currentPid != null && currentCmd && currentCmd.toLowerCase().includes("haoclaw")) {
 ...
 * Returns only PIDs that belong to haoclaw gateway processes (not the current process).
 */
