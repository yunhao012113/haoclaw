const SUPERVISOR_HINTS = {
  launchd: ["LAUNCH_JOB_LABEL", "LAUNCH_JOB_NAME", "XPC_SERVICE_NAME", "HAOCLAW_LAUNCHD_LABEL"],
  systemd: ["HAOCLAW_SYSTEMD_UNIT", "INVOCATION_ID", "SYSTEMD_EXEC_PID", "JOURNAL_STREAM"],
  schtasks: ["HAOCLAW_WINDOWS_TASK_NAME"],
} as const;
...
export const SUPERVISOR_HINT_ENV_VARS = [
  ...
  "HAOCLAW_SERVICE_MARKER",
  "HAOCLAW_SERVICE_KIND",
] as const;
...
export function detectRespawnSupervisor(
  ...
  if (platform === "win32") {
    ...
    const marker = env.HAOCLAW_SERVICE_MARKER?.trim();
    const serviceKind = env.HAOCLAW_SERVICE_KIND?.trim();
