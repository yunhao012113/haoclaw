...
export async function resolveApnsAuthConfigFromEnv(
  ...
  const teamId = normalizeNonEmptyString(env.HAOCLAW_APNS_TEAM_ID);
  const keyId = normalizeNonEmptyString(env.HAOCLAW_APNS_KEY_ID);
  if (!teamId || !keyId) {
    return {
      ok: false,
      error: "APNs auth missing: set HAOCLAW_APNS_TEAM_ID and HAOCLAW_APNS_KEY_ID",
    };
  }

  const inlineKeyRaw =
    normalizeNonEmptyString(env.HAOCLAW_APNS_PRIVATE_KEY_P8) ??
    normalizeNonEmptyString(env.HAOCLAW_APNS_PRIVATE_KEY);
  if (inlineKeyRaw) {
    return {
      ok: true,
      value: {
  ...
  const keyPath = normalizeNonEmptyString(env.HAOCLAW_APNS_PRIVATE_KEY_PATH);
  if (!keyPath) {
    return {
      ok: false,
      error:
        "APNs private key missing: set HAOCLAW_APNS_PRIVATE_KEY_P8 or HAOCLAW_APNS_PRIVATE_KEY_PATH",
    };
  }
  try {
    ...
    return {
      ok: false,
      error: `failed reading HAOCLAW_APNS_PRIVATE_KEY_PATH (${keyPath}): ${message}`,
    };
  }
}
...
function createHaoclawPushMetadata(params: {
...
export async function sendApnsAlert(params: {
  ...
  const payload = {
    ...
    haoclaw: createHaoclawPushMetadata({
      kind: "push.test",
      nodeId: params.nodeId,
    }),
  };
