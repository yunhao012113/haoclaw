...
import { triggerHaoclawRestart } from "./restart.js";
 ...
 * - HAOCLAW_NO_RESPAWN=1: caller should keep in-process restart behavior (tests/dev)
 * - otherwise: spawn detached child with current argv/execArgv, then caller exits
 */
export function restartGatewayProcessWithFreshPid(): GatewayRespawnResult {
  if (isTruthy(process.env.HAOCLAW_NO_RESPAWN)) {
  ...
  if (supervisor) {
    ...
    // See: https://github.com/haoclaw/haoclaw/issues/39760
    if (supervisor === "schtasks") {
      const restart = triggerHaoclawRestart();
      if (!restart.ok) {
        return {
          mode: "failed",
