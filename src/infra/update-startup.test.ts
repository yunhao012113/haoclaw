...
describe("update-startup", () => {
  ...
  beforeAll(async () => {
    suiteRoot = await fs.mkdtemp(path.join(os.tmpdir(), "haoclaw-update-check-suite-"));
  });
  ...
  beforeEach(async () => {
    ...
    envSnapshot = captureEnv(["HAOCLAW_STATE_DIR", "NODE_ENV", "VITEST"]);
    process.env.HAOCLAW_STATE_DIR = tempDir;
    ...
    if (!loaded) {
      ({ resolveHaoclawPackageRoot } = await import("./haoclaw-root.js"));
    ...
    vi.mocked(resolveHaoclawPackageRoot).mockClear();
  ...
  function mockPackageInstallStatus() {
    vi.mocked(resolveHaoclawPackageRoot).mockResolvedValue("/opt/haoclaw");
  ...
  it("uses current runtime + entrypoint for default auto-update command execution", async () => {
    ...
    process.argv = [process.execPath, "/opt/haoclaw/dist/entry.js"];
    try {
      await runAutoUpdateCheckWithDefaults({
        cfg: createBetaAutoUpdateConfig(),
      });
