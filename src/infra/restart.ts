...
export function triggerHaoclawRestart(): RestartAttempt {
  ...
  if (process.platform === "linux") {
    const unit = normalizeSystemdUnit(
      process.env.HAOCLAW_SYSTEMD_UNIT,
      process.env.HAOCLAW_PROFILE,
    );
  ...
  const label =
    process.env.HAOCLAW_LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env.HAOCLAW_PROFILE);
