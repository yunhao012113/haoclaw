...
const DEFAULT_PACKAGE_NAME = "haoclaw";
...
function normalizeTag(tag?: string) {
  return normalizePackageTagInput(tag, ["haoclaw", DEFAULT_PACKAGE_NAME]) ?? "latest";
}
...
export async function runGatewayUpdate(opts: UpdateRunnerOptions = {}): Promise<UpdateRunResult> {
  ...
  if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
    ...
    if (channel === "dev") {
      ...
      const preflightRoot = await fs.mkdtemp(path.join(os.tmpdir(), "haoclaw-update-preflight-"));
    ...
    const doctorEntry = path.join(gitRoot, "haoclaw.mjs");
    ...
    const doctorStep = await runStep(
      step("haoclaw doctor", doctorArgv, gitRoot, { HAOCLAW_UPDATE_IN_PROGRESS: "1" }),
    );
