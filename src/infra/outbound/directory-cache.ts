...
export class DirectoryCache<T> {
  ...
  private lastConfigRef: HaoclawConfig | null = null;
  ...
  get(key: string, cfg: HaoclawConfig): T | undefined {
  ...
  set(key: string, value: T, cfg: HaoclawConfig): void {
    this.resetIfConfigChanged(cfg);
    const now = Date.now();
  ...
  private resetIfConfigChanged(cfg: HaoclawConfig): void {
