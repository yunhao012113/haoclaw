...
describe("runMessageAction sandboxed media validation", () => {
  ...
  it("allows media paths under preferred Haoclaw tmp root", async () => {
    const tmpRoot = resolvePreferredHaoclawTmpDir();
    ...
    try {
      ...
      const hostTmpOutsideHaoclaw = path.join(os.tmpdir(), "outside-haoclaw", "test-media.png");
      await expect(
        runMessageAction({
          cfg: slackConfig,
          action: "send",
