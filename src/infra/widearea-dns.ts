...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.HAOCLAW_WIDE_AREA_DOMAIN ?? null;
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*haoclaw-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "haoclaw";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "haoclaw");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "haoclaw-gw");
  ...
  records.push(`_haoclaw-gw._tcp IN PTR ${instanceLabel}._haoclaw-gw._tcp`);
  records.push(`${instanceLabel}._haoclaw-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._haoclaw-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; haoclaw-content-hash: ${contentHash}\n${contentBody}`;
}
