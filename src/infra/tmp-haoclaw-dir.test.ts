...
import { POSIX_HAOCLAW_TMP_DIR, resolvePreferredHaoclawTmpDir } from "./tmp-haoclaw-dir.js";

type TmpDirOptions = NonNullable<Parameters<typeof resolvePreferredHaoclawTmpDir>[0]>;

function fallbackTmp(uid = 501) {
  return path.join("/var/fallback", `haoclaw-${uid}`);
}
...
function resolveWithReadOnlyTmpFallback(params: {
  ...
  return resolvePreferredHaoclawTmpDir({
    accessSync: readOnlyTmpAccessSync(),
    lstatSync: vi.fn((target: string) => {
      if (target === POSIX_HAOCLAW_TMP_DIR) {
...
function resolveWithMocks(params: {
  ...
  const resolved = resolvePreferredHaoclawTmpDir({
    accessSync,
    chmodSync,
...
describe("resolvePreferredHaoclawTmpDir", () => {
  it("prefers /tmp/haoclaw when it already exists and is writable", () => {
    ...
    expect(resolved).toBe(POSIX_HAOCLAW_TMP_DIR);
    expect(tmpdir).not.toHaveBeenCalled();
  });

  it("prefers /tmp/haoclaw when it does not exist but /tmp is writable", () => {
    ...
    expect(mkdirSync).toHaveBeenCalledWith(POSIX_HAOCLAW_TMP_DIR, expect.any(Object));
    expect(tmpdir).not.toHaveBeenCalled();
  });

  it("falls back to os.tmpdir()/haoclaw when /tmp/haoclaw is not a directory", () => {
  ...
  it("falls back to os.tmpdir()/haoclaw when /tmp is not writable", () => {
  ...
  it("falls back when /tmp/haoclaw is a symlink", () => {
    expectFallsBackToOsTmpDir({ lstatSync: symlinkTmpDirLstat() });
  });

  it("falls back when /tmp/haoclaw is not owned by the current user", () => {
    expectFallsBackToOsTmpDir({ lstatSync: vi.fn(() => makeDirStat({ uid: 0 })) });
  });

  it("falls back when /tmp/haoclaw is group/other writable", () => {
  ...
  it("throws when fallback path is a symlink", () => {
    ...
    ).toThrow(/Unsafe fallback Haoclaw temp dir/);
  });
