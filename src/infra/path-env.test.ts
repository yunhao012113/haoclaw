...
let ensureHaoclawCliOnPath: typeof import("./path-env.js").ensureHaoclawCliOnPath;

describe("ensureHaoclawCliOnPath", () => {
  const envKeys = [
    "PATH",
    "HAOCLAW_PATH_BOOTSTRAPPED",
    "HAOCLAW_ALLOW_PROJECT_LOCAL_BIN",
    "MISE_DATA_DIR",
    "HOMEBREW_PREFIX",
    "HOMEBREW_BREW_FILE",
    "XDG_BIN_HOME",
  ...
  beforeAll(async () => {
    ({ ensureHaoclawCliOnPath } = await import("./path-env.js"));
  });
  ...
  it("prepends the bundled app bin dir when a sibling haoclaw exists", () => {
    const tmp = abs("/tmp/haoclaw-path/case-bundled");
    const appBinDir = path.join(tmp, "AppBin");
    const cliPath = path.join(appBinDir, "haoclaw");
    setDir(tmp);
    setDir(appBinDir);
    setExe(cliPath);

    process.env.PATH = "/usr/bin";
    delete process.env.HAOCLAW_PATH_BOOTSTRAPPED;

    ensureHaoclawCliOnPath({
      execPath: cliPath,
      cwd: tmp,
      homeDir: tmp,
      platform: "darwin",
  ...
  it("is idempotent", () => {
    ...
    process.env.HAOCLAW_PATH_BOOTSTRAPPED = "1";
    ensureHaoclawCliOnPath({
  ...
  it("prepends mise shims when available", () => {
    const tmp = abs("/tmp/haoclaw-path/case-mise");
    const appBinDir = path.join(tmp, "AppBin");
    const appCli = path.join(appBinDir, "haoclaw");
    setDir(tmp);
    setDir(appBinDir);
    setExe(appCli);
  ...
  it("only appends project-local node_modules/.bin when explicitly enabled", () => {
    const tmp = abs("/tmp/haoclaw-path/case-project-local");
    const appBinDir = path.join(tmp, "AppBin");
    const appCli = path.join(appBinDir, "haoclaw");
    setDir(tmp);
    setDir(appBinDir);
    ...
    const localCli = path.join(localBinDir, "haoclaw");
  ...
  it("prepends Linuxbrew dirs when present", () => {
    const tmp = abs("/tmp/haoclaw-path/case-linuxbrew");
