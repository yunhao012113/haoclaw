...
function candidateBinDirs(opts: EnsureHaoclawPathOpts): { prepend: string[]; append: string[] } {
  ...
  // Bundled macOS app: `haoclaw` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingCli = path.join(execDir, "haoclaw");
  ...
  const allowProjectLocalBin =
    ...
    isTruthyEnvValue(process.env.HAOCLAW_ALLOW_PROJECT_LOCAL_BIN);
  if (allowProjectLocalBin) {
    const localBinDir = path.join(cwd, "node_modules", ".bin");
    if (isExecutable(path.join(localBinDir, "haoclaw"))) {
...
/**
 * Best-effort PATH bootstrap so skills that require the `haoclaw` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureHaoclawCliOnPath(opts: EnsureHaoclawPathOpts = {}) {
  if (isTruthyEnvValue(process.env.HAOCLAW_PATH_BOOTSTRAPPED)) {
    return;
  }
