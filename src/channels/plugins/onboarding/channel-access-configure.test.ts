...
async function runConfigureChannelAccess<TResolved>(params: {
  ...
  label?: string;
  placeholder?: string;
  setPolicy: (cfg: HaoclawConfig, policy: ChannelAccessPolicy) => HaoclawConfig;
  resolveAllowlist: (params: { cfg: HaoclawConfig; entries: string[] }) => Promise<TResolved>;
  applyAllowlist: (params: { cfg: HaoclawConfig; resolved: TResolved }) => HaoclawConfig;
}) {
...
describe("configureChannelAccessWithAllowlist", () => {
  it("returns input config when user skips access configuration", async () => {
    ...
    const setPolicy = vi.fn((next: HaoclawConfig) => next);
    const resolveAllowlist = vi.fn(async () => [] as string[]);
    const applyAllowlist = vi.fn((params: { cfg: HaoclawConfig }) => params.cfg);
  ...
  it("applies non-allowlist policy directly", async () => {
    ...
    const setPolicy = vi.fn(
      (next: HaoclawConfig, policy: ChannelAccessPolicy): HaoclawConfig => ({
        ...next,
  ...
  it("resolves allowlist entries and applies them after forcing allowlist policy", async () => {
    ...
    const calls: string[] = [];
    const setPolicy = vi.fn((next: HaoclawConfig, policy: ChannelAccessPolicy): HaoclawConfig => {
    ...
    const resolveAllowlist = vi.fn(async (params: { cfg: HaoclawConfig; entries: string[] }) => {
      calls.push("resolve");
      expect(params.cfg).toBe(cfg);
      expect(params.entries).toEqual(["#general", "#support"]);
      return ["C1", "C2"];
    });
    const applyAllowlist = vi.fn((params: { cfg: HaoclawConfig; resolved: string[] }) => {
      calls.push("apply");
