...
async function createAuditFixture(): Promise<AuditFixture> {
  const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "haoclaw-secrets-audit-"));
  const stateDir = path.join(rootDir, ".haoclaw");
...
describe("secrets audit", () => {
  ...
  it("does not flag non-sensitive routing headers in haoclaw config", async () => {
    await writeJsonFile(fixture.configPath, {
      models: {
        providers: {
          openai: {
