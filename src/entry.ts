...
const ENTRY_WRAPPER_PAIRS = [
  { wrapperBasename: "haoclaw.mjs", entryBasename: "entry.js" },
  { wrapperBasename: "haoclaw.js", entryBasename: "entry.js" },
] as const;
...
if (
  ...
  if (shouldForceReadOnlyAuthStore(process.argv)) {
    process.env.HAOCLAW_AUTH_STORE_READONLY = "1";
  }
  ...
  function ensureExperimentalWarningSuppressed(): boolean {
    ...
    if (isTruthyEnvValue(process.env.HAOCLAW_NO_RESPAWN)) {
      return false;
    }
    if (isTruthyEnvValue(process.env.HAOCLAW_NODE_OPTIONS_READY)) {
      return false;
    }
    ...
    process.env.HAOCLAW_NODE_OPTIONS_READY = "1";
  ...
  function tryHandleRootVersionFastPath(argv: string[]): boolean {
    ...
    Promise.all([import("./version.js"), import("./infra/git-commit.js")])
      .then(([{ VERSION }, { resolveCommitHash }]) => {
        ...
        console.log(commit ? `Haoclaw ${VERSION} (${commit})` : `Haoclaw ${VERSION}`);
        process.exit(0);
      })
      .catch((error) => {
        console.error(
          "[haoclaw] Failed to resolve version:",
          error instanceof Error ? (error.stack ?? error.message) : error,
        );
        process.exitCode = 1;
      });
  ...
  function tryHandleRootHelpFastPath(argv: string[]): boolean {
    ...
    import("./cli/program.js")
      ...
      .catch((error) => {
        console.error(
          "[haoclaw] Failed to display help:",
          error instanceof Error ? (error.stack ?? error.message) : error,
        );
        process.exitCode = 1;
      });
  ...
  if (!ensureExperimentalWarningSuppressed()) {
    ...
    if (!parsed.ok) {
      ...
      console.error(`[haoclaw] ${parsed.error}`);
      process.exit(2);
    }
