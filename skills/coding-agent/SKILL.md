...
metadata:
  {
    "haoclaw": { "emoji": "🧩", "requires": { "anyBins": ["claude", "codex", "opencode", "pi"] } },
  }
---
...
**⚠️ CRITICAL: Never review PRs in Haoclaw's own project folder!**
...
8. **NEVER start Codex in ~/.haoclaw/** - it'll read your soul docs and get weird ideas about the org chart!
9. **NEVER checkout branches in ~/Projects/haoclaw/** - that's the LIVE Haoclaw instance!
...
For long-running background tasks, append a wake trigger to your prompt so Haoclaw gets notified immediately when the agent finishes (instead of waiting for the next heartbeat):

```
... your task here.

When completely finished, run this command to notify me:
haoclaw system event --text "Done: [brief summary of what was built]" --mode now
```
...
When completely finished, run: haoclaw system event --text \"Done: Built todos REST API with CRUD endpoints\" --mode now'"
```
...
- **Sass works:** Codex responds well to playful prompts. Asked it to write a haiku about being second fiddle to a space octopus, got: _"Second chair, I code / Space octopus sets the tempo / Keys glow, I follow"_ 🐙
