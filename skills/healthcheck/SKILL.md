---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for Haoclaw deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, Haoclaw cron scheduling for periodic checks, or version status checks on a machine running Haoclaw (laptop, workstation, Pi, VPS).
---

# Haoclaw Host Hardening

## Overview

Assess and harden the host running Haoclaw, then align it to a user-defined risk tolerance without breaking access. Use Haoclaw security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim Haoclaw changes the host firewall, SSH, or OS updates; it does not.
...
5. Haoclaw gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if Haoclaw can access sensitive data.
...
### 2) Run Haoclaw security audits (read-only)

As part of the default read-only checks, run `haoclaw security audit --deep`. Only offer alternatives if the user requests them:

1. `haoclaw security audit` (faster, non-probing)
2. `haoclaw security audit --json` (structured output)

Offer to apply Haoclaw safe defaults (numbered):

1. `haoclaw security audit --fix`

Be explicit that `--fix` only tightens Haoclaw defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check Haoclaw version/update status (read-only)

As part of the default read-only checks, run `haoclaw update status`.
...
- Credential hygiene notes (location of Haoclaw creds, prefer disk encryption)
...
After Haoclaw install or first hardening pass, run at least one baseline audit and version check:

- `haoclaw security audit`
- `haoclaw security audit --deep`
- `haoclaw update status`

Ongoing monitoring is recommended. Use the Haoclaw cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `haoclaw cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `haoclaw update status`
...
Before creating, `haoclaw cron list` and match on exact `name`. If found, `haoclaw cron edit <id> ...`.
If not found, `haoclaw cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `haoclaw update status` (preferred for source checkouts and channels)
2. `npm view haoclaw version` (published npm version)

## Haoclaw command accuracy

Use only supported commands and flags:

- `haoclaw security audit [--deep] [--fix] [--json]`
- `haoclaw status` / `haoclaw status --deep`
- `haoclaw health --json`
- `haoclaw update status`
- `haoclaw cron add|list|runs|run`

Do not invent CLI flags or imply Haoclaw enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by Haoclaw compaction:
