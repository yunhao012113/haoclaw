...
describe("control UI routing", () => {
  ...
  it("infers nested base paths", async () => {
    const app = mountApp("/apps/haoclaw/cron");
    await app.updateComplete;

    expect(app.basePath).toBe("/apps/haoclaw");
    expect(app.tab).toBe("cron");
    expect(window.location.pathname).toBe("/apps/haoclaw/cron");
  });

  it("honors explicit base path overrides", async () => {
    window.__HAOCLAW_CONTROL_UI_BASE_PATH__ = "/haoclaw";
    const app = mountApp("/haoclaw/sessions");
    await app.updateComplete;

    expect(app.basePath).toBe("/haoclaw");
    expect(app.tab).toBe("sessions");
    expect(window.location.pathname).toBe("/haoclaw/sessions");
  });
  ...
  it("hydrates token from URL params and strips it", async () => {
    ...
    expect(JSON.parse(localStorage.getItem("haoclaw.control.settings.v1") ?? "{}").token).toBe(
      undefined,
    );
  ...
  it("hydrates token from URL params even when settings already set", async () => {
    localStorage.setItem(
      "haoclaw.control.settings.v1",
      JSON.stringify({ token: "existing-token", gatewayUrl: "wss://gateway.example/haoclaw" }),
    );
    const app = mountApp("/ui/overview?token=abc123");
    await app.updateComplete;

    expect(app.settings.token).toBe("abc123");
    expect(JSON.parse(localStorage.getItem("haoclaw.control.settings.v1") ?? "{}")).toMatchObject({
      gatewayUrl: "wss://gateway.example/haoclaw",
    });
    expect(JSON.parse(localStorage.getItem("haoclaw.control.settings.v1") ?? "{}").token).toBe(
      undefined,
    );
