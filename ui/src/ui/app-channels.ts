...
export async function handleWhatsAppStart(host: HaoclawApp, force: boolean) {
  await startWhatsAppLogin(host, force);
  await loadChannels(host, true);
}

export async function handleWhatsAppWait(host: HaoclawApp) {
  await waitWhatsAppLogin(host);
  await loadChannels(host, true);
}

export async function handleWhatsAppLogout(host: HaoclawApp) {
  await logoutWhatsApp(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigSave(host: HaoclawApp) {
  await saveConfig(host);
  await loadConfig(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigReload(host: HaoclawApp) {
  await loadConfig(host);
  await loadChannels(host, true);
}
...
function resolveNostrAccountId(host: HaoclawApp): string {
...
function resolveGatewayHttpAuthHeader(host: HaoclawApp): string | null {
...
function buildGatewayHttpHeaders(host: HaoclawApp): Record<string, string> {
...
export function handleNostrProfileCancel(host: HaoclawApp) {
...
export function handleNostrProfileToggleAdvanced(host: HaoclawApp) {
  const state = host.nostrProfileFormState;
  if (!state) {
    return;
  }
...
export async function handleNostrProfileSave(host: HaoclawApp) {
  const state = host.nostrProfileFormState;
...
export async function handleNostrProfileImport(host: HaoclawApp) {
  const state = host.nostrProfileFormState;
