...
# Haoclaw Installer for macOS and Linux
# Usage: curl -fsSL --proto '=https' --tlsv1.2 https://haoclaw.ai/install.sh | bash
...
DEFAULT_TAGLINE="All your chats, one Haoclaw."
NODE_MIN_MAJOR=22
NODE_MIN_MINOR=12
...
GUM_VERSION="${HAOCLAW_GUM_VERSION:-0.17.0}"
GUM=""
GUM_STATUS="skipped"
GUM_REASON=""
...
print_installer_banner() {
    if [[ -n "$GUM" ]]; then
        ...
        title="$("$GUM" style --foreground "#ff4d4d" --bold "🐙 Haoclaw Installer")"
...
detect_os_or_die() {
    ...
    if [[ "$OS" == "unknown" ]]; then
        ...
        echo "For Windows, use: iwr -useb https://haoclaw.ai/install.ps1 | iex"
        exit 1
    fi
...
show_install_plan() {
    ...
    ui_kv "Requested version" "$HAOCLAW_VERSION"
    if [[ "$USE_BETA" == "1" ]]; then
...
show_footer_links() {
    local faq_url="https://docs.haoclaw.ai/start/faq"
    if [[ -n "$GUM" ]]; then
        local content
...
cleanup_npm_haoclaw_paths() {
    local npm_root=""
    npm_root="$(npm root -g 2>/dev/null || true)"
    if [[ -z "$npm_root" || "$npm_root" != *node_modules* ]]; then
        return 1
    fi
    rm -rf "$npm_root"/.haoclaw-* "$npm_root"/haoclaw 2>/dev/null || true
}

extract_haoclaw_conflict_path() {
    local log="$1"
    local path=""
...
cleanup_haoclaw_bin_conflict() {
    ...
    if [[ -n "$npm_bin" && "$bin_path" != "$npm_bin/haoclaw" ]]; then
        case "$bin_path" in
            "/opt/homebrew/bin/haoclaw"|"/usr/local/bin/haoclaw")
                ;;
            *)
                return 1
                ;;
    ...
    if [[ -L "$bin_path" ]]; then
        ...
        if [[ "$target" == *"/node_modules/haoclaw/"* ]]; then
            rm -f "$bin_path"
            ui_info "Removed stale haoclaw symlink at ${bin_path}"
            return 0
        fi
        return 1
    fi
    ...
    if mv "$bin_path" "$backup"; then
        ui_info "Moved existing haoclaw binary to ${backup}"
        return 0
    fi
    return 1
}
...
run_npm_global_install() {
    ...
    if [[ -n "$GUM" ]] && gum_is_tty; then
        ...
        printf -v log_quoted '%q' "$log"
        run_with_spinner "Installing Haoclaw package" bash -c "${cmd_quoted}>${log_quoted} 2>&1"
        return $?
    fi
...
install_haoclaw_npm() {
    ...
    if ! run_npm_global_install "$spec" "$log"; then
        ...
        if auto_install_build_tools_for_npm_failure "$log"; then
            ...
            if run_npm_global_install "$spec" "$log"; then
                ui_success "Haoclaw npm package installed"
                return 0
            fi
        fi
        ...
        if grep -q "ENOTEMPTY: directory not empty, rename .*haoclaw" "$log"; then
        ...
        if grep -q "EEXIST" "$log"; then
            local conflict=""
            conflict="$(extract_haoclaw_conflict_path "$log" || true)"
            if [[ -n "$conflict" ]] && cleanup_haoclaw_bin_conflict "$conflict"; then
                if run_npm_global_install "$spec" "$log"; then
                    ui_success "Haoclaw npm package installed"
                    return 0
                fi
            ...
            ui_error "npm failed because an haoclaw binary already exists"
            if [[ -n "$conflict" ]]; then
...
TAGLINES+=("I'll butter your workflow like a octopus roll: messy, delicious, effective.")
...
map_legacy_env "HAOCLAW_TAGLINE_INDEX" "CLAWDBOT_TAGLINE_INDEX"
map_legacy_env "HAOCLAW_NO_ONBOARD" "CLAWDBOT_NO_ONBOARD"
map_legacy_env "HAOCLAW_NO_PROMPT" "CLAWDBOT_NO_PROMPT"
map_legacy_env "HAOCLAW_DRY_RUN" "CLAWDBOT_DRY_RUN"
map_legacy_env "HAOCLAW_INSTALL_METHOD" "CLAWDBOT_INSTALL_METHOD"
map_legacy_env "HAOCLAW_VERSION" "CLAWDBOT_VERSION"
map_legacy_env "HAOCLAW_BETA" "CLAWDBOT_BETA"
map_legacy_env "HAOCLAW_GIT_DIR" "CLAWDBOT_GIT_DIR"
map_legacy_env "HAOCLAW_GIT_UPDATE" "CLAWDBOT_GIT_UPDATE"
map_legacy_env "HAOCLAW_NPM_LOGLEVEL" "CLAWDBOT_NPM_LOGLEVEL"
map_legacy_env "HAOCLAW_VERBOSE" "CLAWDBOT_VERBOSE"
map_legacy_env "HAOCLAW_PROFILE" "CLAWDBOT_PROFILE"
map_legacy_env "HAOCLAW_INSTALL_SH_NO_RUN" "CLAWDBOT_INSTALL_SH_NO_RUN"
...
pick_tagline() {
    ...
    if [[ -n "${HAOCLAW_TAGLINE_INDEX:-}" ]]; then
        if [[ "${HAOCLAW_TAGLINE_INDEX}" =~ ^[0-9]+$ ]]; then
            local idx=$((HAOCLAW_TAGLINE_INDEX % count))
            echo "${TAGLINES[$idx]}"
            return
        fi
    fi
...
NO_ONBOARD=${HAOCLAW_NO_ONBOARD:-0}
NO_PROMPT=${HAOCLAW_NO_PROMPT:-0}
DRY_RUN=${HAOCLAW_DRY_RUN:-0}
INSTALL_METHOD=${HAOCLAW_INSTALL_METHOD:-}
HAOCLAW_VERSION=${HAOCLAW_VERSION:-latest}
USE_BETA=${HAOCLAW_BETA:-0}
GIT_DIR_DEFAULT="${HOME}/haoclaw"
GIT_DIR=${HAOCLAW_GIT_DIR:-$GIT_DIR_DEFAULT}
GIT_UPDATE=${HAOCLAW_GIT_UPDATE:-1}
SHARP_IGNORE_GLOBAL_LIBVIPS="${SHARP_IGNORE_GLOBAL_LIBVIPS:-1}"
NPM_LOGLEVEL="${HAOCLAW_NPM_LOGLEVEL:-error}"
NPM_SILENT_FLAG="--silent"
VERBOSE="${HAOCLAW_VERBOSE:-0}"
HAOCLAW_BIN=""
PNPM_CMD=()
HELP=0
...
print_usage() {
    cat <<EOF
Haoclaw installer (macOS + Linux)

Usage:
  curl -fsSL --proto '=https' --tlsv1.2 https://haoclaw.ai/install.sh | bash -s -- [options]
...
Options:
  ...
  --git-dir, --dir <path>             Checkout directory (default: ~/haoclaw)
...
Environment variables:
  HAOCLAW_INSTALL_METHOD=git|npm
  HAOCLAW_VERSION=latest|next|<semver>
  HAOCLAW_BETA=0|1
  HAOCLAW_GIT_DIR=...
  HAOCLAW_GIT_UPDATE=0|1
  HAOCLAW_NO_PROMPT=1
  HAOCLAW_DRY_RUN=1
  HAOCLAW_NO_ONBOARD=1
  HAOCLAW_VERBOSE=1
  HAOCLAW_NPM_LOGLEVEL=error|warn|notice  Default: error (hide npm deprecation noise)
...
Examples:
  ...
  curl -fsSL --proto '=https' --tlsv1.2 https://haoclaw.ai/install.sh | bash -s -- --install-method git --no-onboard
EOF
}
...
choose_install_method_interactive() {
    ...
    if [[ -n "$GUM" ]] && gum_is_tty; then
        ...
        header="Detected Haoclaw checkout in: ${detected_checkout}
...
${WARN}→${NC} Detected a Haoclaw source checkout in: ${INFO}${detected_checkout}${NC}
Choose install method:
...
detect_haoclaw_checkout() {
    ...
    if ! grep -q '"name"[[:space:]]*:[[:space:]]*"haoclaw"' "$dir/package.json" 2>/dev/null; then
        return 1
    fi
    echo "$dir"
    return 0
...
print_homebrew_admin_fix() {
    ...
    echo "Then retry:"
    echo "  curl -fsSL https://haoclaw.ai/install.sh | bash"
}
...
ensure_haoclaw_bin_link() {
    local npm_root=""
    npm_root="$(npm root -g 2>/dev/null || true)"
    if [[ -z "$npm_root" || ! -d "$npm_root/haoclaw" ]]; then
        return 1
    fi
    local npm_bin=""
    ...
    mkdir -p "$npm_bin"
    if [[ ! -x "${npm_bin}/haoclaw" ]]; then
        ln -sf "$npm_root/haoclaw/dist/entry.js" "${npm_bin}/haoclaw"
        ui_info "Created haoclaw bin link at ${npm_bin}/haoclaw"
    fi
    return 0
}

# Check for existing Haoclaw installation
check_existing_haoclaw() {
    if [[ -n "$(type -P haoclaw 2>/dev/null || true)" ]]; then
        ui_info "Existing Haoclaw installation detected, upgrading"
        return 0
    fi
    return 1
}
...
warn_shell_path_missing_dir() {
    ...
    echo "  This can make haoclaw show as \"command not found\" in new terminals."
...
warn_haoclaw_not_found() {
    ui_warn "Installed, but haoclaw is not discoverable on PATH in this shell"
    echo "  Try: hash -r (bash) or rehash (zsh), then retry."
    local t=""
    t="$(type -t haoclaw 2>/dev/null || true)"
    if [[ "$t" == "alias" || "$t" == "function" ]]; then
        ui_warn "Found a shell ${t} named haoclaw; it may shadow the real binary"
    fi
    if command -v nodenv &> /dev/null; then
...
resolve_haoclaw_bin() {
    refresh_shell_command_cache
    local resolved=""
    resolved="$(type -P haoclaw 2>/dev/null || true)"
    ...
    local npm_bin=""
    npm_bin="$(npm_global_bin_dir || true)"
    if [[ -n "$npm_bin" && -x "${npm_bin}/haoclaw" ]]; then
        echo "${npm_bin}/haoclaw"
        return 0
    fi
...
install_haoclaw_from_git() {
    local repo_dir="$1"
    local repo_url="https://github.com/haoclaw/haoclaw.git"

    if [[ -d "$repo_dir/.git" ]]; then
        ui_info "Installing Haoclaw from git checkout: ${repo_dir}"
    else
        ui_info "Installing Haoclaw from GitHub (${repo_url})"
    fi
    ...
    if [[ ! -d "$repo_dir" ]]; then
        run_quiet_step "Cloning Haoclaw" git clone "$repo_url" "$repo_dir"
    fi
    ...
    run_quiet_step "Building Haoclaw" run_pnpm -C "$repo_dir" build

    ensure_user_local_bin_on_path

    cat > "$HOME/.local/bin/haoclaw" <<EOF
#!/usr/bin/env bash
set -euo pipefail
exec node "${repo_dir}/dist/entry.js" "\$@"
EOF
    chmod +x "$HOME/.local/bin/haoclaw"
    ui_success "Haoclaw wrapper installed to \$HOME/.local/bin/haoclaw"
...
resolve_beta_version() {
    local beta=""
    beta="$(npm view haoclaw dist-tags.beta 2>/dev/null || true)"
...
install_haoclaw() {
    local package_name="haoclaw"
    if [[ "$USE_BETA" == "1" ]]; then
        local beta_version=""
        beta_version="$(resolve_beta_version || true)"
        if [[ -n "$beta_version" ]]; then
            HAOCLAW_VERSION="$beta_version"
            ui_info "Beta tag detected (${beta_version})"
            package_name="haoclaw"
        else
            HAOCLAW_VERSION="latest"
            ui_info "No beta tag found; using latest"
        fi
    fi

    if [[ -z "${HAOCLAW_VERSION}" ]]; then
        HAOCLAW_VERSION="latest"
    fi

    local resolved_version=""
    resolved_version="$(npm view "${package_name}@${HAOCLAW_VERSION}" version 2>/dev/null || true)"
    if [[ -n "$resolved_version" ]]; then
        ui_info "Installing Haoclaw v${resolved_version}"
    else
        ui_info "Installing Haoclaw (${HAOCLAW_VERSION})"
    fi
    local install_spec=""
    if [[ "${HAOCLAW_VERSION}" == "latest" ]]; then
        install_spec="${package_name}@latest"
    else
        install_spec="${package_name}@${HAOCLAW_VERSION}"
    fi

    if ! install_haoclaw_npm "${install_spec}"; then
        ui_warn "npm install failed; retrying"
        cleanup_npm_haoclaw_paths
        install_haoclaw_npm "${install_spec}"
    fi

    if [[ "${HAOCLAW_VERSION}" == "latest" && "${package_name}" == "haoclaw" ]]; then
        if ! resolve_haoclaw_bin &> /dev/null; then
            ui_warn "npm install haoclaw@latest failed; retrying haoclaw@next"
            cleanup_npm_haoclaw_paths
            install_haoclaw_npm "haoclaw@next"
        fi
    fi

    ensure_haoclaw_bin_link || true

    ui_success "Haoclaw installed"
}

# Run doctor for migrations (safe, non-interactive)
run_doctor() {
    ui_info "Running doctor to migrate settings"
    local claw="${HAOCLAW_BIN:-}"
    if [[ -z "$claw" ]]; then
        claw="$(resolve_haoclaw_bin || true)"
    fi
    if [[ -z "$claw" ]]; then
        ui_info "Skipping doctor (haoclaw not on PATH yet)"
        warn_haoclaw_not_found
        return 0
    fi
...
resolve_workspace_dir() {
    local profile="${HAOCLAW_PROFILE:-default}"
    if [[ "${profile}" != "default" ]]; then
        echo "${HOME}/.haoclaw/workspace-${profile}"
    else
        echo "${HOME}/.haoclaw/workspace"
    fi
}
...
run_bootstrap_onboarding_if_needed() {
    ...
    local config_path="${HAOCLAW_CONFIG_PATH:-$HOME/.haoclaw/haoclaw.json}"
    ...
    if [[ ! -r /dev/tty || ! -w /dev/tty ]]; then
        ui_info "BOOTSTRAP.md found but no TTY; run haoclaw onboard to finish setup"
        return
    fi
    ...
    if [[ -z "$claw" ]]; then
        ui_info "BOOTSTRAP.md found but haoclaw not on PATH; skipping onboarding"
        warn_haoclaw_not_found
        return
    fi

    "$claw" onboard || {
        ui_error "Onboarding failed; run haoclaw onboard to retry"
        return
    }
}
...
if ! declare -F extract_haoclaw_semver >/dev/null 2>&1; then
...
resolve_haoclaw_version() {
    ...
    local claw="${HAOCLAW_BIN:-}"
    if [[ -z "$claw" ]] && command -v haoclaw &> /dev/null; then
        claw="$(command -v haoclaw)"
    fi
    if [[ -n "$claw" ]]; then
        raw_version_output=$("$claw" --version 2>/dev/null | head -n 1 | tr -d '\r')
        version="$(extract_haoclaw_semver "$raw_version_output")"
        if [[ -z "$version" ]]; then
    ...
    if [[ -z "$version" ]]; then
        local npm_root=""
        npm_root=$(npm root -g 2>/dev/null || true)
        if [[ -n "$npm_root" && -f "$npm_root/haoclaw/package.json" ]]; then
            version=$(node -e "console.log(require('${npm_root}/haoclaw/package.json').version)" 2>/dev/null || true)
        fi
    fi
    echo "$version"
}
...
main() {
    ...
    detected_checkout="$(detect_haoclaw_checkout "$PWD" || true)"

    if [[ -z "$INSTALL_METHOD" && -n "$detected_checkout" ]]; then
        if ! is_promptable; then
            ui_info "Found Haoclaw checkout but no TTY; defaulting to npm install"
            INSTALL_METHOD="npm"
        else
            ...
            case "$selected_method" in
                ...
                *)
                    ...
                    echo "Re-run with: --install-method git|npm (or set HAOCLAW_INSTALL_METHOD)."
                    exit 2
                    ;;
            esac
        fi
    ...
    if check_existing_haoclaw; then
        is_upgrade=true
    fi
    ...
    ui_stage "Installing Haoclaw"

    local final_git_dir=""
    if [[ "$INSTALL_METHOD" == "git" ]]; then
        # Clean up npm global install if switching to git
        if npm list -g haoclaw &>/dev/null; then
            ui_info "Removing npm global install (switching to git)"
            npm uninstall -g haoclaw 2>/dev/null || true
        ...
        install_haoclaw_from_git "$repo_dir"
    else
        # Clean up git wrapper if switching to npm
        if [[ -x "$HOME/.local/bin/haoclaw" ]]; then
            ui_info "Removing git wrapper (switching to npm)"
            rm -f "$HOME/.local/bin/haoclaw"
    ...
    HAOCLAW_BIN="$(resolve_haoclaw_bin || true)"
    ...
    installed_version=$(resolve_haoclaw_version)

    echo ""
    if [[ -n "$installed_version" ]]; then
        ui_celebrate "🐙 Haoclaw installed successfully (${installed_version})!"
    else
        ui_celebrate "🐙 Haoclaw installed successfully!"
    fi
    if [[ "$is_upgrade" == "true" ]]; then
        local update_messages=(
            ...
            "The octopus shipped a new build. Cleaner, faster, calmer."
    ...
    if [[ "$INSTALL_METHOD" == "git" && -n "$final_git_dir" ]]; then
        ...
        ui_kv "Wrapper" "$HOME/.local/bin/haoclaw"
        ui_kv "Update command" "haoclaw update --restart"
        ui_kv "Switch to npm" "curl -fsSL --proto '=https' --tlsv1.2 https://haoclaw.ai/install.sh | bash -s -- --install-method npm"
        ...
        if [[ -r /dev/tty && -w /dev/tty ]]; then
            ...
            ui_info "Running haoclaw doctor"
            local doctor_ok=0
            if (( ${#doctor_args[@]} )); then
                HAOCLAW_UPDATE_IN_PROGRESS=1 "$claw" doctor "${doctor_args[@]}" </dev/tty && doctor_ok=1
            else
                HAOCLAW_UPDATE_IN_PROGRESS=1 "$claw" doctor </dev/tty && doctor_ok=1
            fi
            if (( doctor_ok )); then
                ui_info "Updating plugins"
                HAOCLAW_UPDATE_IN_PROGRESS=1 "$claw" plugins update --all || true
            else
                ui_warn "Doctor failed; skipping plugin updates"
            fi
        else
            ui_info "No TTY; run haoclaw doctor and haoclaw plugins update --all manually"
        fi
    else
        if [[ "$NO_ONBOARD" == "1" || "$skip_onboard" == "true" ]]; then
            ui_info "Skipping onboard (requested); run haoclaw onboard later"
        else
            local config_path="${HAOCLAW_CONFIG_PATH:-$HOME/.haoclaw/haoclaw.json}"
            if [[ -f "${config_path}" || -f "$HOME/.clawdbot/clawdbot.json" || -f "$HOME/.moltbot/moltbot.json" || -f "$HOME/.moldbot/moldbot.json" ]]; then
            ...
            if [[ -r /dev/tty && -w /dev/tty ]]; then
                ...
                if [[ -z "$claw" ]]; then
                    ui_info "Skipping onboarding (haoclaw not on PATH yet)"
                    warn_haoclaw_not_found
                    return 0
                fi
                exec </dev/tty
            ...
            ui_info "No TTY; run haoclaw onboard to finish setup"
            return 0
        fi
    fi

    if command -v haoclaw &> /dev/null; then
        local claw="${HAOCLAW_BIN:-}"
        if [[ -z "$claw" ]]; then
            claw="$(resolve_haoclaw_bin || true)"
        fi
        if [[ -n "$claw" ]] && is_gateway_daemon_loaded "$claw"; then
            if [[ "$DRY_RUN" == "1" ]]; then
                ui_info "Gateway daemon detected; would restart (haoclaw daemon restart)"
            else
                ui_info "Gateway daemon detected; restarting"
                if HAOCLAW_UPDATE_IN_PROGRESS=1 "$claw" daemon restart >/dev/null 2>&1; then
                    ui_success "Gateway restarted"
                else
                    ui_warn "Gateway restart failed; try: haoclaw daemon restart"
                fi
            fi
        fi
    fi
...
if [[ "${HAOCLAW_INSTALL_SH_NO_RUN:-0}" != "1" ]]; then
    parse_args "$@"
    configure_verbose
    main
fi
