...
#
# Suggested cron: */30 * * * * /home/admin/haoclaw/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send Haoclaw notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., haoclaw-alerts)
...
STATE_FILE="$HOME/.haoclaw/auth-monitor-state"
...
send_notification() {
    ...
    # Send via Haoclaw if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use haoclaw
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via Haoclaw to $NOTIFY_PHONE..."
            haoclaw send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! Haoclaw is down. Run: ssh l36 '~/haoclaw/scripts/mobile-reauth.sh'" "urgent"
    exit 1
