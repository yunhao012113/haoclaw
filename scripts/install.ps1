# Haoclaw Installer for Windows (PowerShell)
# Usage: iwr -useb https://haoclaw.ai/install.ps1 | iex
# Or: & ([scriptblock]::Create((iwr -useb https://haoclaw.ai/install.ps1))) -NoOnboard

param(
    [string]$InstallMethod = "npm",
    [string]$Tag = "latest",
    [string]$GitDir = "$env:USERPROFILE\haoclaw",
    [switch]$NoOnboard,
    [switch]$NoGitUpdate,
    [switch]$DryRun
)
...
function Write-Banner {
    Write-Host ""
    Write-Host "${ACCENT}  🐙 Haoclaw Installer$NC" -Level info
    Write-Host "${MUTED}  All your chats, one Haoclaw.$NC" -Level info
    Write-Host ""
}
...
function Install-HaoclawNpm {
    param([string]$Version = "latest")
    
    Write-Host "Installing Haoclaw (haoclaw@$Version)..." -Level info
    
    try {
        # Use -ExecutionPolicy Bypass to handle restricted execution policy
        npm install -g haoclaw@$Version --no-fund --no-audit 2>&1
        Write-Host "Haoclaw installed" -Level success
        return $true
    } catch {
...
function Install-HaoclawGit {
    param([string]$RepoDir, [switch]$Update)
    
    Write-Host "Installing Haoclaw from git..." -Level info
    
    if (!(Test-Path $RepoDir)) {
        Write-Host "  Cloning repository..." -Level info
        git clone https://github.com/haoclaw/haoclaw.git $RepoDir 2>&1
    } elseif ($Update) {
    ...
    @"
@echo off
node "%~dp0..\haoclaw\dist\entry.js" %*
"@ | Out-File -FilePath "$wrapperDir\haoclaw.cmd" -Encoding ASCII -Force
...
function Main {
    ...
    if ($InstallMethod -eq "git") {
        ...
        if ($DryRun) {
            Write-Host "[DRY RUN] Would install Haoclaw from git to $GitDir" -Level info
        } else {
            Install-HaoclawGit -RepoDir $GitDir -Update:(-not $NoGitUpdate)
        }
    } else {
        # npm method
        if (!(Ensure-Git)) {
        ...
        if ($DryRun) {
            Write-Host "[DRY RUN] Would install Haoclaw via npm (tag: $Tag)" -Level info
        } else {
            if (!(Install-HaoclawNpm -Version $Tag)) {
                exit 1
            }
        }
    }
    ...
    if (!$NoOnboard -and !$DryRun) {
        Write-Host ""
        Write-Host "Run 'haoclaw onboard' to complete setup" -Level info
    }
    
    Write-Host ""
    Write-Host "🐙 Haoclaw installed successfully!" -Level success
}
