#!/bin/bash
# Setup Haoclaw Auth Management System
...
echo "=== Haoclaw Auth System Setup ==="
echo ""
...
echo "  2. Subscribe to a topic (e.g., 'haoclaw-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  Haoclaw message: Send warning via Haoclaw itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/haoclaw-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/haoclaw-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/haoclaw-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now haoclaw-auth-monitor.timer
...
echo "  - Auth monitor: systemctl --user status haoclaw-auth-monitor.timer"
echo ""
echo "Quick commands:"
