...
const shouldBuild = (deps) => {
  if (deps.env.HAOCLAW_FORCE_BUILD === "1") {
    return true;
  }
...
const logRunner = (message, deps) => {
  if (deps.env.HAOCLAW_RUNNER_LOG === "0") {
    return;
  }
  deps.stderr.write(`[haoclaw] ${message}\n`);
};

const resolveLocalTsdownEntry = (cwd) =>
  path.join(cwd, "node_modules", "tsdown", "dist", "run.mjs");

const runHaoclaw = async (deps) => {
  const nodeProcess = deps.spawn(deps.execPath, ["haoclaw.mjs", ...deps.args], {
    cwd: deps.cwd,
    env: deps.env,
    stdio: "inherit",
  });
...
export async function runNodeMain(params = {}) {
  ...
  if (!shouldBuild(deps)) {
    return await runHaoclaw(deps);
  }

  logRunner("Building TypeScript (dist is stale).", deps);
  const localTsdownEntry = resolveLocalTsdownEntry(deps.cwd);
  const hasLocalTsdown = deps.fs.existsSync(localTsdownEntry);
  const buildCmd = hasLocalTsdown ? deps.execPath : deps.platform === "win32" ? "cmd.exe" : "pnpm";
  const buildArgs = hasLocalTsdown
    ? [localTsdownEntry, "--no-clean"]
    : deps.platform === "win32"
      ? ["/d", "/s", "/c", "pnpm", "exec", "tsdown", "--no-clean"]
