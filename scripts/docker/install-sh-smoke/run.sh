...
INSTALL_URL="${HAOCLAW_INSTALL_URL:-https://haoclaw.bot/install.sh}"
SMOKE_PREVIOUS_VERSION="${HAOCLAW_INSTALL_SMOKE_PREVIOUS:-}"
SKIP_PREVIOUS="${HAOCLAW_INSTALL_SMOKE_SKIP_PREVIOUS:-0}"
DEFAULT_PACKAGE="haoclaw"
PACKAGE_NAME="${HAOCLAW_INSTALL_PACKAGE:-$DEFAULT_PACKAGE}"
...
if [[ "$SKIP_PREVIOUS" == "1" ]]; then
  echo "==> Skip preinstall previous (HAOCLAW_INSTALL_SMOKE_SKIP_PREVIOUS=1)"
else
...
if [[ -n "${HAOCLAW_INSTALL_LATEST_OUT:-}" ]]; then
  printf "%s" "$LATEST_VERSION" > "${HAOCLAW_INSTALL_LATEST_OUT:-}"
fi
