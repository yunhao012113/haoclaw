...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/haoclaw/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "Haoclaw Re-Auth" -c "After getting key, run: ssh $SERVER '~/haoclaw/scripts/mobile-reauth.sh'" --id haoclaw-auth
        ;;
    *)
