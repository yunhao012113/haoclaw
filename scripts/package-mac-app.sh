...
# Build and bundle Haoclaw into a minimal .app we can open.
# Outputs to dist/Haoclaw.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_ROOT="$ROOT_DIR/dist/Haoclaw.app"
BUILD_ROOT="$ROOT_DIR/apps/macos/.build"
PRODUCT="Haoclaw"
BUNDLE_ID="${BUNDLE_ID:-ai.haoclaw.mac.debug}"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/yunhao012113/haoclaw/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/Haoclaw/Resources/Info.plist"
...
/usr/libexec/PlistBuddy -c "Set :HaoclawBuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :HaoclawGitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/Haoclaw"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/Haoclaw"
fi
chmod +x "$APP_ROOT/Contents/MacOS/Haoclaw"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/Haoclaw" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/Haoclaw/Resources/Haoclaw.icns" "$APP_ROOT/Contents/Resources/Haoclaw.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/Haoclaw/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
if [ -d "$HAOCLAWKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/HaoclawKit_HaoclawKit.bundle"
  cp -R "$HAOCLAWKIT_BUNDLE" "$APP_ROOT/Contents/Resources/HaoclawKit_HaoclawKit.bundle"
else
...
killall -q Haoclaw 2>/dev/null || true
