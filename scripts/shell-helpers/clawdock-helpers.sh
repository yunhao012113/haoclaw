#!/usr/bin/env bash
# ClawDock - Docker helpers for Haoclaw
# Inspired by Simon Willison's "Running Haoclaw in Docker"
# https://til.simonwillison.net/llms/haoclaw-docker
#
# Installation:
...
# Common paths to check for Haoclaw
CLAWDOCK_COMMON_PATHS=(
  "${HOME}/haoclaw"
  "${HOME}/workspace/haoclaw"
  "${HOME}/projects/haoclaw"
  "${HOME}/dev/haoclaw"
  "${HOME}/code/haoclaw"
  "${HOME}/src/haoclaw"
)
...
_clawdock_ensure_dir() {
  ...
  if [[ -n "$found_path" ]]; then
    echo ""
    echo "🐙 Found Haoclaw at: $found_path"
    echo -n "   Use this location? [Y/n] "
    read -r response
    if [[ "$response" =~ ^[Nn] ]]; then
      echo ""
      echo "Set CLAWDOCK_DIR manually:"
      echo "  export CLAWDOCK_DIR=/path/to/haoclaw"
      return 1
    fi
    CLAWDOCK_DIR="$found_path"
  else
    echo ""
    echo "❌ Haoclaw not found in common locations."
    echo ""
    echo "Clone it first:"
    echo ""
    echo "  git clone https://github.com/haoclaw/haoclaw.git ~/haoclaw"
    echo "  cd ~/haoclaw && ./docker-setup.sh"
    echo ""
...
_clawdock_read_env_token() {
  ...
  local raw
  raw=$(sed -n 's/^HAOCLAW_GATEWAY_TOKEN=//p' "${CLAWDOCK_DIR}/.env" | head -n 1)
  if [[ -z "$raw" ]]; then
    return 1
  fi
  _clawdock_trim_quotes "$raw"
...
# Basic Operations
clawdock-start() {
  _clawdock_compose up -d haoclaw-gateway
}
...
clawdock-restart() {
  _clawdock_compose restart haoclaw-gateway
}

clawdock-logs() {
  _clawdock_compose logs -f haoclaw-gateway
}
...
clawdock-workspace() {
  cd ~/.haoclaw/workspace
}

# Container Access
clawdock-shell() {
  _clawdock_compose exec haoclaw-gateway \
    bash -c 'echo "alias haoclaw=\"./haoclaw.mjs\"" > /tmp/.bashrc_haoclaw && bash --rcfile /tmp/.bashrc_haoclaw'
}
...
clawdock-cli() {
  _clawdock_compose run --rm haoclaw-cli "$@"
}

# Maintenance
clawdock-rebuild() {
  _clawdock_compose build haoclaw-gateway
}
...
clawdock-health() {
  ...
  _clawdock_compose exec -e "HAOCLAW_GATEWAY_TOKEN=$token" haoclaw-gateway \
...
clawdock-fix-token() {
  ...
  _clawdock_compose exec -e "TOKEN=$token" haoclaw-gateway \
    bash -c './haoclaw.mjs config set gateway.remote.token "$TOKEN" && ./haoclaw.mjs config set gateway.auth.token "$TOKEN"' 2>&1 | _clawdock_filter_warnings

  echo "🔍 Verifying token was saved..."
  local saved_token
  saved_token=$(_clawdock_compose exec haoclaw-gateway \
    bash -c "./haoclaw.mjs config get gateway.remote.token 2>/dev/null" 2>&1 | _clawdock_filter_warnings | tr -d '\r\n' | head -c 64)
  ...
  _clawdock_compose restart haoclaw-gateway 2>&1 | _clawdock_filter_warnings
...
clawdock-dashboard() {
  ...
  output=$(_clawdock_compose run --rm haoclaw-cli dashboard --no-open 2>&1)
  exit_status=$?
...
clawdock-devices() {
  ...
  output=$(_clawdock_compose exec haoclaw-gateway node dist/index.js devices list 2>&1)
  exit_status=$?
  ...
  if [ $exit_status -ne 0 ]; then
    ...
    echo -e "      $(_cmd 'haoclaw config get gateway.remote.token')"
    return 1
  fi
...
clawdock-help() {
  echo -e "\n${_CLR_BOLD}${_CLR_CYAN}🐙 ClawDock - Docker Helpers for Haoclaw${_CLR_RESET}\n"
  ...
  echo -e "  $(_cmd clawdock-shell)       ${_CLR_DIM}Shell into container (haoclaw alias ready)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-cd)          ${_CLR_DIM}Jump to haoclaw project directory${_CLR_RESET}"
  echo -e "  $(_cmd clawdock-config)      ${_CLR_DIM}Open config directory (~/.haoclaw)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-shell)"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'haoclaw channels login --channel whatsapp')"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'haoclaw status')"
  echo ""
  ...
  echo -e "${_CLR_BLUE}📚 Docs: ${_CLR_RESET}${_CLR_CYAN}https://docs.haoclaw.ai${_CLR_RESET}"
  echo ""
}
