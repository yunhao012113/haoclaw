...
IMAGE_NAME="haoclaw-gateway-network-e2e"

PORT="18789"
TOKEN="e2e-$(date +%s)-$$"
NET_NAME="haoclaw-net-e2e-$$"
GW_NAME="haoclaw-gateway-e2e-$$"
...
docker run -d \
  --name "$GW_NAME" \
  --network "$NET_NAME" \
  -e "HAOCLAW_GATEWAY_TOKEN=$TOKEN" \
  -e "HAOCLAW_SKIP_CHANNELS=1" \
  -e "HAOCLAW_SKIP_GMAIL_WATCHER=1" \
  -e "HAOCLAW_SKIP_CRON=1" \
  -e "HAOCLAW_SKIP_CANVAS_HOST=1" \
  "$IMAGE_NAME" \
