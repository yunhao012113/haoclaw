 ...
 * `haoclaw update`. Use it to verify the completion installation flow
...
\`haoclaw update\`, \`haoclaw doctor\`, and \`haoclaw onboard\`.
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(
      theme.muted("  1. Remove the '# Haoclaw Completion' block from your shell profile"),
    );
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`haoclaw completion --install\` later to enable.`));
    return;
  }
