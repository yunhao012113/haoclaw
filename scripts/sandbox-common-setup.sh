...
BASE_IMAGE="${BASE_IMAGE:-haoclaw-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-haoclaw-sandbox-common:bookworm-slim}"
...
HAOCLAW_DOCKER_BUILD_USE_BUILDX="${HAOCLAW_DOCKER_BUILD_USE_BUILDX:-0}"
HAOCLAW_DOCKER_BUILD_CACHE_FROM="${HAOCLAW_DOCKER_BUILD_CACHE_FROM:-}"
HAOCLAW_DOCKER_BUILD_CACHE_TO="${HAOCLAW_DOCKER_BUILD_CACHE_TO:-}"
...
if [ "${HAOCLAW_DOCKER_BUILD_USE_BUILDX}" = "1" ]; then
  build_cmd=(docker buildx build --load)
  if [ -n "${HAOCLAW_DOCKER_BUILD_CACHE_FROM}" ]; then
    build_cmd+=(--cache-from "${HAOCLAW_DOCKER_BUILD_CACHE_FROM}")
  fi
  if [ -n "${HAOCLAW_DOCKER_BUILD_CACHE_TO}" ]; then
    build_cmd+=(--cache-to "${HAOCLAW_DOCKER_BUILD_CACHE_TO}")
  fi
fi
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=haoclaw.sandbox=1)
NOTE
