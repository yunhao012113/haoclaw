...
struct ControlAgentEvent: Codable, Identifiable {
    ...
    let stream: String
    let ts: Double
    let data: [String: HaoclawProtocol.AnyCodable]
    let summary: String?
}
...
final class ControlChannel {
    ...
    private let logger = Logger(subsystem: "ai.haoclaw", category: "control")
    ...
    {
        do {
            let rawParams = params?.reduce(into: [String: HaoclawKit.AnyCodable]()) {
                $0[$1.key] = HaoclawKit.AnyCodable($1.value.base)
            }
    ...
    private static func bridgeToProtocolArgs(
        _ value: HaoclawProtocol.AnyCodable?) -> [String: HaoclawProtocol.AnyCodable]?
    {
        guard let value else { return nil }
        if let dict = value.value as? [String: HaoclawProtocol.AnyCodable] {
            return dict
        }
        if let dict = value.value as? [String: HaoclawKit.AnyCodable],
           let data = try? JSONEncoder().encode(dict),
           let decoded = try? JSONDecoder().decode([String: HaoclawProtocol.AnyCodable].self, from: data)
        {
            return decoded
        }
...
extension Notification.Name {
    static let controlHeartbeat = Notification.Name("haoclaw.control.heartbeat")
    static let controlAgentEvent = Notification.Name("haoclaw.control.agent")
}
