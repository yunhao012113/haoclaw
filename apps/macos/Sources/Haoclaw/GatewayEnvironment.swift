...
enum GatewayEnvironment {
    private static let logger = Logger(subsystem: "ai.haoclaw", category: "gateway.env")
    private static let supportedBindModes: Set<String> = ["loopback", "tailnet", "lan", "auto"]

    static func gatewayPort() -> Int {
        if let raw = ProcessInfo.processInfo.environment["HAOCLAW_GATEWAY_PORT"] {
            let trimmed = raw.trimmingCharacters(in: .whitespacesAndNewlines)
            if let parsed = Int(trimmed), parsed > 0 { return parsed }
        }
        if let configPort = HaoclawConfigFile.gatewayPort(), configPort > 0 {
            return configPort
        }
        let stored = UserDefaults.standard.integer(forKey: "gatewayPort")
        return stored > 0 ? stored : 3456
    }
    ...
    static func check() -> GatewayEnvironmentStatus {
        ...
        case let .success(runtime):
            let gatewayBin = CommandResolver.haoclawExecutable()
            ...
            if gatewayBin == nil, projectEntrypoint == nil {
                return GatewayEnvironmentStatus(
                    ...
                    gatewayVersion: nil,
                    requiredGateway: expectedString,
                    message: "haoclaw CLI not found in PATH; install the CLI.")
            }
    ...
    static func resolveGatewayCommand() -> GatewayCommandResolution {
        ...
        if let gatewayBin {
            ...
            let cmd = [gatewayBin, "gateway", "run", "--port", "\(port)", "--bind", bind, "--allow-unconfigured"]
    ...
    private static func preferredGatewayBind() -> String? {
        ...
        if let env = ProcessInfo.processInfo.environment["HAOCLAW_GATEWAY_BIND"] {
    ...
    static func installGlobal(versionString: String?, statusHandler: @escaping @Sendable (String) -> Void) async {
        ...
        let (label, cmd): (String, [String]) =
            if let npm {
                ("npm", [npm, "install", "-g", "haoclaw@\(target)"])
            } else if let pnpm {
                ("pnpm", [pnpm, "add", "-g", "haoclaw@\(target)"])
            } else if let bun {
                ("bun", [bun, "add", "-g", "haoclaw@\(target)"])
            } else {
                ("npm", ["npm", "install", "-g", "haoclaw@\(target)"])
            }

        statusHandler("Installing haoclaw@\(target) via \(label)…")
        ...
        if response.success {
            statusHandler("Installed haoclaw@\(target)")
        } else {
