...
final class AppState {
    ...
    init(preview: Bool = false) {
        ...
        let configRoot = HaoclawConfigFile.loadDict()
        ...
        remoteTokenDirty: Bool) -> (remote: [String: Any], changed: Bool)
    {
        ...
        case .ssh:
            ...
            if let host = remoteHost {
                ...
                let port = parsedExisting?.port ?? 3456
    ...
    private func startConfigWatcher() {
        let configUrl = HaoclawConfigFile.url()
    ...
    private func syncGatewayConfigIfNeeded() {
        ...
        Task { @MainActor in
            ...
            let synced = Self.syncedGatewayRoot(
                currentRoot: HaoclawConfigFile.loadDict(),
                connectionMode: connectionMode,
                remoteTransport: remoteTransport,
                remoteTarget: remoteTarget,
                remoteIdentity: remoteIdentity,
...
extension AppState {
    static var preview: AppState {
        ...
        state.remoteProjectRoot = "~/Projects/haoclaw"
        state.remoteCliPath = ""
        return state
    }
}
