...
private let webChatSwiftLogger = Logger(subsystem: "ai.haoclaw", category: "WebChatSwiftUI")
...
struct MacGatewayChatTransport: HaoclawChatTransport {
    ...
    static func mapPushToTransportEvent(_ push: GatewayPush) -> HaoclawChatTransportEvent? {
        switch push {
        case let .snapshot(hello):
            let ok = (try? JSONDecoder().decode(
                HaoclawGatewayHealthOK.self,
...
final class WebChatSwiftUIWindowController {
    ...
    private let hosting: NSHostingController<HaoclawChatView>
    ...
    init(sessionKey: String, presentation: WebChatPresentation, transport: any HaoclawChatTransport) {
        self.sessionKey = sessionKey
        self.presentation = presentation
        let vm = HaoclawChatViewModel(sessionKey: sessionKey, transport: transport)
        let accent = Self.color(fromHex: AppStateStore.shared.seamColorHex)
        self.hosting = NSHostingController(rootView: HaoclawChatView(
            viewModel: vm,
    ...
    private static func makeContentController(
        for presentation: WebChatPresentation,
        hosting: NSHostingController<HaoclawChatView>) -> NSViewController
    {
