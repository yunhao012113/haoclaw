...
enum LogLocator {
    private static var logDir: URL {
        if let override = ProcessInfo.processInfo.environment["HAOCLAW_LOG_DIR"],
           !override.isEmpty
        {
            return URL(fileURLWithPath: override)
        }
        return URL(fileURLWithPath: "/tmp/haoclaw")
    }

    private static var stdoutLog: URL {
        logDir.appendingPathComponent("haoclaw-stdout.log")
    }

    private static var gatewayLog: URL {
        logDir.appendingPathComponent("haoclaw-gateway.log")
    }
    ...
    /// Returns the newest log file under /tmp/haoclaw/ (rolling or stdout), or nil if none exist.
