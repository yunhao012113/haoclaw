...
private struct DesktopControlSkillsPane: View {
    ...
    @State private var filter: DesktopSkillsFilter = .all
    @State private var envEditor: DesktopSkillEnvEditorState?
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 18) {
                ...
                VStack(alignment: .leading, spacing: 12) {
                    ...
                    HStack(spacing: 10) {
                        ...
                        Picker("筛选", selection: self.$filter) {
                            ForEach(DesktopSkillsFilter.allCases) { filter in
            ...
            DesktopControlCard(title: "技能列表", subtitle: "直接在应用里查看、启停和补齐依赖。") {
                VStack(alignment: .leading, spacing: 12) {
                    if let error = self.model.error, !error.isEmpty {
                        Text(error)
                            .font(.footnote)
                    ...
                    } else if let statusMessage = self.model.statusMessage, !statusMessage.isEmpty {
                        Text(statusMessage)
                            .font(.footnote)
                            .foregroundStyle(.secondary)
                    }

                    if self.model.isLoading && self.model.skills.isEmpty {
                        HStack(spacing: 10) {
                            ProgressView()
                            Text("正在加载技能列表…")
                                .foregroundStyle(.secondary)
                        ...
                        Text("当前没有可显示的技能。先刷新一次，或者把带 `SKILL.md` 的技能放到 `~/.haoclaw/skills`。")
                            .foregroundStyle(.secondary)
                    } else {
                        LazyVStack(spacing: 12) {
                            ForEach(self.filteredSkills) { skill in
                                DesktopSkillStatusCard(
                                    skill: skill,
                                    ...
                                    onSetEnv: { envKey, isPrimary in
                                        self.envEditor = DesktopSkillEnvEditorState(
        ...
        .sheet(item: self.$envEditor) { editor in
            DesktopSkillEnvEditorView(editor: editor) { value in
                Task {
...
private enum DesktopSkillsFilter: String, CaseIterable, Identifiable {
    case all
    case ready
    case needsSetup
    case disabled
...
private struct DesktopSkillStatusCard: View {
    ...
    private var installOptions: [SkillInstallOption] {
        guard !self.skill.missing.bins.isEmpty else { return [] }
        let missing = Set(self.skill.missing.bins)
    ...
    private var requirementsMet: Bool {
        self.skill.missing.bins.isEmpty && self.skill.missing.env.isEmpty && self.skill.missing.config.isEmpty
    }

    private var enabledBinding: Binding<Bool> {
        Binding(get: { !self.skill.disabled }, set: { self.onToggleEnabled($0) })
    }
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 12) {
                ...
                VStack(alignment: .leading, spacing: 4) {
                    HStack(spacing: 8) {
                        Text(self.skill.name)
                            .font(.headline)
                        DesktopSkillBadge(text: self.sourceLabel)
                        if self.skill.disabled {
                            DesktopSkillBadge(text: "已禁用", tint: .orange)
                        } else if self.skill.eligible {
                            DesktopSkillBadge(text: "可用", tint: .green)
                        } else {
                            DesktopSkillBadge(text: "待配置", tint: .blue)
                        }
                    }
            ...
            if !self.requirementsMet {
                VStack(alignment: .leading, spacing: 4) {
                    if !self.skill.missing.bins.isEmpty {
                        Text("缺少命令：\(self.skill.missing.bins.joined(separator: ", "))")
                    }
                    if !self.skill.missing.env.isEmpty {
                        Text("缺少变量：\(self.skill.missing.env.joined(separator: ", "))")
                    }
                    if !self.skill.missing.config.isEmpty {
                        Text("缺少配置：\(self.skill.missing.config.joined(separator: ", "))")
                    }
                }
                .font(.caption)
                .foregroundStyle(.secondary)
            ...
            if !self.skill.configChecks.isEmpty {
                VStack(alignment: .leading, spacing: 4) {
                    ForEach(self.skill.configChecks) { check in
                        HStack(spacing: 6) {
                            Image(systemName: check.satisfied ? "checkmark.circle.fill" : "exclamationmark.circle")
                                .foregroundStyle(check.satisfied ? .green : .orange)
                            Text(check.path)
                                .font(.caption)
                            Spacer()
                        }
            ...
            HStack(spacing: 8) {
                if self.installOptions.isEmpty {
                    Toggle("启用技能", isOn: self.enabledBinding)
                        .toggleStyle(.switch)
                        .disabled(self.isBusy || !self.requirementsMet)
                } else {
                    ForEach(self.installOptions) { option in
                        Button("安装 \(option.label)") {
                            self.onInstall(option, .local)
                        }
                        .buttonStyle(.borderedProminent)
                ...
                ForEach(self.skill.missing.env, id: \.self) { envKey in
                    Button(envKey == self.skill.primaryEnv ? "填写 API Key" : "填写 \(envKey)") {
                        self.onSetEnv(envKey, envKey == self.skill.primaryEnv)
                    }
                    .buttonStyle(.bordered)
                    .disabled(self.isBusy)
                }

                if let homepage = self.homepageURL {
                    Button("说明") {
    ...
    private var homepageURL: URL? {
        guard let raw = self.skill.homepage?.trimmingCharacters(in: .whitespacesAndNewlines), !raw.isEmpty else {
            return nil
        }
...
private struct DesktopSkillBadge: View {
    let text: String
...
private struct DesktopSkillEnvEditorState: Identifiable {
    let skillKey: String
...
private struct DesktopSkillEnvEditorView: View {
    let editor: DesktopSkillEnvEditorState
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 12) {
            Text(self.editor.isPrimary ? "填写 API Key" : "填写环境变量")
                .font(.headline)
            Text("技能：\(self.editor.skillName)")
                .font(.subheadline)
                .foregroundStyle(.secondary)
