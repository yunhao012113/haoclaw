...
enum HaoclawLogging {
    ...
    private static let didBootstrap: Void = {
        LoggingSystem.bootstrap { label in
            ...
            let osHandler = HaoclawOSLogHandler(subsystem: subsystem, category: category)
            let fileHandler = HaoclawFileLogHandler(label: label)
...
extension Logging.Logger {
    init(subsystem: String, category: String) {
        HaoclawLogging.bootstrapIfNeeded()
        let label = HaoclawLogging.makeLabel(subsystem: subsystem, category: category)
...
struct HaoclawOSLogHandler: AppLogLevelBackedHandler {
...
struct HaoclawFileLogHandler: AppLogLevelBackedHandler {
    let label: String
    var metadata: Logger.Metadata = [:]
        ...
        line: UInt)
    {
        ...
        let (subsystem, category) = HaoclawLogging.parseLabel(self.label)
