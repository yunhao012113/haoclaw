...
enum CommandResolver {
    private static let projectRootDefaultsKey = "haoclaw.gatewayProjectRootPath"
    private static let helperName = "haoclaw"

    static func gatewayEntrypoint(in root: URL) -> String? {
        let distEntry = root.appendingPathComponent("dist/index.js").path
        if FileManager().isReadableFile(atPath: distEntry) { return distEntry }
        let haoclawEntry = root.appendingPathComponent("haoclaw.mjs").path
        if FileManager().isReadableFile(atPath: haoclawEntry) { return haoclawEntry }
        let binEntry = root.appendingPathComponent("bin/haoclaw.js").path
    ...
    static func projectRoot() -> URL {
        ...
        let fallback = FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent("Projects/haoclaw")
    ...
    static func preferredPaths(home: URL, current: [String], projectRoot: URL) -> [String] {
        ...
        #endif
        let haoclawPaths = self.haoclawManagedPaths(home: home)
        if !haoclawPaths.isEmpty {
            extras.insert(contentsOf: haoclawPaths, at: 1)
        }
        extras.insert(contentsOf: self.nodeManagerBinPaths(home: home), at: 1 + haoclawPaths.count)
        var seen = Set<String>()
        // Preserve order while stripping duplicates so PATH lookups remain deterministic.
        return (extras + current).filter { seen.insert($0).inserted }
    }

    private static func haoclawManagedPaths(home: URL) -> [String] {
        let bases = [
            home.appendingPathComponent(".haoclaw"),
        ]
        var paths: [String] = []
        for base in bases {
    ...
    static func nodeCliPath() -> String? {
        ...
        let candidates = [
            root.appendingPathComponent("haoclaw.mjs").path,
            root.appendingPathComponent("bin/haoclaw.js").path,
        ]
    ...
    static func hasAnyHaoclawInvoker(searchPaths: [String]? = nil) -> Bool {
        if self.haoclawExecutable(searchPaths: searchPaths) != nil { return true }
    ...
    static func haoclawNodeCommand(
        subcommand: String,
        extraArgs: [String] = [],
        ...
        searchPaths: [String]? = nil) -> [String]
    {
        ...
        let root = self.projectRoot()
        if let haoclawPath = self.projectHaoclawExecutable(projectRoot: root) {
            return [haoclawPath, subcommand] + extraArgs
        }
        if let haoclawPath = self.haoclawExecutable(searchPaths: searchPaths) {
            return [haoclawPath, subcommand] + extraArgs
        }
        ...
        if let pnpm = self.findExecutable(named: "pnpm", searchPaths: searchPaths) {
            ...
            return [pnpm, "--silent", "haoclaw", subcommand] + extraArgs
        }

        switch runtimeResult {
        case .success:
            let missingEntry = """
            haoclaw entrypoint missing (looked for dist/index.js or haoclaw.mjs); run pnpm build.
            """
    ...
    private static func sshNodeCommand(subcommand: String, extraArgs: [String], settings: RemoteSettings) -> [String]? {
        ...
        // Run the real haoclaw CLI on the remote host.
        let exportedPath = [
            "/opt/homebrew/bin",
            "/usr/local/bin",
            "/usr/bin",
        ...
        let projectSection = if userPRJ.isEmpty {
            """
            DEFAULT_PRJ="$HOME/Projects/haoclaw"
        ...
        \(cliSection)
        \(projectSection)
        if command -v haoclaw >/dev/null 2>&1; then
          CLI="$(command -v haoclaw)"
          haoclaw \(quotedArgs);
        ...
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/haoclaw.mjs" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/haoclaw.mjs"
            node "$PRJ/haoclaw.mjs" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/bin/haoclaw.js" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/bin/haoclaw.js"
            node "$PRJ/bin/haoclaw.js" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif command -v pnpm >/dev/null 2>&1; then
          CLI="pnpm --silent haoclaw"
          pnpm --silent haoclaw \(quotedArgs);
        else
          echo "haoclaw CLI missing on remote host"; exit 127;
        fi
        """
        let options: [String] = [
            "-o", "BatchMode=yes",
    ...
    {
        let root = configRoot ?? HaoclawConfigFile.loadDict()
