...
extension ProcessInfo {
    ...
    /// Nix deployments may write defaults into a stable suite (`ai.haoclaw.mac`) even if the shipped
    ...
    {
        if environment["HAOCLAW_NIX_MODE"] == "1" { return true }
        if standard.bool(forKey: "haoclaw.nixMode") { return true }
        ...
        if isAppBundle, let stableSuite, stableSuite.bool(forKey: "haoclaw.nixMode") { return true }
