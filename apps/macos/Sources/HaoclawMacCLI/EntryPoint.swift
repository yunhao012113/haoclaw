...
struct HaoclawMacCLI {
    static func main() async {
        ...
        default:
            fputs("haoclaw-mac: unknown command\n", stderr)
            printUsage()
            exit(1)
        }
    }
...
private func printUsage() {
    ...
    Usage:
      haoclaw-mac connect [--url <ws://host:port>] [--token <token>] [--password <password>]
                           [--mode <local|remote>] [--timeout <ms>] [--probe] [--json]
                           [--client-id <id>] [--client-mode <mode>] [--display-name <name>]
                           [--role <role>] [--scopes <a,b,c>]
      haoclaw-mac discover [--timeout <ms>] [--json] [--include-local]
      haoclaw-mac wizard [--url <ws://host:port>] [--token <token>] [--password <password>]
                          [--mode <local|remote>] [--workspace <path>] [--json]

    Examples:
      haoclaw-mac connect
      haoclaw-mac connect --url ws://127.0.0.1:18789 --json
      haoclaw-mac discover --timeout 3000 --json
      haoclaw-mac wizard --mode local
    """)
}
