...
struct MacGatewayChatTransportMappingTests {
    @Test func `snapshot maps to health`() {
        let snapshot = Snapshot(
            presence: [],
            health: HaoclawProtocol.AnyCodable(["ok": HaoclawProtocol.AnyCodable(false)]),
    ...
    @Test func `health event maps to health`() {
        let frame = EventFrame(
            type: "event",
            event: "health",
            payload: HaoclawProtocol.AnyCodable(["ok": HaoclawProtocol.AnyCodable(true)]),
            seq: 1,
            stateversion: nil)
    ...
    @Test func `chat event maps to chat`() {
        let payload = HaoclawProtocol.AnyCodable([
            "runId": HaoclawProtocol.AnyCodable("run-1"),
            "sessionKey": HaoclawProtocol.AnyCodable("main"),
            "state": HaoclawProtocol.AnyCodable("final"),
        ])
    ...
    @Test func `unknown event maps to nil`() {
        let frame = EventFrame(
            type: "event",
            event: "unknown",
            payload: HaoclawProtocol.AnyCodable(["a": HaoclawProtocol.AnyCodable(1)]),
            seq: 1,
            stateversion: nil)
        let mapped = MacGatewayChatTransport.mapPushToTransportEvent(.event(frame))
        #expect(mapped == nil)
