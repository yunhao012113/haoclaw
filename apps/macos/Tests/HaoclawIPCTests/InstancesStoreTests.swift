...
struct InstancesStoreTests {
    ...
    func `presence event payload decodes via JSON encoder`() {
        ...
        let entry: [String: HaoclawProtocol.AnyCodable] = [
            "host": .init("gw"),
            "ip": .init("10.0.0.1"),
            "version": .init("2.0.0"),
            "mode": .init("gateway"),
        ...
        let payloadMap: [String: HaoclawProtocol.AnyCodable] = [
            "presence": .init([HaoclawProtocol.AnyCodable(entry)]),
        ]
        let payload = HaoclawProtocol.AnyCodable(payloadMap)
