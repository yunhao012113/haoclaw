...
struct VoiceWakeRuntimeTests {
    ...
    @Test func `has content after trigger false when only trigger`() {
        let triggers = ["haoclaw"]
        let text = "hey haoclaw"
    ...
    @Test func `gate requires gap between trigger and command`() {
        let transcript = "hey haoclaw do thing"
        ...
        let config = WakeWordGateConfig(triggers: ["haoclaw"], minPostTriggerGap: 0.3)
