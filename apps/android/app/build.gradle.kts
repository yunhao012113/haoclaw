...
val androidStoreFile = providers.gradleProperty("HAOCLAW_ANDROID_STORE_FILE").orNull?.takeIf { it.isNotBlank() }
val androidStorePassword = providers.gradleProperty("HAOCLAW_ANDROID_STORE_PASSWORD").orNull?.takeIf { it.isNotBlank() }
val androidKeyAlias = providers.gradleProperty("HAOCLAW_ANDROID_KEY_ALIAS").orNull?.takeIf { it.isNotBlank() }
val androidKeyPassword = providers.gradleProperty("HAOCLAW_ANDROID_KEY_PASSWORD").orNull?.takeIf { it.isNotBlank() }
...
if (wantsAndroidReleaseBuild && !hasAndroidReleaseSigning) {
    error(
        "Missing Android release signing properties. Set HAOCLAW_ANDROID_STORE_FILE, " +
            "HAOCLAW_ANDROID_STORE_PASSWORD, HAOCLAW_ANDROID_KEY_ALIAS, and " +
            "HAOCLAW_ANDROID_KEY_PASSWORD in ~/.gradle/gradle.properties.",
    )
}
...
android {
    ...
    sourceSets {
        getByName("main") {
            assets.directories.add("../../shared/HaoclawKit/Sources/HaoclawKit/Resources")
        }
    }

    defaultConfig {
        applicationId = "ai.haoclaw.app"
        minSdk = 31
        targetSdk = 36
    ...
    onVariants { variant ->
        variant.outputs
            ...
            .forEach { output ->
                ...
                val outputFileName = "haoclaw-$versionName-$buildType.apk"
