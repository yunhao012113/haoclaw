...
private func historyPayload(
    ...
    messages: [AnyCodable] = []) -> HaoclawChatHistoryPayload
{
    HaoclawChatHistoryPayload(
...
private func sessionEntry(key: String, updatedAt: Double) -> HaoclawChatSessionEntry {
...
private func makeViewModel(
    sessionKey: String = "main",
    historyResponses: [HaoclawChatHistoryPayload],
    sessionsResponses: [HaoclawChatSessionsListResponse] = []) async -> (TestChatTransport, HaoclawChatViewModel)
{
    let transport = TestChatTransport(historyResponses: historyResponses, sessionsResponses: sessionsResponses)
    let vm = await MainActor.run { HaoclawChatViewModel(sessionKey: sessionKey, transport: transport) }
    return (transport, vm)
}

private func loadAndWaitBootstrap(
    vm: HaoclawChatViewModel,
...
private func sendUserMessage(_ vm: HaoclawChatViewModel, text: String = "hi") async {
    await MainActor.run {
        vm.input = text
        vm.send()
    }
...
private final class TestChatTransport: @unchecked Sendable, HaoclawChatTransport {
    private let state = TestChatTransportState()
    private let historyResponses: [HaoclawChatHistoryPayload]
    private let sessionsResponses: [HaoclawChatSessionsListResponse]

    private let stream: AsyncStream<HaoclawChatTransportEvent>
    private let continuation: AsyncStream<HaoclawChatTransportEvent>.Continuation

    init(
        historyResponses: [HaoclawChatHistoryPayload],
        sessionsResponses: [HaoclawChatSessionsListResponse] = [])
    {
        self.historyResponses = historyResponses
        self.sessionsResponses = sessionsResponses
        var cont: AsyncStream<HaoclawChatTransportEvent>.Continuation!
    ...
    func requestHistory(sessionKey: String) async throws -> HaoclawChatHistoryPayload {
        ...
        return self.historyResponses.last ?? HaoclawChatHistoryPayload(
            sessionKey: sessionKey,
            sessionId: nil,
            messages: [],
            thinkingLevel: "off")
        ...
        attachments _: [HaoclawChatAttachmentPayload]) async throws -> HaoclawChatSendResponse
    {
        ...
        return HaoclawChatSendResponse(runId: idempotencyKey, status: "ok")
    }
    ...
    func listSessions(limit _: Int?) async throws -> HaoclawChatSessionsListResponse {
        ...
        return self.sessionsResponses.last ?? HaoclawChatSessionsListResponse(
            ts: nil,
            path: nil,
            count: 0,
            defaults: nil,
    ...
    func emit(_ evt: HaoclawChatTransportEvent) {
...
@Suite struct ChatViewModelTests {
    ...
    @Test func sessionChoicesPreferMainAndRecent() async throws {
        ...
        let sessions = HaoclawChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 4,
            defaults: nil,
...
"""]],
        ...
        let vm = await MainActor.run { HaoclawChatViewModel(sessionKey: "main", transport: transport) }
