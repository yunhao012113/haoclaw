...
private let chatUILogger = Logger(subsystem: "ai.haoclaw", category: "HaoclawChatUI")

@MainActor
@Observable
public final class HaoclawChatViewModel {
    public private(set) var messages: [HaoclawChatMessage] = []
    ...
    public var attachments: [HaoclawPendingAttachment] = []
    ...
    public private(set) var pendingToolCalls: [HaoclawChatPendingToolCall] = []
    public private(set) var sessions: [HaoclawChatSessionEntry] = []
    private let transport: any HaoclawChatTransport
    ...
    private var pendingToolCallsById: [String: HaoclawChatPendingToolCall] = [:] {
        didSet {
    ...
    public init(sessionKey: String, transport: any HaoclawChatTransport) {
    ...
    public var sessionChoices: [HaoclawChatSessionEntry] {
        ...
        var result: [HaoclawChatSessionEntry] = []
    ...
    public func removeAttachment(_ id: HaoclawPendingAttachment.ID) {
    ...
    private static func decodeMessages(_ raw: [AnyCodable]) -> [HaoclawChatMessage] {
        let decoded = raw.compactMap { item in
            (try? ChatPayloadDecoding.decode(item, as: HaoclawChatMessage.self))
                .map { Self.stripInboundMetadata(from: $0) }
        }
        return Self.dedupeMessages(decoded)
    }

    private static func stripInboundMetadata(from message: HaoclawChatMessage) -> HaoclawChatMessage {
        guard message.role.lowercased() == "user" else {
            return message
        }

        let sanitizedContent = message.content.map { content -> HaoclawChatMessageContent in
            guard let text = content.text else { return content }
            let cleaned = ChatMarkdownPreprocessor.preprocess(markdown: text).cleaned
            return HaoclawChatMessageContent(
                type: content.type,
                text: cleaned,
    ...
    private static func messageIdentityKey(for message: HaoclawChatMessage) -> String? {
    ...
    private static func reconcileMessageIDs(
        previous: [HaoclawChatMessage],
        incoming: [HaoclawChatMessage]) -> [HaoclawChatMessage]
    {
    ...
    private static func dedupeMessages(_ messages: [HaoclawChatMessage]) -> [HaoclawChatMessage] {
        var result: [HaoclawChatMessage] = []
    ...
    private static func dedupeKey(for message: HaoclawChatMessage) -> String? {
    ...
    private func performSend() async {
        ...
        var userContent: [HaoclawChatMessageContent] = [
            HaoclawChatMessageContent(
                type: "text",
                text: messageText,
                thinking: nil,
        ...
        let encodedAttachments = self.attachments.map { att -> HaoclawChatAttachmentPayload in
            HaoclawChatAttachmentPayload(
                type: att.type,
    ...
    private func handleTransportEvent(_ evt: HaoclawChatTransportEvent) {
        switch evt {
        case let .health(ok):
            self.healthOK = ok
        case .tick:
    ...
    private func handleChatEvent(_ chat: HaoclawChatEventPayload) {
    ...
    private func handleAgentEvent(_ evt: HaoclawAgentEventPayload) {
        ...
        case "tool":
            ...
            if phase == "start" {
                ...
                self.pendingToolCallsById[toolCallId] = HaoclawChatPendingToolCall(
    ...
    private static func previewImage(data: Data) -> HaoclawPlatformImage? {
        #if canImport(AppKit)
        NSImage(data: data)
        #elseif canImport(UIKit)
        UIImage(data: data)
