...
@MainActor
public struct HaoclawChatView: View {
    public enum Style {
        case standard
        case onboarding
    }

    @State private var viewModel: HaoclawChatViewModel
    ...
    public init(
        viewModel: HaoclawChatViewModel,
    ...
    private var visibleMessages: [HaoclawChatMessage] {
        let base: [HaoclawChatMessage]
    ...
    private func isToolResultMessage(_ message: HaoclawChatMessage) -> Bool {
        let role = message.role.lowercased()
        return role == "toolresult" || role == "tool_result"
    }

    private func shouldDisplayMessage(_ message: HaoclawChatMessage) -> Bool {
    ...
    private func primaryText(in message: HaoclawChatMessage) -> String {
    ...
    private func hasInlineAttachments(in message: HaoclawChatMessage) -> Bool {
    ...
    private func toolCalls(in message: HaoclawChatMessage) -> [HaoclawChatMessageContent] {
    ...
    private func inlineToolResults(in message: HaoclawChatMessage) -> [HaoclawChatMessageContent] {
        message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private func toolCallIds(in message: HaoclawChatMessage) -> Set<String> {
        var ids = Set<String>()
    ...
    private func toolResultText(from message: HaoclawChatMessage) -> String {
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 18, style: .continuous)
                .fill(HaoclawChatTheme.subtleCard)
                .overlay(
                    RoundedRectangle(cornerRadius: 18, style: .continuous)
