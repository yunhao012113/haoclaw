...
@MainActor
final class HaoclawAppDelegate: NSObject, UIApplicationDelegate, @preconcurrency UNUserNotificationCenterDelegate {
    private let logger = Logger(subsystem: "ai.haoclaw.ios", category: "Push")
    private let backgroundWakeLogger = Logger(subsystem: "ai.haoclaw.ios", category: "BackgroundWake")
    private static let wakeRefreshTaskIdentifier = "ai.haoclaw.ios.bgrefresh"
...
enum WatchPromptNotificationBridge {
    static let typeKey = "haoclaw.type"
    static let typeValue = "watch.prompt"
    static let promptIDKey = "haoclaw.watch.promptId"
    static let sessionKeyKey = "haoclaw.watch.sessionKey"
    static let actionPrimaryIDKey = "haoclaw.watch.action.primary.id"
    static let actionPrimaryLabelKey = "haoclaw.watch.action.primary.label"
    static let actionSecondaryIDKey = "haoclaw.watch.action.secondary.id"
    static let actionSecondaryLabelKey = "haoclaw.watch.action.secondary.label"
    static let actionPrimaryIdentifier = "haoclaw.watch.action.primary"
    static let actionSecondaryIdentifier = "haoclaw.watch.action.secondary"
    static let actionIdentifierPrefix = "haoclaw.watch.action."
    static let actionIDKeyPrefix = "haoclaw.watch.action.id."
    static let actionLabelKeyPrefix = "haoclaw.watch.action.label."
    static let categoryPrefix = "haoclaw.watch.prompt.category."

    @MainActor
    static func scheduleMirroredWatchPromptNotificationIfNeeded(
        invokeID: String,
        params: HaoclawWatchNotifyParams,
        sendResult: WatchNotificationSendResult) async
    {
        ...
        let normalizedActions = (params.actions ?? []).compactMap { action -> HaoclawWatchAction? in
            let id = action.id.trimmingCharacters(in: .whitespacesAndNewlines)
            let label = action.label.trimmingCharacters(in: .whitespacesAndNewlines)
            guard !id.isEmpty, !label.isEmpty else { return nil }
            return HaoclawWatchAction(id: id, label: label, style: action.style)
        }
        ...
        content.title = title.isEmpty ? "Haoclaw" : title
        content.body = body
    ...
    private static func categoryActions(_ actions: [HaoclawWatchAction]) -> [UNNotificationAction] {
...
@main
struct HaoclawApp: App {
    @State private var appModel: NodeAppModel
    @State private var gatewayController: GatewayConnectionController
    @UIApplicationDelegateAdaptor(HaoclawAppDelegate.self) private var appDelegate
...
extension HaoclawApp {
    private static func installUncaughtExceptionLogger() {
        NSLog("Haoclaw: installing uncaught exception handler")
