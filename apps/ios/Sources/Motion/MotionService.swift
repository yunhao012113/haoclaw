...
final class MotionService: MotionServicing {
    func activities(params: HaoclawMotionActivityParams) async throws -> HaoclawMotionActivityPayload {
        ...
        let mapped: [HaoclawMotionActivityEntry] = try await withCheckedThrowingContinuation { cont in
        ...
        return HaoclawMotionActivityPayload(activities: mapped)
    }
    ...
    func pedometer(params: HaoclawPedometerParams) async throws -> HaoclawPedometerPayload {
        ...
        let payload: HaoclawPedometerPayload = try await withCheckedThrowingContinuation { cont in
            pedometer.queryPedometerData(from: start, to: end) { data, error in
                if let error {
                    cont.resume(throwing: error)
                } else {
                    let formatter = ISO8601DateFormatter()
                    let payload = HaoclawPedometerPayload(
