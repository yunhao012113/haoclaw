...
struct IOSGatewayChatTransport: HaoclawChatTransport, Sendable {
    private static let logger = Logger(subsystem: "ai.haoclaw", category: "ios.chat.transport")
    ...
    func listSessions(limit: Int?) async throws -> HaoclawChatSessionsListResponse {
        ...
        return try JSONDecoder().decode(HaoclawChatSessionsListResponse.self, from: res)
    }
    ...
    func requestHistory(sessionKey: String) async throws -> HaoclawChatHistoryPayload {
        ...
        return try JSONDecoder().decode(HaoclawChatHistoryPayload.self, from: res)
    }
        ...
        attachments: [HaoclawChatAttachmentPayload]) async throws -> HaoclawChatSendResponse
    {
        ...
        struct Params: Codable {
            ...
            var message: String
            var thinking: String
            var attachments: [HaoclawChatAttachmentPayload]?
            var timeoutMs: Int
        ...
        do {
            ...
            let decoded = try JSONDecoder().decode(HaoclawChatSendResponse.self, from: res)
    ...
    func requestHealth(timeoutMs: Int) async throws -> Bool {
        ...
        return (try? JSONDecoder().decode(HaoclawGatewayHealthOK.self, from: res))?.ok ?? true
    }
                ...
                for await evt in stream {
                    ...
                    case "health":
                        ...
                        let ok = (try? GatewayPayloadDecoding.decode(
                            payload,
                            as: HaoclawGatewayHealthOK.self))?.ok ?? true
                        continuation.yield(.health(ok: ok))
                    case "chat":
                        guard let payload = evt.payload else { break }
                        if let chatPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: HaoclawChatEventPayload.self)
                        {
