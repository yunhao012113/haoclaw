...
final class ContactsService: ContactsServicing {
    ...
    func search(params: HaoclawContactsSearchParams) async throws -> HaoclawContactsSearchPayload {
        ...
        return HaoclawContactsSearchPayload(contacts: payload)
    }
    ...
    func add(params: HaoclawContactsAddParams) async throws -> HaoclawContactsAddPayload {
        ...
        if !phoneNumbers.isEmpty || !emails.isEmpty {
            if let existing = try Self.findExistingContact(
                ...
                emails: emails)
            {
                return HaoclawContactsAddPayload(contact: Self.payload(from: existing))
            }
        }
        ...
        return HaoclawContactsAddPayload(contact: Self.payload(from: persisted))
    }
    ...
    private static func payload(from contact: CNContact) -> HaoclawContactPayload {
        HaoclawContactPayload(
