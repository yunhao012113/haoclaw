...
extension NodeAppModel {
    static func normalizeWatchNotifyParams(_ params: HaoclawWatchNotifyParams) -> HaoclawWatchNotifyParams {
    ...
    static func normalizeWatchActions(
        _ actions: [HaoclawWatchAction]?,
        kind: String?,
        promptId: String?) -> [HaoclawWatchAction]
    {
        let provided = (actions ?? []).compactMap { action -> HaoclawWatchAction? in
            let id = action.id.trimmingCharacters(in: .whitespacesAndNewlines)
            let label = action.label.trimmingCharacters(in: .whitespacesAndNewlines)
            guard !id.isEmpty, !label.isEmpty else { return nil }
            return HaoclawWatchAction(
                id: id,
                label: label,
        ...
        if normalizedKind.contains("approval") || normalizedKind.contains("approve") {
            return [
                HaoclawWatchAction(id: "approve", label: "Approve"),
                HaoclawWatchAction(id: "decline", label: "Decline", style: "destructive"),
                HaoclawWatchAction(id: "open_phone", label: "Open iPhone"),
                HaoclawWatchAction(id: "escalate", label: "Escalate"),
            ]
        }

        return [
            HaoclawWatchAction(id: "done", label: "Done"),
            HaoclawWatchAction(id: "snooze_10m", label: "Snooze 10m"),
            HaoclawWatchAction(id: "open_phone", label: "Open iPhone"),
            HaoclawWatchAction(id: "escalate", label: "Escalate"),
        ]
    }
    ...
    static func normalizedWatchRisk(
        _ risk: HaoclawWatchRisk?,
        priority: HaoclawNotificationPriority?) -> HaoclawWatchRisk?
    {
    ...
    static func normalizedWatchPriority(
        _ priority: HaoclawNotificationPriority?,
        risk: HaoclawWatchRisk?) -> HaoclawNotificationPriority?
    {
