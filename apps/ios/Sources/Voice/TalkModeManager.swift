...
final class TalkModeManager: NSObject {
    ...
    private static let redactedConfigSentinel = "__HAOCLAW_REDACTED__"
    ...
    private var pttCompletion: CheckedContinuation<HaoclawTalkPTTStopPayload, Never>?
    ...
    private let logger = Logger(subsystem: "ai.haoclaw", category: "TalkMode")
    ...
    func stop() {
        ...
        if pendingPTT {
            let payload = HaoclawTalkPTTStopPayload(
    ...
    func beginPushToTalk() async throws -> HaoclawTalkPTTStartPayload {
        guard self.gatewayConnected else {
            self.statusText = "Offline"
        ...
        if self.isPushToTalkActive, let captureId = self.activePTTCaptureId {
            return HaoclawTalkPTTStartPayload(captureId: captureId)
        }
    ...
    func endPushToTalk() async -> HaoclawTalkPTTStopPayload {
    ...
    func runPushToTalkOnce(maxDurationSeconds: TimeInterval = 12) async throws -> HaoclawTalkPTTStopPayload {
        ...
        if self.isPushToTalkActive {
            let captureId = self.activePTTCaptureId ?? UUID().uuidString
            return HaoclawTalkPTTStopPayload(
                captureId: captureId,
                transcript: nil,
                status: "busy")
        }
    ...
    func cancelPushToTalk() async -> HaoclawTalkPTTStopPayload {
        let captureId = self.activePTTCaptureId ?? UUID().uuidString
        guard self.isPushToTalkActive else {
            let payload = HaoclawTalkPTTStopPayload(
                captureId: captureId,
    ...
    private func finishPTTOnce(_ payload: HaoclawTalkPTTStopPayload) {
    ...
    private func streamAssistant(runId: String, gateway: GatewayNodeSession) async {
        ...
        for await evt in stream {
            ...
            guard let agentEvent = try? GatewayPayloadDecoding.decode(
                payload,
                as: HaoclawAgentEventPayload.self
            ) else {
                continue
            }
