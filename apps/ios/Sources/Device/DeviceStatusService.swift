...
final class DeviceStatusService: DeviceStatusServicing {
    ...
    func status() async throws -> HaoclawDeviceStatusPayload {
        ...
        return HaoclawDeviceStatusPayload(
            battery: battery,
            thermal: thermal,
            storage: storage,
            network: network,
    ...
    func info() -> HaoclawDeviceInfoPayload {
        ...
        return HaoclawDeviceInfoPayload(
    ...
    private func batteryStatus() -> HaoclawBatteryStatusPayload {
        let device = UIDevice.current
        device.isBatteryMonitoringEnabled = true
        let level = device.batteryLevel >= 0 ? Double(device.batteryLevel) : nil
        let state: HaoclawBatteryState = switch device.batteryState {
        ...
        return HaoclawBatteryStatusPayload(
            level: level,
            state: state,
            lowPowerModeEnabled: ProcessInfo.processInfo.isLowPowerModeEnabled)
    }

    private func thermalStatus() -> HaoclawThermalStatusPayload {
        let state: HaoclawThermalState = switch ProcessInfo.processInfo.thermalState {
        ...
        return HaoclawThermalStatusPayload(state: state)
    }

    private func storageStatus() -> HaoclawStorageStatusPayload {
        let attrs = (try? FileManager.default.attributesOfFileSystem(forPath: NSHomeDirectory())) ?? [:]
        let total = (attrs[.systemSize] as? NSNumber)?.int64Value ?? 0
        let free = (attrs[.systemFreeSize] as? NSNumber)?.int64Value ?? 0
        let used = max(0, total - free)
        return HaoclawStorageStatusPayload(totalBytes: total, freeBytes: free, usedBytes: used)
    }
