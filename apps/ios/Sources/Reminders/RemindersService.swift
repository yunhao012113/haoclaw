...
final class RemindersService: RemindersServicing {
    func list(params: HaoclawRemindersListParams) async throws -> HaoclawRemindersListPayload {
        ...
        let payload: [HaoclawReminderPayload] = try await withCheckedThrowingContinuation { cont in
        ...
        return HaoclawRemindersListPayload(reminders: payload)
    }
    ...
    func add(params: HaoclawRemindersAddParams) async throws -> HaoclawRemindersAddPayload {
        ...
        let formatter = ISO8601DateFormatter()
        let due = reminder.dueDateComponents.flatMap { Calendar.current.date(from: $0) }
        let payload = HaoclawReminderPayload(
            identifier: reminder.calendarItemIdentifier,
            title: reminder.title,
            dueISO: due.map { formatter.string(from: $0) },
            completed: reminder.isCompleted,
        ...
        return HaoclawRemindersAddPayload(reminder: payload)
    }
