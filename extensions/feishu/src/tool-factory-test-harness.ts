import type { AnyAgentTool, HaoclawPluginApi } from "haoclaw/plugin-sdk/feishu";
...
export function createToolFactoryHarness(cfg: HaoclawPluginApi["config"]) {
  const registered: RegisteredTool[] = [];

  const api: Pick<HaoclawPluginApi, "config" | "logger" | "registerTool"> = {
    config: cfg,
    logger: {
      info: () => {},
      warn: () => {},
