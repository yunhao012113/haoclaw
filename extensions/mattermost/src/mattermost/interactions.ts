...
const SIGNED_CHANNEL_ID_CONTEXT_KEY = "__haoclaw_channel_id";
...
type InteractionCallbackConfig = Pick<HaoclawConfig, "gateway" | "channels"> & {
  interactions?: {
...
function deriveInteractionSecret(botToken: string): string {
  return createHmac("sha256", "haoclaw-mattermost-interactions").update(botToken).digest("hex");
}
