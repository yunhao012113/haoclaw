...
import type { HaoclawPluginApi } from "haoclaw/plugin-sdk/diffs";
...
describe("diffs tool", () => {
  ...
  beforeEach(async () => {
    rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "haoclaw-diffs-tool-"));
...
function createApi(): HaoclawPluginApi {
  return {
    id: "diffs",
    name: "Diffs",
    description: "Diffs",
    ...
    runtime: {} as HaoclawPluginApi["runtime"],
    logger: {
      info() {},
      warn() {},
      error() {},
