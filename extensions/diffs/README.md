...
Read-only diff viewer plugin for **Haoclaw** agents.
...
Set plugin-wide defaults in `~/.haoclaw/haoclaw.json`:
...
Before:
Haoclaw supports plugins.

After:
Haoclaw supports plugins and hosted diff views.
```
...
- Artifacts are ephemeral and stored in the plugin temp subfolder (`$TMPDIR/haoclaw-diffs`).
