...
import type { HaoclawPluginApi } from "haoclaw/plugin-sdk/device-pair";
import {
...
} from "haoclaw/plugin-sdk/device-pair";
...
function resolveGatewayPort(cfg: HaoclawPluginApi["config"]): number {
  const envPort =
    parsePositiveInteger(process.env.HAOCLAW_GATEWAY_PORT?.trim()) ??
...
function resolveAuth(cfg: HaoclawPluginApi["config"]): ResolveAuthResult {
...
async function resolveGatewayUrl(api: HaoclawPluginApi): Promise<ResolveUrlResult> {
  ...
  api.registerCommand({
    ...
    handler: async (ctx) => {
      ...
      if (action === "qr") {
        ...
        if (channel === "telegram" && target) {
          try {
            ...
            if (send) {
              await send(
                target,
                ["Scan this QR code with the Haoclaw iOS app:", "", "```", qrAscii, "```"].join(
                  "\n",
                ),
                {
        ...
        return {
          text: [
            "Scan this QR code with the Haoclaw iOS app:",
            "",
            "```",
            qrAscii,
            "```",
