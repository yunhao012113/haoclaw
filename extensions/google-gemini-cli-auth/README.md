# Google Gemini CLI Auth (Haoclaw plugin)
...
```bash
haoclaw plugins enable google-gemini-cli-auth
```
...
```bash
haoclaw models auth login --provider google-gemini-cli --set-default
```
...
- `HAOCLAW_GEMINI_OAUTH_CLIENT_ID` / `GEMINI_CLI_OAUTH_CLIENT_ID`
- `HAOCLAW_GEMINI_OAUTH_CLIENT_SECRET` / `GEMINI_CLI_OAUTH_CLIENT_SECRET`
