...
import type { HaoclawPluginApi, HaoclawPluginService } from "haoclaw/plugin-sdk/phone-control";
...
function normalizeDenyList(cfg: HaoclawPluginApi["config"]): string[] {
  return uniqSorted([...(cfg.gateway?.nodes?.denyCommands ?? [])]);
}

function normalizeAllowList(cfg: HaoclawPluginApi["config"]): string[] {
...
export default function register(api: HaoclawPluginApi) {
  ...
  const timerService: HaoclawPluginService = {
