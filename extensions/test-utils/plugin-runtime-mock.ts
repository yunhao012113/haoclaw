import type { PluginRuntime } from "haoclaw/plugin-sdk/test-utils";
import { removeAckReactionAfterReply, shouldAckReaction } from "haoclaw/plugin-sdk/test-utils";
...
export function createPluginRuntimeMock(overrides: DeepPartial<PluginRuntime> = {}): PluginRuntime {
  const base: PluginRuntime = {
    ...
    state: {
      resolveStateDir: vi.fn(() => "/tmp/haoclaw"),
    },
    subagent: {
      run: vi.fn(),
      waitForRun: vi.fn(),
