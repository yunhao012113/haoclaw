...
export function extractMSTeamsPollVote(
  ...
  const pollId =
    readNestedString(value, ["haoclawPollId"]) ??
    readNestedString(value, ["pollId"]) ??
    readNestedString(value, ["haoclaw", "pollId"]) ??
    readNestedString(value, ["haoclaw", "poll", "id"]) ??
    readNestedString(value, ["data", "haoclawPollId"]) ??
    readNestedString(value, ["data", "pollId"]) ??
    readNestedString(value, ["data", "haoclaw", "pollId"]);
  if (!pollId) {
    return null;
  }
    ...
    actions: [
      {
        ...
        title: "Vote",
        data: {
          haoclawPollId: pollId,
          pollId,
        },
        msteams: {
          type: "messageBack",
          text: "haoclaw poll vote",
          displayText: "Vote recorded",
          value: { haoclawPollId: pollId, pollId },
        },
      },
    ],
  };
