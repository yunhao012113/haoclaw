...
function resolveHaoclawRoot(): string {
  if (coreRootCache) {
    return coreRootCache;
  }
  const override = process.env.HAOCLAW_ROOT?.trim();
  if (override) {
  ...
  throw new Error("Unable to resolve core root. Set HAOCLAW_ROOT to the package root.");
}
...
async function importCoreExtensionAPI(): Promise<{
  ...
  const distPath = path.join(resolveHaoclawRoot(), "dist", "extensionAPI.js");
