...
metadata: { "haoclaw": { "emoji": "🪶", "homepage": "https://www.prose.md" } }
---
...
- **Task tool** in the upstream spec == Haoclaw `sessions_spawn`
- **File I/O** == Haoclaw `read`/`write`
- **Remote fetch** == Haoclaw `web_fetch` (or `exec` with curl when POST is required)
