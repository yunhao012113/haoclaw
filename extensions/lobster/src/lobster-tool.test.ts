...
import type { HaoclawPluginApi, HaoclawPluginToolContext } from "haoclaw/plugin-sdk/lobster";
...
function fakeApi(overrides: Partial<HaoclawPluginApi> = {}): HaoclawPluginApi {
  return {
    id: "lobster",
    name: "lobster",
    source: "test",
...
function fakeCtx(overrides: Partial<HaoclawPluginToolContext> = {}): HaoclawPluginToolContext {
  return {
    config: {},
    workspaceDir: "/tmp",
    agentDir: "/tmp",
...
describe("lobster plugin tool", () => {
  ...
  beforeAll(async () => {
    ...
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "haoclaw-lobster-plugin-"));
  });
  ...
  it("can be gated off in sandboxed contexts", async () => {
    const api = fakeApi();
    const factoryTool = (ctx: HaoclawPluginToolContext) => {
      if (ctx.sandboxed) {
        return null;
      }
