/**
 * Twitch channel plugin for Haoclaw.
 *
...
import { buildChannelConfigSchema } from "haoclaw/plugin-sdk/twitch";
 ...
 * for Haoclaw. Supports message sending, receiving, access control, and
 * status monitoring.
 */
export const twitchPlugin: ChannelPlugin<TwitchAccountConfig> = {
  ...
  config: {
    ...
    listAccountIds: (cfg: HaoclawConfig): string[] => listAccountIds(cfg),

    /** Resolve an account config by ID */
    resolveAccount: (cfg: HaoclawConfig, accountId?: string | null): TwitchAccountConfig => {
    ...
    isConfigured: (_account: unknown, cfg: HaoclawConfig): boolean => {
