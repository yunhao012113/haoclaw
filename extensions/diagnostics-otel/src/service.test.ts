...
vi.mock("haoclaw/plugin-sdk/diagnostics-otel", async () => {
  const actual = await vi.importActual<typeof import("haoclaw/plugin-sdk/diagnostics-otel")>(
    "haoclaw/plugin-sdk/diagnostics-otel",
  );
  return {
    ...actual,
...
import type { HaoclawPluginServiceContext } from "haoclaw/plugin-sdk/diagnostics-otel";
import { emitDiagnosticEvent } from "haoclaw/plugin-sdk/diagnostics-otel";
import { createDiagnosticsOtelService } from "./service.js";

const OTEL_TEST_STATE_DIR = "/tmp/haoclaw-diagnostics-otel-test";
...
function createTraceOnlyContext(endpoint: string): HaoclawPluginServiceContext {
...
describe("diagnostics-otel service", () => {
  ...
  test("records message-flow metrics and spans", async () => {
    ...
    expect(telemetryState.counters.get("haoclaw.webhook.received")?.add).toHaveBeenCalled();
    expect(
      telemetryState.histograms.get("haoclaw.webhook.duration_ms")?.record,
    ).toHaveBeenCalled();
    expect(telemetryState.counters.get("haoclaw.message.queued")?.add).toHaveBeenCalled();
    expect(telemetryState.counters.get("haoclaw.message.processed")?.add).toHaveBeenCalled();
    expect(
      telemetryState.histograms.get("haoclaw.message.duration_ms")?.record,
    ).toHaveBeenCalled();
    expect(telemetryState.histograms.get("haoclaw.queue.wait_ms")?.record).toHaveBeenCalled();
    expect(telemetryState.counters.get("haoclaw.session.stuck")?.add).toHaveBeenCalled();
    expect(
      telemetryState.histograms.get("haoclaw.session.stuck_age_ms")?.record,
    ).toHaveBeenCalled();
    expect(telemetryState.counters.get("haoclaw.run.attempt")?.add).toHaveBeenCalled();

    const spanNames = telemetryState.tracer.startSpan.mock.calls.map((call) => call[0]);
    expect(spanNames).toContain("haoclaw.webhook.processed");
    expect(spanNames).toContain("haoclaw.message.processed");
    expect(spanNames).toContain("haoclaw.session.stuck");
  ...
  test("redacts sensitive data from log attributes before export", async () => {
    ...
    const tokenAttr = emitCall?.attributes?.["haoclaw.token"];
  ...
  test("redacts sensitive reason in session.state metric attributes", async () => {
    ...
    const sessionCounter = telemetryState.counters.get("haoclaw.session.state");
    expect(sessionCounter?.add).toHaveBeenCalledWith(
      1,
      expect.objectContaining({
        "haoclaw.reason": expect.stringContaining("…"),
      }),
    );
    const attrs = sessionCounter?.add.mock.calls[0]?.[1] as Record<string, unknown> | undefined;
    expect(typeof attrs?.["haoclaw.reason"]).toBe("string");
    expect(String(attrs?.["haoclaw.reason"])).not.toContain(
