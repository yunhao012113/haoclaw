...
import type { HaoclawConfig, PluginRuntime } from "haoclaw/plugin-sdk/bluebubbles";
...
function createConfig(overrides?: Record<string, unknown>): HaoclawConfig {
  return {
    channels: {
      bluebubbles: {
        ...overrides,
...
async function makeTempDir(): Promise<string> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "haoclaw-bb-media-"));
  tempDirs.push(dir);
  return dir;
}
