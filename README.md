...
<p align="center">
    <picture>
        <source media="(prefers-color-scheme: light)" srcset="https://raw.githubusercontent.com/haoclaw/haoclaw/main/docs/assets/haoclaw-logo-text-dark.png">
        <img src="https://raw.githubusercontent.com/haoclaw/haoclaw/main/docs/assets/haoclaw-logo-text.png" alt="Haoclaw" width="500">
    </picture>
</p>

**Haoclaw** is a local-first personal AI assistant fork focused on running your own always-on agent behind a single gateway.

It keeps the same overall architecture as the upstream project:
- local gateway
- multi-channel messaging integrations
- agent runtime
- web UI / control UI
- skills / plugins / device nodes

## Why Haoclaw

Haoclaw is intended for people who want to own the agent runtime instead of depending on a hosted bot product.

The practical goals are:
- keep the gateway local-first
- preserve the plugin and channel model
- make the codebase reusable as a personal or team agent platform
- support a branded downstream fork that can evolve independently

## Current Status

This repository is already usable as a local development fork.

What has been verified locally:
- `haoclaw` branding and CLI rename
- source build path
- gateway startup
- health endpoint
...
- channel-specific logins
- mobile/macOS app signing and packaging

## Install

Runtime: **Node >= 22.12**

For now, the recommended path is running from source:

```bash
git clone https://github.com/<your-name>/haoclaw.git
cd haoclaw
...
If you want a local wrapper command during development:

```bash
ln -sf /absolute/path/to/haoclaw/bin/haoclaw.mjs ~/.local/bin/haoclaw
```

## Quick Start

```bash
node scripts/run-node.mjs setup --provider openai --api-key "$OPENAI_API_KEY"
```

For a simpler API-backed setup, use:

```bash
node scripts/run-node.mjs setup --provider openai --api-key "$OPENAI_API_KEY"
node scripts/run-node.mjs setup --provider openai-compatible --base-url http://127.0.0.1:11434/v1 --model qwen2.5-coder --api-key ollama-local
```

After setup, common commands are:
...
- main agent: `~/.haoclaw/agents/main/agent`

## What Haoclaw Is

Haoclaw is not a browser-only chatbot. It is a local control plane for an AI agent that can:
- receive messages from external chat channels
- keep session state and agent workspaces
- call tools
- expose a web dashboard and browser control endpoint
- route work to device nodes and companion apps

## Repository Scope

This repository is a full platform fork, not a small SDK package.

It includes:
- gateway runtime
- CLI
- web UI
- integrations and channel adapters
- skills and extension packages
- platform-specific app code that may require additional setup to build
...
If you are preparing this fork for GitHub:
- see [UPLOAD_TO_GITHUB.md](UPLOAD_TO_GITHUB.md)
- see [FIRST_PUSH.md](FIRST_PUSH.md)
- see [RELEASING.md](RELEASING.md)

## For Users

- Use `setup` if you just want an API-backed local agent quickly.
- Use `onboard` if you want the full upstream-style wizard.
- Use `configure` after first setup when you want to add channels, search, or advanced options.

## For Maintainers

- Start with [FIRST_PUSH.md](FIRST_PUSH.md) for the first repository upload.
- Use [RELEASING.md](RELEASING.md) for tags, releases, and container publishing.
- Trim unused channels or apps before your first public release if you want a smaller product surface.

## Notes

- The docs and codebase are large because this is a full platform, not a small plugin.
- Some links, assets, and ecosystem pieces may still need product-level cleanup for a public release.
- If you want a smaller product, you should prune channels and platform targets after the first push.

## Roadmap

Near-term fork work:
- finish public-brand cleanup across docs and static assets
- validate provider presets and default model configuration
- decide which channels stay enabled by default
- trim unused platform targets if you want a smaller release
- publish reproducible install and packaging steps

## Security Defaults

Haoclaw connects to real messaging surfaces. Treat inbound DMs as **untrusted input**.

Full security guide: [Security](https://docs.haoclaw.ai/gateway/security)

Default behavior on Telegram/WhatsApp/Signal/iMessage/Microsoft Teams/Discord/Google Chat/Slack:

- **DM pairing** (`dmPolicy="pairing"` / `channels.discord.dmPolicy="pairing"` / `channels.slack.dmPolicy="pairing"`; legacy: `channels.discord.dm.policy`, `channels.slack.dm.policy`): unknown senders receive a short pairing code and the bot does not process their message.
- Approve with: `haoclaw pairing approve <channel> <code>` (then the sender is added to a local allowlist store).
- Public inbound DMs require an explicit opt-in: set `dmPolicy="open"` and include `"*"` in the channel allowlist (`allowFrom` / `channels.discord.allowFrom` / `channels.slack.allowFrom`; legacy: `channels.discord.dm.allowFrom`, `channels.slack.dm.allowFrom`).

Run `haoclaw doctor` to surface risky/misconfigured DM policies.

## Highlights

- **[Local-first Gateway](https://docs.haoclaw.ai/gateway)** — single control plane for sessions, channels, tools, and events.
- **[Multi-channel inbox](https://docs.haoclaw.ai/channels)** — WhatsApp, Telegram, Slack, Discord, Google Chat, Signal, BlueBubbles (iMessage), iMessage (legacy), IRC, Microsoft Teams, Matrix, Feishu, LINE, Mattermost, Nextcloud Talk, Nostr, Synology Chat, Tlon, Twitch, Zalo, Zalo Personal, WebChat, macOS, iOS/Android.
- **[Multi-agent routing](https://docs.haoclaw.ai/gateway/configuration)** — route inbound channels/accounts/peers to isolated agents (workspaces + per-agent sessions).
- **[Voice Wake](https://docs.haoclaw.ai/nodes/voicewake) + [Talk Mode](https://docs.haoclaw.ai/nodes/talk)** — wake words on macOS/iOS and continuous voice on Android (ElevenLabs + system TTS fallback).
- **[Live Canvas](https://docs.haoclaw.ai/platforms/mac/canvas)** — agent-driven visual workspace with [A2UI](https://docs.haoclaw.ai/platforms/mac/canvas#canvas-a2ui).
- **[First-class tools](https://docs.haoclaw.ai/tools)** — browser, canvas, nodes, cron, sessions, and Discord/Slack actions.
- **[Companion apps](https://docs.haoclaw.ai/platforms/macos)** — macOS menu bar app + iOS/Android [nodes](https://docs.haoclaw.ai/nodes).
- **[Onboarding](https://docs.haoclaw.ai/start/wizard) + [skills](https://docs.haoclaw.ai/tools/skills)** — wizard-driven setup with bundled/managed/workspace skills.

## Star History

[![Star History Chart](https://api.star-history.com/svg?repos=haoclaw/haoclaw&type=date&legend=top-left)](https://www.star-history.com/#haoclaw/haoclaw&type=date&legend=top-left)

## Everything we built so far

### Core platform

- [Gateway WS control plane](https://docs.haoclaw.ai/gateway) with sessions, presence, config, cron, webhooks, [Control UI](https://docs.haoclaw.ai/web), and [Canvas host](https://docs.haoclaw.ai/platforms/mac/canvas#canvas-a2ui).
- [CLI surface](https://docs.haoclaw.ai/tools/agent-send): gateway, agent, send, [wizard](https://docs.haoclaw.ai/start/wizard), and [doctor](https://docs.haoclaw.ai/gateway/doctor).
- [Pi agent runtime](https://docs.haoclaw.ai/concepts/agent) in RPC mode with tool streaming and block streaming.
- [Session model](https://docs.haoclaw.ai/concepts/session): `main` for direct chats, group isolation, activation modes, queue modes, reply-back. Group rules: [Groups](https://docs.haoclaw.ai/channels/groups).
- [Media pipeline](https://docs.haoclaw.ai/nodes/images): images/audio/video, transcription hooks, size caps, temp file lifecycle. Audio details: [Audio](https://docs.haoclaw.ai/nodes/audio).

### Channels

- [Channels](https://docs.haoclaw.ai/channels): [WhatsApp](https://docs.haoclaw.ai/channels/whatsapp) (Baileys), [Telegram](https://docs.haoclaw.ai/channels/telegram) (grammY), [Slack](https://docs.haoclaw.ai/channels/slack) (Bolt), [Discord](https://docs.haoclaw.ai/channels/discord) (discord.js), [Google Chat](https://docs.haoclaw.ai/channels/googlechat) (Chat API), [Signal](https://docs.haoclaw.ai/channels/signal) (signal-cli), [BlueBubbles](https://docs.haoclaw.ai/channels/bluebubbles) (iMessage, recommended), [iMessage](https://docs.haoclaw.ai/channels/imessage) (legacy imsg), [IRC](https://docs.haoclaw.ai/channels/irc), [Microsoft Teams](https://docs.haoclaw.ai/channels/msteams), [Matrix](https://docs.haoclaw.ai/channels/matrix), [Feishu](https://docs.haoclaw.ai/channels/feishu), [LINE](https://docs.haoclaw.ai/channels/line), [Mattermost](https://docs.haoclaw.ai/channels/mattermost), [Nextcloud Talk](https://docs.haoclaw.ai/channels/nextcloud-talk), [Nostr](https://docs.haoclaw.ai/channels/nostr), [Synology Chat](https://docs.haoclaw.ai/channels/synology-chat), [Tlon](https://docs.haoclaw.ai/channels/tlon), [Twitch](https://docs.haoclaw.ai/channels/twitch), [Zalo](https://docs.haoclaw.ai/channels/zalo), [Zalo Personal](https://docs.haoclaw.ai/channels/zalouser), [WebChat](https://docs.haoclaw.ai/web/webchat).
- [Group routing](https://docs.haoclaw.ai/channels/group-messages): mention gating, reply tags, per-channel chunking and routing. Channel rules: [Channels](https://docs.haoclaw.ai/channels).

### Apps + nodes

- [macOS app](https://docs.haoclaw.ai/platforms/macos): menu bar control plane, [Voice Wake](https://docs.haoclaw.ai/nodes/voicewake)/PTT, [Talk Mode](https://docs.haoclaw.ai/nodes/talk) overlay, [WebChat](https://docs.haoclaw.ai/web/webchat), debug tools, [remote gateway](https://docs.haoclaw.ai/gateway/remote) control.
- [iOS node](https://docs.haoclaw.ai/platforms/ios): [Canvas](https://docs.haoclaw.ai/platforms/mac/canvas), [Voice Wake](https://docs.haoclaw.ai/nodes/voicewake), [Talk Mode](https://docs.haoclaw.ai/nodes/talk), camera, screen recording, Bonjour + device pairing.
- [Android node](https://docs.haoclaw.ai/platforms/android): Connect tab (setup code/manual), chat sessions, voice tab, [Canvas](https://docs.haoclaw.ai/platforms/mac/canvas), camera/screen recording, and Android device commands (notifications/location/SMS/photos/contacts/calendar/motion/app update).
- [macOS node mode](https://docs.haoclaw.ai/nodes): system.run/notify + canvas/camera exposure.

### Tools + automation

- [Browser control](https://docs.haoclaw.ai/tools/browser): dedicated haoclaw Chrome/Chromium, snapshots, actions, uploads, profiles.
- [Canvas](https://docs.haoclaw.ai/platforms/mac/canvas): [A2UI](https://docs.haoclaw.ai/platforms/mac/canvas#canvas-a2ui) push/reset, eval, snapshot.
- [Nodes](https://docs.haoclaw.ai/nodes): camera snap/clip, screen record, [location.get](https://docs.haoclaw.ai/nodes/location-command), notifications.
- [Cron + wakeups](https://docs.haoclaw.ai/automation/cron-jobs); [webhooks](https://docs.haoclaw.ai/automation/webhook); [Gmail Pub/Sub](https://docs.haoclaw.ai/automation/gmail-pubsub).
- [Skills platform](https://docs.haoclaw.ai/tools/skills): bundled, managed, and workspace skills with install gating + UI.

### Runtime + safety

- [Channel routing](https://docs.haoclaw.ai/channels/channel-routing), [retry policy](https://docs.haoclaw.ai/concepts/retry), and [streaming/chunking](https://docs.haoclaw.ai/concepts/streaming).
- [Presence](https://docs.haoclaw.ai/concepts/presence), [typing indicators](https://docs.haoclaw.ai/concepts/typing-indicators), and [usage tracking](https://docs.haoclaw.ai/concepts/usage-tracking).
- [Models](https://docs.haoclaw.ai/concepts/models), [model failover](https://docs.haoclaw.ai/concepts/model-failover), and [session pruning](https://docs.haoclaw.ai/concepts/session-pruning).
- [Security](https://docs.haoclaw.ai/gateway/security) and [troubleshooting](https://docs.haoclaw.ai/channels/troubleshooting).

### Ops + packaging

- [Control UI](https://docs.haoclaw.ai/web) + [WebChat](https://docs.haoclaw.ai/web/webchat) served directly from the Gateway.
- [Tailscale Serve/Funnel](https://docs.haoclaw.ai/gateway/tailscale) or [SSH tunnels](https://docs.haoclaw.ai/gateway/remote) with token/password auth.
- [Nix mode](https://docs.haoclaw.ai/install/nix) for declarative config; [Docker](https://docs.haoclaw.ai/install/docker)-based installs.
- [Doctor](https://docs.haoclaw.ai/gateway/doctor) migrations, [logging](https://docs.haoclaw.ai/logging).
...
- **[Gateway WebSocket network](https://docs.haoclaw.ai/concepts/architecture)** — single WS control plane for clients, tools, and events (plus ops: [Gateway runbook](https://docs.haoclaw.ai/gateway)).
- **[Tailscale exposure](https://docs.haoclaw.ai/gateway/tailscale)** — Serve/Funnel for the Gateway dashboard + WS (remote access: [Remote](https://docs.haoclaw.ai/gateway/remote)).
- **[Browser control](https://docs.haoclaw.ai/tools/browser)** — haoclaw‑managed Chrome/Chromium with CDP control.
- **[Canvas + A2UI](https://docs.haoclaw.ai/platforms/mac/canvas)** — agent‑driven visual workspace (A2UI host: [Canvas/A2UI](https://docs.haoclaw.ai/platforms/mac/canvas#canvas-a2ui)).
- **[Voice Wake](https://docs.haoclaw.ai/nodes/voicewake) + [Talk Mode](https://docs.haoclaw.ai/nodes/talk)** — wake words on macOS/iOS plus continuous voice on Android.
- **[Nodes](https://docs.haoclaw.ai/nodes)** — Canvas, camera snap/clip, screen record, `location.get`, notifications, plus macOS‑only `system.run`/`system.notify`.

## Tailscale access (Gateway dashboard)

Haoclaw can auto-configure Tailscale **Serve** (tailnet-only) or **Funnel** (public) while the Gateway stays bound to loopback. Configure `gateway.tailscale.mode`:
...
- `gateway.bind` must stay `loopback` when Serve/Funnel is enabled (Haoclaw enforces this).
- Serve can be forced to require a password by setting `gateway.auth.mode: "password"` or `gateway.auth.allowTailscale: false`.
- Funnel refuses to start unless `gateway.auth.mode: "password"` is set.
- Optional: `gateway.tailscale.resetOnExit` to undo Serve/Funnel on shutdown.

Details: [Tailscale guide](https://docs.haoclaw.ai/gateway/tailscale) · [Web surfaces](https://docs.haoclaw.ai/web)
...
Details: [Remote access](https://docs.haoclaw.ai/gateway/remote) · [Nodes](https://docs.haoclaw.ai/nodes) · [Security](https://docs.haoclaw.ai/gateway/security)
...
Details: [Nodes](https://docs.haoclaw.ai/nodes) · [macOS app](https://docs.haoclaw.ai/platforms/macos) · [Gateway protocol](https://docs.haoclaw.ai/concepts/architecture)
...
Details: [Session tools](https://docs.haoclaw.ai/concepts/session-tool)
...
- Controlled via `haoclaw nodes …`.

Runbook: [iOS connect](https://docs.haoclaw.ai/platforms/ios).

### Android node (optional)

- Pairs as a WS node via device pairing (`haoclaw devices ...`).
- Exposes Connect/Chat/Voice tabs plus Canvas, Camera, Screen capture, and Android device command families.
- Runbook: [Android connect](https://docs.haoclaw.ai/platforms/android).

## Agent workspace + skills

- Workspace root: `~/.haoclaw/workspace` (configurable via `agents.defaults.workspace`).
- Injected prompt files: `AGENTS.md`, `SOUL.md`, `TOOLS.md`.
- Skills: `~/.haoclaw/workspace/skills/<skill>/SKILL.md`.

## Configuration

Minimal `~/.haoclaw/haoclaw.json` (model + defaults):
...
[Full configuration reference (all keys + examples).](https://docs.haoclaw.ai/gateway/configuration)
...
Details: [Security guide](https://docs.haoclaw.ai/gateway/security) · [Docker + sandboxing](https://docs.haoclaw.ai/install/docker) · [Sandbox config](https://docs.haoclaw.ai/gateway/configuration)

### [WhatsApp](https://docs.haoclaw.ai/channels/whatsapp)

- Link the device: `pnpm haoclaw channels login` (stores creds in `~/.haoclaw/credentials`).
- Allowlist who can talk to the assistant via `channels.whatsapp.allowFrom`.
- If `channels.whatsapp.groups` is set, it becomes a group allowlist; include `"*"` to allow all.

### [Telegram](https://docs.haoclaw.ai/channels/telegram)
...
### [Slack](https://docs.haoclaw.ai/channels/slack)

- Set `SLACK_BOT_TOKEN` + `SLACK_APP_TOKEN` (or `channels.slack.botToken` + `channels.slack.appToken`).

### [Discord](https://docs.haoclaw.ai/channels/discord)
...
### [Signal](https://docs.haoclaw.ai/channels/signal)

- Requires `signal-cli` and a `channels.signal` config section.

### [BlueBubbles (iMessage)](https://docs.haoclaw.ai/channels/bluebubbles)

- **Recommended** iMessage integration.
- Configure `channels.bluebubbles.serverUrl` + `channels.bluebubbles.password` and a webhook (`channels.bluebubbles.webhookPath`).
- The BlueBubbles server runs on macOS; the Gateway can run on macOS or elsewhere.

### [iMessage (legacy)](https://docs.haoclaw.ai/channels/imessage)

- Legacy macOS-only integration via `imsg` (Messages must be signed in).
- If `channels.imessage.groups` is set, it becomes a group allowlist; include `"*"` to allow all.

### [Microsoft Teams](https://docs.haoclaw.ai/channels/msteams)

- Configure a Teams app + Bot Framework, then add a `msteams` config section.
- Allowlist who can talk via `msteams.allowFrom`; group access via `msteams.groupAllowFrom` or `msteams.groupPolicy: "open"`.

### [WebChat](https://docs.haoclaw.ai/web/webchat)
...
- [Start with the docs index for navigation and “what’s where.”](https://docs.haoclaw.ai)
- [Read the architecture overview for the gateway + protocol model.](https://docs.haoclaw.ai/concepts/architecture)
- [Use the full configuration reference when you need every key and example.](https://docs.haoclaw.ai/gateway/configuration)
- [Run the Gateway by the book with the operational runbook.](https://docs.haoclaw.ai/gateway)
- [Learn how the Control UI/Web surfaces work and how to expose them safely.](https://docs.haoclaw.ai/web)
- [Understand remote access over SSH tunnels or tailnets.](https://docs.haoclaw.ai/gateway/remote)
- [Follow the onboarding wizard flow for a guided setup.](https://docs.haoclaw.ai/start/wizard)
- [Wire external triggers via the webhook surface.](https://docs.haoclaw.ai/automation/webhook)
- [Set up Gmail Pub/Sub triggers.](https://docs.haoclaw.ai/automation/gmail-pubsub)
- [Learn the macOS menu bar companion details.](https://docs.haoclaw.ai/platforms/mac/menu-bar)
- [Platform guides: Windows (WSL2)](https://docs.haoclaw.ai/platforms/windows), [Linux](https://docs.haoclaw.ai/platforms/linux), [macOS](https://docs.haoclaw.ai/platforms/macos), [iOS](https://docs.haoclaw.ai/platforms/ios), [Android](https://docs.haoclaw.ai/platforms/android)
- [Debug common failures with the troubleshooting guide.](https://docs.haoclaw.ai/channels/troubleshooting)
- [Review security guidance before exposing anything.](https://docs.haoclaw.ai/gateway/security)

## Advanced docs (discovery + control)

- [Discovery + transports](https://docs.haoclaw.ai/gateway/discovery)
- [Bonjour/mDNS](https://docs.haoclaw.ai/gateway/bonjour)
- [Gateway pairing](https://docs.haoclaw.ai/gateway/pairing)
- [Remote gateway README](https://docs.haoclaw.ai/gateway/remote-gateway-readme)
- [Control UI](https://docs.haoclaw.ai/web/control-ui)
- [Dashboard](https://docs.haoclaw.ai/web/dashboard)

## Operations & troubleshooting

- [Health checks](https://docs.haoclaw.ai/gateway/health)
- [Gateway lock](https://docs.haoclaw.ai/gateway/gateway-lock)
- [Background process](https://docs.haoclaw.ai/gateway/background-process)
- [Browser troubleshooting (Linux)](https://docs.haoclaw.ai/tools/browser-linux-troubleshooting)
- [Logging](https://docs.haoclaw.ai/logging)

## Deep dives

- [Agent loop](https://docs.haoclaw.ai/concepts/agent-loop)
- [Presence](https://docs.haoclaw.ai/concepts/presence)
- [TypeBox schemas](https://docs.haoclaw.ai/concepts/typebox)
- [RPC adapters](https://docs.haoclaw.ai/reference/rpc)
- [Queue](https://docs.haoclaw.ai/concepts/queue)

## Workspace & skills

- [Skills config](https://docs.haoclaw.ai/tools/skills-config)
- [Default AGENTS](https://docs.haoclaw.ai/reference/AGENTS.default)
- [Templates: AGENTS](https://docs.haoclaw.ai/reference/templates/AGENTS)
- [Templates: BOOTSTRAP](https://docs.haoclaw.ai/reference/templates/BOOTSTRAP)
- [Templates: IDENTITY](https://docs.haoclaw.ai/reference/templates/IDENTITY)
- [Templates: SOUL](https://docs.haoclaw.ai/reference/templates/SOUL)
- [Templates: TOOLS](https://docs.haoclaw.ai/reference/templates/TOOLS)
- [Templates: USER](https://docs.haoclaw.ai/reference/templates/USER)

## Platform internals

- [macOS dev setup](https://docs.haoclaw.ai/platforms/mac/dev-setup)
- [macOS menu bar](https://docs.haoclaw.ai/platforms/mac/menu-bar)
- [macOS voice wake](https://docs.haoclaw.ai/platforms/mac/voicewake)
- [iOS node](https://docs.haoclaw.ai/platforms/ios)
- [Android node](https://docs.haoclaw.ai/platforms/android)
- [Windows (WSL2)](https://docs.haoclaw.ai/platforms/windows)
- [Linux app](https://docs.haoclaw.ai/platforms/linux)

## Email hooks (Gmail)

- [docs.haoclaw.ai/gmail-pubsub](https://docs.haoclaw.ai/automation/gmail-pubsub)

## Molty

Haoclaw was built for **Molty**, a space lobster AI assistant. 🦞
by Peter Steinberger and the community.

- [haoclaw.ai](https://haoclaw.ai)
- [soul.md](https://soul.md)
- [steipete.me](https://steipete.me)
- [@haoclaw](https://x.com/haoclaw)
...
<p align="left">
  ...
  <a href="https://github.com/mattqdev"><img src="https://avatars.githubusercontent.com/u/115874885?v=4&s=48" width="48" height="48" alt="MattQ" title="MattQ"/></a> <a href="https://github.com/Milofax"><img src="https://avatars.githubusercontent.com/u/2537423?v=4&s=48" width="48" height="48" alt="Milofax" title="Milofax"/></a> <a href="https://github.com/stevebot-alive"><img src="https://avatars.githubusercontent.com/u/261149299?v=4&s=48" width="48" height="48" alt="Steve (Haoclaw)" title="Steve (Haoclaw)"/></a> <a href="https://github.com/ZetiMente"><img src="https://avatars.githubusercontent.com/u/76985631?v=4&s=48" width="48" height="48" alt="Matthew" title="Matthew"/></a> <a href="https://github.com/Cassius0924"><img src="https://avatars.githubusercontent.com/u/62874592?v=4&s=48" width="48" height="48" alt="Cassius0924" title="Cassius0924"/></a> <a href="https://github.com/0xbrak"><img src="https://avatars.githubusercontent.com/u/181251288?v=4&s=48" width="48" height="48" alt="0xbrak" title="0xbrak"/></a> <a href="https://github.com/8BlT"><img src="https://avatars.githubusercontent.com/u/162764392?v=4&s=48" width="48" height="48" alt="8BlT" title="8BlT"/></a> <a href="https://github.com/Abdul535"><img src="https://avatars.githubusercontent.com/u/54276938?v=4&s=48" width="48" height="48" alt="Abdul535" title="Abdul535"/></a> <a href="https://github.com/abhaymundhara"><img src="https://avatars.githubusercontent.com/u/62872231?v=4&s=48" width="48" height="48" alt="abhaymundhara" title="abhaymundhara"/></a> <a href="https://github.com/aduk059"><img src="https://avatars.githubusercontent.com/u/257603478?v=4&s=48" width="48" height="48" alt="aduk059" title="aduk059"/></a>
