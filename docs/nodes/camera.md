...
Haoclaw supports **camera capture** for agent workflows:
...
```bash
haoclaw nodes camera snap --node <id>               # default: both front + back (2 MEDIA lines)
haoclaw nodes camera snap --node <id> --facing front
haoclaw nodes camera clip --node <id> --duration 3000
haoclaw nodes camera clip --node <id> --no-audio
```
...
- **Settings → General → Allow Camera** (`haoclaw.cameraEnabled`)
  - Default: **off**
  - When off: camera requests return “Camera disabled by user”.

### CLI helper (node invoke)

Use the main `haoclaw` CLI to invoke camera commands on the macOS node.

Examples:

```bash
haoclaw nodes camera list --node <id>            # list camera ids
haoclaw nodes camera snap --node <id>            # prints MEDIA:<path>
haoclaw nodes camera snap --node <id> --max-width 1280
haoclaw nodes camera snap --node <id> --delay-ms 2000
haoclaw nodes camera snap --node <id> --device-id <id>
haoclaw nodes camera clip --node <id> --duration 10s          # prints MEDIA:<path>
haoclaw nodes camera clip --node <id> --duration-ms 3000      # prints MEDIA:<path> (legacy flag)
haoclaw nodes camera clip --node <id> --device-id <id>
haoclaw nodes camera clip --node <id> --no-audio
```

Notes:

- `haoclaw nodes camera snap` defaults to `maxWidth=1600` unless overridden.
...
```bash
haoclaw nodes screen record --node <id> --duration 10s --fps 15   # prints MEDIA:<path>
```
