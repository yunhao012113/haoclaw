...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in Haoclaw
...
Running Haoclaw via Node with `tsx` fails at startup with:

```
[haoclaw] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `haoclaw status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch haoclaw.mjs status
  ```

- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node haoclaw.mjs status` works on Node 25.
