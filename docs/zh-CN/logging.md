...
你可以在 `~/.haoclaw/haoclaw.json` 中覆盖此设置：
...
所有日志配置都在 `~/.haoclaw/haoclaw.json` 的 `logging` 下。
...
- Haoclaw 目前通过 **OTLP/HTTP（protobuf）** 导出。
...
- 你也可以使用 `haoclaw plugins enable diagnostics-otel` 启用插件。
...
- `haoclaw.tokens`（计数器，属性：`haoclaw.token`、`haoclaw.channel`、`haoclaw.provider`、`haoclaw.model`）
- `haoclaw.cost.usd`（计数器，属性：`haoclaw.channel`、`haoclaw.provider`、`haoclaw.model`）
- `haoclaw.run.duration_ms`（直方图，属性：`haoclaw.channel`、`haoclaw.provider`、`haoclaw.model`）
- `haoclaw.context.tokens`（直方图，属性：`haoclaw.context`、`haoclaw.channel`、`haoclaw.provider`、`haoclaw.model`）

消息流：

- `haoclaw.webhook.received`（计数器，属性：`haoclaw.channel`、`haoclaw.webhook`）
- `haoclaw.webhook.error`（计数器，属性：`haoclaw.channel`、`haoclaw.webhook`）
- `haoclaw.webhook.duration_ms`（直方图，属性：`haoclaw.channel`、`haoclaw.webhook`）
- `haoclaw.message.queued`（计数器，属性：`haoclaw.channel`、`haoclaw.source`）
- `haoclaw.message.processed`（计数器，属性：`haoclaw.channel`、`haoclaw.outcome`）
- `haoclaw.message.duration_ms`（直方图，属性：`haoclaw.channel`、`haoclaw.outcome`）

队列 + 会话：

- `haoclaw.queue.lane.enqueue`（计数器，属性：`haoclaw.lane`）
- `haoclaw.queue.lane.dequeue`（计数器，属性：`haoclaw.lane`）
- `haoclaw.queue.depth`（直方图，属性：`haoclaw.lane` 或 `haoclaw.channel=heartbeat`）
- `haoclaw.queue.wait_ms`（直方图，属性：`haoclaw.lane`）
- `haoclaw.session.state`（计数器，属性：`haoclaw.state`、`haoclaw.reason`）
- `haoclaw.session.stuck`（计数器，属性：`haoclaw.state`）
- `haoclaw.session.stuck_age_ms`（直方图，属性：`haoclaw.state`）
- `haoclaw.run.attempt`（计数器，属性：`haoclaw.attempt`）
...
- `haoclaw.webhook.error`
  - `haoclaw.channel`、`haoclaw.webhook`、`haoclaw.chatId`、`haoclaw.error`
- `haoclaw.message.processed`
  - `haoclaw.channel`、`haoclaw.outcome`、`haoclaw.chatId`、`haoclaw.messageId`、`haoclaw.sessionKey`、`haoclaw.sessionId`、`haoclaw.reason`
- `haoclaw.session.stuck`
  - `haoclaw.state`、`haoclaw.ageMs`、`haoclaw.queueDepth`、`haoclaw.sessionKey`、`haoclaw.sessionId`
...
- **Gateway 网关无法访问？** 先运行 `haoclaw doctor`。
