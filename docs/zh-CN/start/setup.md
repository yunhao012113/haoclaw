...
summary: 设置指南：在保持最新的同时保持你的 Haoclaw 设置个性化
title: 设置
x-i18n:
...
- **个性化设置存放在仓库之外：** `~/.haoclaw/workspace`（工作区）+ `~/.haoclaw/haoclaw.json`（配置）。
...
- **配置：** `~/.haoclaw/haoclaw.json`（JSON/JSON5 格式）
- **工作区：** `~/.haoclaw/workspace`（Skills、提示、记忆；将其设为私有 git 仓库）
...
如果你还没有全局安装，通过 `pnpm haoclaw setup` 运行它。

## 稳定工作流（macOS 应用优先）

1. 安装并启动 **Haoclaw.app**（菜单栏）。
...
- 运行 `haoclaw setup`，然后 `haoclaw channels login`，然后手动启动 Gateway 网关（`haoclaw gateway`）。
...
- **配对允许列表**：`~/.haoclaw/credentials/<channel>-allowFrom.json`
- **模型认证配置文件**：`~/.haoclaw/agents/<agentId>/agent/auth-profiles.json`
- **旧版 OAuth 导入**：`~/.haoclaw/credentials/oauth.json`
  更多详情：[安全](/gateway/security#credential-storage-map)。

## 更新（不破坏你的设置）

- 将 `~/.haoclaw/workspace` 和 `~/.haoclaw/` 保持为"你的东西"；不要将个人提示/配置放入 `haoclaw` 仓库。
