...
## Octopus Workflow：带审批的确定性工作流

Octopus Workflow 是用于**多步骤工具管道**的工作流运行时，适用于需要确定性执行和明确审批的场景。当任务不只是单次智能体轮次，且你需要可恢复的带人工检查点的工作流时，使用它。

### 何时适合使用 Octopus Workflow
...
- **心跳/定时任务**决定*何时*运行。
- **Octopus Workflow** 定义运行开始后*执行哪些步骤*。

对于计划性工作流，使用定时任务或心跳触发一次具备确定性执行特征的智能体轮次。对于临时工作流，直接调用相关工具或智能体流程。

### 操作说明（来自代码）

- 对于需要稳定重复执行的任务，优先使用确定性的工具链路。
- 对外部副作用建议增加明确审批点。
- 如果你使用工具白名单，建议采用附加式配置，而不是一次性禁掉核心工具。
