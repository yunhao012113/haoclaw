...
read_when:
  - 你想要索引或搜索语义记忆
  - 你正在调试记忆可用性或索引问题
summary: "`haoclaw memory`（status/index/search）的 CLI 参考"
title: memory
x-i18n:
...
```bash
haoclaw memory status
haoclaw memory status --deep
haoclaw memory status --deep --index
haoclaw memory status --deep --index --verbose
haoclaw memory index
haoclaw memory index --verbose
haoclaw memory search "release checklist"
haoclaw memory status --agent main
haoclaw memory index --agent main --verbose
```
