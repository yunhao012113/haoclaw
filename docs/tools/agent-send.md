---
summary: "Direct `haoclaw agent` CLI runs (with optional delivery)"
read_when:
  - Adding or modifying the agent CLI entrypoint
title: "Agent Send"
---

# `haoclaw agent` (direct agent runs)

`haoclaw agent` runs a single agent turn without needing an inbound chat message.
...
- Optional delivery back to a channel with `--deliver` + `--channel` (target formats match `haoclaw message --target`).
...
```bash
haoclaw agent --to +15555550123 --message "status update"
...
haoclaw agent --to +15555550123 --message "Summon reply" --deliver
