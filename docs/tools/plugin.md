---
summary: "Haoclaw plugins/extensions: discovery, config, and safety"
read_when:
...
A plugin is just a **small code module** that extends Haoclaw with extra
features (commands, tools, and Gateway RPC).

Most of the time, you’ll use plugins when you want a feature that’s not built
into core Haoclaw yet (or you want to keep optional features out of your main
install).
...
```bash
haoclaw plugins install @haoclaw/voice-call
```
...
- Microsoft Teams is plugin-only as of 2026.1.15; install `@haoclaw/msteams` if you use Teams.
- Memory (Core) — bundled memory search plugin (enabled by default via `plugins.slots.memory`)
- Memory (LanceDB) — bundled long-term memory plugin (auto-recall/capture; set `plugins.slots.memory = "memory-lancedb"`)
- [Voice Call](/plugins/voice-call) — `@haoclaw/voice-call`
- [Zalo Personal](/plugins/zalouser) — `@haoclaw/zalouser`
- [Matrix](/channels/matrix) — `@haoclaw/matrix`
- [Nostr](/channels/nostr) — `@haoclaw/nostr`
- [Zalo](/channels/zalo) — `@haoclaw/zalo`
- [Microsoft Teams](/channels/msteams) — `@haoclaw/msteams`
- Google Antigravity OAuth (provider auth) — bundled as `google-antigravity-auth` (disabled by default)
- Gemini CLI OAuth (provider auth) — bundled as `google-gemini-cli-auth` (disabled by default)
- Qwen OAuth (provider auth) — bundled as `qwen-portal-auth` (disabled by default)
- Copilot Proxy (provider auth) — local VS Code Copilot Proxy bridge; distinct from built-in `github-copilot` device login (bundled, disabled by default)

Haoclaw plugins are **TypeScript modules** loaded at runtime via jiti. **Config
...
Use SDK subpaths instead of the monolithic `haoclaw/plugin-sdk` import when
authoring plugins:

- `haoclaw/plugin-sdk/core` for generic plugin APIs, provider auth types, and shared helpers.
- `haoclaw/plugin-sdk/compat` for bundled/internal plugin code that needs broader shared runtime helpers than `core`.
- `haoclaw/plugin-sdk/telegram` for Telegram channel plugins.
- `haoclaw/plugin-sdk/discord` for Discord channel plugins.
- `haoclaw/plugin-sdk/slack` for Slack channel plugins.
- `haoclaw/plugin-sdk/signal` for Signal channel plugins.
- `haoclaw/plugin-sdk/imessage` for iMessage channel plugins.
- `haoclaw/plugin-sdk/whatsapp` for WhatsApp channel plugins.
- `haoclaw/plugin-sdk/line` for LINE channel plugins.
- `haoclaw/plugin-sdk/msteams` for the bundled Microsoft Teams plugin surface.
- Bundled extension-specific subpaths are also available:
  `haoclaw/plugin-sdk/acpx`, `haoclaw/plugin-sdk/bluebubbles`,
  `haoclaw/plugin-sdk/copilot-proxy`, `haoclaw/plugin-sdk/device-pair`,
  `haoclaw/plugin-sdk/diagnostics-otel`, `haoclaw/plugin-sdk/diffs`,
  `haoclaw/plugin-sdk/feishu`,
  `haoclaw/plugin-sdk/google-gemini-cli-auth`, `haoclaw/plugin-sdk/googlechat`,
  `haoclaw/plugin-sdk/irc`, `haoclaw/plugin-sdk/llm-task`,
  `haoclaw/plugin-sdk/lobster`, `haoclaw/plugin-sdk/matrix`,
  `haoclaw/plugin-sdk/mattermost`, `haoclaw/plugin-sdk/memory-core`,
  `haoclaw/plugin-sdk/memory-lancedb`,
  `haoclaw/plugin-sdk/minimax-portal-auth`,
  `haoclaw/plugin-sdk/nextcloud-talk`, `haoclaw/plugin-sdk/nostr`,
  `haoclaw/plugin-sdk/open-prose`, `haoclaw/plugin-sdk/phone-control`,
  `haoclaw/plugin-sdk/qwen-portal-auth`, `haoclaw/plugin-sdk/synology-chat`,
  `haoclaw/plugin-sdk/talk-voice`, `haoclaw/plugin-sdk/test-utils`,
  `haoclaw/plugin-sdk/thread-ownership`, `haoclaw/plugin-sdk/tlon`,
  `haoclaw/plugin-sdk/twitch`, `haoclaw/plugin-sdk/voice-call`,
  `haoclaw/plugin-sdk/zalo`, and `haoclaw/plugin-sdk/zalouser`.

Compatibility note:

- `haoclaw/plugin-sdk` remains supported for existing external plugins.
...
- Read-only command paths such as `haoclaw status`, `haoclaw status --all`,
  `haoclaw channels status`, `haoclaw channels resolve`, and doctor/config
...
- Set `HAOCLAW_DISABLE_PLUGIN_DISCOVERY_CACHE=1` or
  `HAOCLAW_DISABLE_PLUGIN_MANIFEST_CACHE=1` to disable these caches.
- Tune cache windows with `HAOCLAW_PLUGIN_DISCOVERY_CACHE_MS` and
  `HAOCLAW_PLUGIN_MANIFEST_CACHE_MS`.

## Discovery & precedence

Haoclaw scans, in order:
...
- `<workspace>/.haoclaw/extensions/*.ts`
- `<workspace>/.haoclaw/extensions/*/index.ts`

3. Global extensions

- `~/.haoclaw/extensions/*.ts`
- `~/.haoclaw/extensions/*/index.ts`

4. Bundled extensions (shipped with Haoclaw, mostly disabled by default)

- `<haoclaw>/extensions/*`

Most bundled plugins must be enabled explicitly via
`plugins.entries.<id>.enabled` or `haoclaw plugins enable <id>`.
...
- If `plugins.allow` is empty and non-bundled plugins are discoverable, Haoclaw logs a startup warning with plugin ids and sources.
- Candidate paths are safety-checked before discovery admission. Haoclaw blocks candidates when:
...
Each plugin must include a `haoclaw.plugin.json` file in its root. If a path
...
A plugin directory may include a `package.json` with `haoclaw.extensions`:
...
Security guardrail: every `haoclaw.extensions` entry must stay inside the plugin
directory after symlink resolution. Entries that escape the package directory are
rejected.

Security note: `haoclaw plugins install` installs plugin dependencies with
`npm install --ignore-scripts` (no lifecycle scripts). Keep plugin dependency
trees "pure JS/TS" and avoid packages that require `postinstall` builds.

### Channel catalog metadata

Channel plugins can advertise onboarding metadata via `haoclaw.channel` and
install hints via `haoclaw.install`. This keeps the core catalog data-free.

Example:

```json
{
  "name": "@haoclaw/nextcloud-talk",
  "haoclaw": {
    "extensions": ["./index.ts"],
    "channel": {
      "id": "nextcloud-talk",
    ...
    "install": {
      "npmSpec": "@haoclaw/nextcloud-talk",
...
Haoclaw can also merge **external channel catalogs** (for example, an MPM
registry export). Drop a JSON file at one of:

- `~/.haoclaw/mpm/plugins.json`
- `~/.haoclaw/mpm/catalog.json`
- `~/.haoclaw/plugins/catalog.json`

Or point `HAOCLAW_PLUGIN_CATALOG_PATHS` (or `HAOCLAW_MPM_CATALOG_PATHS`) at
one or more JSON files (comma/semicolon/`PATH`-delimited). Each file should
contain `{ "entries": [ { "name": "@scope/pkg", "haoclaw": { "channel": {...}, "install": {...} } } ] }`.
...
If a plugin exports `id`, Haoclaw uses it but warns when it doesn’t match the
configured id.
...
- Plugin config is validated using the JSON Schema embedded in
  `haoclaw.plugin.json` (`configSchema`).
...
Haoclaw augments `uiHints` at runtime based on discovered plugins:
...
haoclaw plugins install <path>                 # copy a local file/dir into ~/.haoclaw/extensions/<id>
haoclaw plugins install ./extensions/voice-call # relative path ok
haoclaw plugins install ./plugin.tgz           # install from a local tarball
haoclaw plugins install ./plugin.zip           # install from a local zip
haoclaw plugins install -l ./extensions/voice-call # link (no copy) for dev
haoclaw plugins install @haoclaw/voice-call # install from npm
haoclaw plugins install @haoclaw/voice-call --pin # store exact resolved name@version
...
If stored integrity metadata changes between updates, Haoclaw warns and asks for confirmation (use global `--yes` to bypass prompts).

Plugins may also register their own top‑level commands (example: `haoclaw voicecall`).
...
- Plugin-managed hooks show up in `haoclaw hooks list` with `plugin:<id>`.
- You cannot enable/disable plugin-managed hooks via `haoclaw hooks`; enable/disable the plugin instead.
...
- When disabled, Haoclaw blocks `before_prompt_build` and ignores prompt-mutating fields returned from legacy `before_agent_start` while preserving legacy `modelOverride` and `providerOverride`.
...
API-key setup inside Haoclaw (no external scripts needed).

Register a provider via `api.registerProvider(...)`. Each provider exposes one
or more auth methods (OAuth, API key, device code, etc.). These methods power:

- `haoclaw models auth login --provider <id> [--method <id>]`
...
- `config`: The current Haoclaw config
...
- Main package: `haoclaw` (this repo)
- Plugins: separate npm packages under `@haoclaw/*` (example: `@haoclaw/voice-call`)

Publishing contract:

- Plugin `package.json` must include `haoclaw.extensions` with one or more entry files.
- Entry files can be `.js` or `.ts` (jiti loads TS at runtime).
- `haoclaw plugins install <npm-spec>` uses `npm pack`, extracts into `~/.haoclaw/extensions/<id>/`, and enables it in config.
...
- CLI: `haoclaw voicecall start|status`
- Tool: `voice_call`
...
- Separately published plugins should run their own CI (lint/build/test) and validate `haoclaw.extensions` points at the built entrypoint (`dist/index.js`).
