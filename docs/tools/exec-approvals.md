...
- **gateway host** → `haoclaw` process on the gateway machine
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.haoclaw/exec-approvals.sock",
...
- allowlist entries live in host-local `~/.haoclaw/exec-approvals.json` under `agents.<id>.allowlist` (or via Control UI / `haoclaw approvals allowlist ...`).
- `haoclaw security audit` warns with `tools.exec.safe_bins_interpreter_unprofiled` when interpreter/runtime bins appear in `safeBins` without explicit profiles.
- `haoclaw doctor --fix` can scaffold missing custom `safeBinProfiles.<bin>` entries as `{}` (review and tighten afterward). Interpreter/runtime bins are not auto-scaffolded.
...
`~/.haoclaw/exec-approvals.json` directly.

CLI: `haoclaw approvals` supports gateway or node editing (see [Approvals CLI](/cli/approvals)).
