---
summary: "Chrome extension: let Haoclaw drive your existing Chrome tab"
read_when:
...
The Haoclaw Chrome extension lets the agent control your **existing Chrome tabs** (your normal Chrome window) instead of launching a separate haoclaw-managed Chrome profile.
...
Haoclaw then controls the attached tab through the normal `browser` tool surface (selecting the right profile).
...
The extension ships inside the Haoclaw release (npm package) as static files. There is no separate “build” step.

After upgrading Haoclaw:

- Re-run `haoclaw browser extension install` to refresh the installed files under your Haoclaw state directory.
- Chrome → `chrome://extensions` → click “Reload” on the extension.

## Use it (set gateway token once)

Haoclaw ships with a built-in browser profile named `chrome` that targets the extension relay on the default port.

Before first attach, open extension Options and set:

- `Port` (default `18792`)
- `Gateway token` (must match `gateway.auth.token` / `HAOCLAW_GATEWAY_TOKEN`)
...
- Open the tab you want Haoclaw to control.
...
- `ON`: attached; Haoclaw can drive that tab.
...
Debugging: `haoclaw sandbox explain`
...
`haoclaw browser extension path` prints the **installed** on-disk directory containing the extension files.

The CLI intentionally does **not** print a `node_modules` path. Always run `haoclaw browser extension install` first to copy the extension to a stable location under your Haoclaw state directory.
...
- **This is not isolated** like the dedicated haoclaw-managed profile.
