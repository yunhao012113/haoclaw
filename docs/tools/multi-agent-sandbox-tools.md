...
For debugging “why is this blocked?”, see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `haoclaw sandbox explain`.
...
Legacy `agent.*` configs are migrated by `haoclaw doctor`; prefer `agents.defaults` + `agents.list` going forward.
...
2. **Verify sandbox containers:**
   ...
   ```exec
   docker ps --filter "name=haoclaw-sbx-"
   ```
...
4. **Monitor logs:**
   ...
   ```exec
   tail -f "${HAOCLAW_STATE_DIR:-$HOME/.haoclaw}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```
