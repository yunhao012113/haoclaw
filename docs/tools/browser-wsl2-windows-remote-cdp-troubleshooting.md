...
read_when:
  - Running Haoclaw Gateway in WSL2 while Chrome lives on Windows
...
- Haoclaw Gateway runs inside WSL2
- Chrome runs on Windows
- browser control must cross the WSL2/Windows boundary

It also covers the layered failure pattern from [issue #39369](https://github.com/haoclaw/haoclaw/issues/39369): several independent problems can show up at once, which makes the wrong layer look broken first.
...
Use the built-in `chrome` profile plus the Haoclaw Chrome extension.
...
- Haoclaw points a browser profile at the address that is reachable from WSL2
...
If this fails on Windows, Haoclaw is not the problem yet.
...
Fix that before touching Haoclaw config.

### Layer 3: Configure the correct browser profile

For raw remote CDP, point Haoclaw at the address that is reachable from WSL2:
...
- test the same URL with `curl` before expecting Haoclaw to succeed
...
```bash
haoclaw browser open https://example.com --browser-profile remote
haoclaw browser tabs --browser-profile remote
```

For the extension relay:

```bash
haoclaw browser tabs --browser-profile chrome
```

Good result:

- the tab opens in Windows Chrome
- `haoclaw browser tabs` returns the target
...
3. Haoclaw config: does `browser.profiles.<name>.cdpUrl` use that exact WSL2-reachable address?
...
- only then debug Haoclaw config or Control UI auth
