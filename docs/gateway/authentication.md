...
Haoclaw supports OAuth and API keys for model providers. For always-on gateway
...
2. Put it on the **gateway host** (the machine running `haoclaw gateway`).
...
3. If the Gateway runs under systemd/launchd, prefer putting the key in
   `~/.haoclaw/.env` so the daemon can read it:
...
API keys for daemon use: `haoclaw onboard`.

See [Help](/help) for details on env inheritance (`env.shellEnv`,
`~/.haoclaw/.env`, systemd/launchd).
...
```bash
haoclaw models auth paste-token --provider anthropic
haoclaw models auth paste-token --provider openrouter
```
...
- Priority order:
  - `HAOCLAW_LIVE_<PROVIDER>_KEY` (single override)
  - `<PROVIDER>_API_KEYS`
  - `<PROVIDER>_API_KEY`
  - `<PROVIDER>_API_KEY_*`
- Google providers also include `GOOGLE_API_KEY` as an additional fallback.
- The same key list is deduplicated before use.
- Haoclaw retries with the next key only for rate-limit errors (for example
...
Run `haoclaw models status` to confirm which profile is expiring. If the profile
