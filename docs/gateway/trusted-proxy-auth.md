...
read_when:
  - Running Haoclaw behind an identity-aware proxy
  - Setting up Pomerium, Caddy, or nginx with OAuth in front of Haoclaw
...
- You run Haoclaw behind an **identity-aware proxy** (Pomerium, Caddy + OAuth, nginx + oauth2-proxy, Traefik + forward auth)
...
3. Haoclaw checks that the request came from a **trusted proxy IP** (configured in `gateway.trustedProxies`)
4. Haoclaw extracts the user identity from the configured header
...
- Haoclaw can stay on loopback HTTP behind the proxy.
...
If Haoclaw itself serves HTTPS directly (no TLS-terminating proxy), set:
...
```yaml
routes:
  - from: https://haoclaw.example.com
    to: http://haoclaw-gateway:18789
    policy:
      - allow:
          or:
            - email:
...
`haoclaw security audit` will flag trusted-proxy auth with a **critical** severity finding. This is intentional — it's a reminder that you're delegating security to your proxy setup.
...
3. Update Haoclaw config with trusted-proxy auth
4. Restart the Gateway
5. Test WebSocket connections from the Control UI
6. Run `haoclaw security audit` and review findings
