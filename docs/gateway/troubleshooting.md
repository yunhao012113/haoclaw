...
- `Gateway start blocked: set gateway.mode=local` → local gateway mode is not enabled. Fix: set `gateway.mode="local"` in your config (or run `haoclaw configure`). If you are running Haoclaw via Podman using the dedicated `haoclaw` user, the config lives at `~haoclaw/.haoclaw/haoclaw.json`.
