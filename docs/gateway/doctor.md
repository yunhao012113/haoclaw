...
`haoclaw doctor` is the repair + migration tool for Haoclaw. It fixes stale
...
- Extra workspace dir detection (`~/haoclaw`).
...
you to run `haoclaw doctor`.

Doctor will:

- Explain which legacy keys were found.
- Show the migration it applied.
- Rewrite `~/.haoclaw/haoclaw.json` with the updated schema.
...
- Sessions store + transcripts:
  - from `~/.haoclaw/sessions/` to `~/.haoclaw/agents/<agentId>/sessions/`
- Agent dir:
  - from `~/.haoclaw/agent/` to `~/.haoclaw/agents/<agentId>/agent/`
- WhatsApp auth state (Baileys):
  - from legacy `~/.haoclaw/credentials/*.json` (except `oauth.json`)
  - to `~/.haoclaw/credentials/whatsapp/<accountId>/...` (default account id: `default`)

These migrations are best-effort and idempotent; doctor will emit warnings when
it leaves any legacy folders behind as backups. The Gateway/CLI also auto-migrates
the legacy sessions + agent dir on startup so history/auth/models land in the
per-agent path without a manual doctor run. WhatsApp auth is intentionally only
migrated via `haoclaw doctor`.
...
- **Multiple state dirs**: warns when multiple `~/.haoclaw` folders exist across
  home directories or when `HAOCLAW_STATE_DIR` points elsewhere (history can
  split between installs).
- **Remote mode reminder**: if `gateway.mode=remote`, doctor reminds you to run
  it on the remote host (the state lives there).
- **Config file permissions**: warns if `~/.haoclaw/haoclaw.json` is
...
offers to remove them and install the Haoclaw service using the current gateway
port. It can also scan for extra gateway-like services and print cleanup hints.
Profile-named Haoclaw gateway services are considered first-class and are not
flagged as "extra."
...
- `haoclaw doctor --generate-gateway-token` forces generation only when no token SecretRef is configured.
...
- `haoclaw doctor --fix` now uses the same read-only SecretRef summary model as status-family commands for targeted config repairs.
...
- `haoclaw doctor` prompts before rewriting supervisor config.
- `haoclaw doctor --yes` accepts the default repair prompts.
- `haoclaw doctor --repair` applies recommended fixes without prompts.
- `haoclaw doctor --repair --force` overwrites custom supervisor configs.
...
- You can always force a full rewrite via `haoclaw gateway install --force`.
