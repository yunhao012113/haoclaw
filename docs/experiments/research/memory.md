...
read_when:
  - Designing workspace memory (~/.haoclaw/workspace) beyond daily Markdown logs
  - Deciding: standalone CLI vs deep Haoclaw integration
...
Target: Clawd-style workspace (`agents.defaults.workspace`, default `~/.haoclaw/workspace`) where “memory” is stored as one Markdown file per day (`memory/YYYY-MM-DD.md`) plus a small set of stable files (e.g. `memory.md`, `SOUL.md`).
...
Keep `~/.haoclaw/workspace` as canonical human-readable memory.
...
```
~/.haoclaw/workspace/.memory/index.sqlite
```
...
Recommendation: **deep integration in Haoclaw**, but keep a separable core library.

### Why integrate into Haoclaw?

- Haoclaw already knows:
  - the workspace path (`agents.defaults.workspace`)
  - the session model + heartbeats
  - logging + troubleshooting patterns
- You want the agent itself to call the tools:
  - `haoclaw memory recall "…" --k 25 --since 30d`
  - `haoclaw memory reflect --since 7d`
...
Pragmatic take for `~/.haoclaw/workspace`:
