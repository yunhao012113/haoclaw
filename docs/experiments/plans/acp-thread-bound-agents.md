...
This plan defines how Haoclaw should support ACP coding agents in thread-capable channels (Discord first) with production-level lifecycle and recovery.
...
- Haoclaw core owns ACP control plane concerns
...
Haoclaw should not reimplement ACP transport internals in core.
Haoclaw should not rely on a pure plugin-only interception path for routing.

## North-star architecture (holy grail)

Treat ACP as a first-class control plane in Haoclaw, with pluggable runtime adapters.
...
- move ACP control-plane state to a dedicated SQLite store (WAL mode) under Haoclaw state dir
...
- ACP event stream is normalized to Haoclaw reply chunks
...
For the first production backend (`extensions/acpx`), Haoclaw and acpx are
...
- encoded payload fields:
  - `name` (acpx named session; uses Haoclaw `sessionKey`)
...
- Haoclaw consumes ndjson events from `acpx --format json --json-strict`
