...
```bash
haoclaw tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Turn delivery on:
  ...
  - or start with `haoclaw tui --deliver`
...
- Local shell commands receive `HAOCLAW_SHELL=tui-local` in their environment.
...
- If your terminal uses a light background and auto-detection is wrong, set `HAOCLAW_THEME=light` before launching `haoclaw tui`.
- To force the original dark palette instead, set `HAOCLAW_THEME=dark`.
...
- Check the Gateway logs: `haoclaw logs --follow`.
- Confirm the agent can run: `haoclaw status` and `haoclaw models status`.
...
- No agents in picker: check `haoclaw agents list` and your routing config.
