...
- optional prefix: set `gateway.controlUi.basePath` (e.g. `/haoclaw`)
...
If the page fails to load, start the Gateway first: `haoclaw gateway`.
...
you revoke it with `haoclaw devices revoke --device <id> --role <role>`. See
...
- Config: view/edit `~/.haoclaw/haoclaw.json` (`config.get`, `config.set`)
...
- Stop:
  ...
  - Type `/stop` (or standalone abort phrases like `stop`, `stop action`, `stop run`, `stop haoclaw`, `please stop`) to abort out-of-band
...
(`tailscale-user-login`) when `gateway.auth.allowTailscale` is `true`. Haoclaw
...
Haoclaw **blocks** Control UI connections without device identity.
