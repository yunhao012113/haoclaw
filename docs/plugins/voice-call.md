...
read_when:
  - You want to place an outbound voice call from Haoclaw
...
Voice calls for Haoclaw via a plugin. Supports outbound notifications and
...
- Use `haoclaw voicecall ...` or the `voice_call` tool
...
```bash
haoclaw plugins install ./extensions/voice-call
