---
summary: "Use OpenAI via API keys or Codex subscription in Haoclaw"
read_when:
  - You want to use OpenAI models in Haoclaw
...
OpenAI explicitly supports subscription OAuth usage in external tools/workflows like Haoclaw.
...
```bash
haoclaw onboard --auth-choice openai-api-key
# or non-interactive
haoclaw onboard --openai-api-key "$OPENAI_API_KEY"
```
...
OpenAI API usage. Haoclaw forwards both through the `openai/*` Responses path.
...
haoclaw onboard --auth-choice openai-codex

# Or run OAuth directly
haoclaw models auth login --provider openai-codex
```
...
OpenAI's current Codex docs list `gpt-5.4` as the current Codex model. Haoclaw
maps that to `openai-codex/gpt-5.4` for ChatGPT/Codex OAuth usage.

### Transport default

Haoclaw uses `pi-ai` for model streaming. For both `openai/*` and
...
For `openai/*` (Responses API), Haoclaw also enables WebSocket warm-up by
...
OpenAI docs describe warm-up as optional. Haoclaw enables it by default for
...
Haoclaw, set `agents.defaults.models["openai/<model>"].params.serviceTier` to
...
`baseUrl` on `api.openai.com`), Haoclaw now auto-enables OpenAI server-side
