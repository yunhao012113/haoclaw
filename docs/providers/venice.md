---
summary: "Use Venice AI privacy-focused models in Haoclaw"
read_when:
  - You want privacy-focused inference in Haoclaw
...
```bash
haoclaw onboard --auth-choice venice-api-key
```
...
```bash
haoclaw agent --model venice/kimi-k2-5 --message "Hello, are you working?"
```

## Model Selection

After setup, Haoclaw shows all available Venice models. Pick based on your needs:
...
```bash
haoclaw models set venice/kimi-k2-5
haoclaw models set venice/claude-opus-4-6
```

List all available models:

```bash
haoclaw models list | grep venice
```

## Configure via `haoclaw configure`

1. Run `haoclaw configure`
2. Select **Model/auth**
3. Choose **Venice AI**
...
Haoclaw automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
haoclaw agent --model venice/kimi-k2-5 --message "Quick health check"

# Use Claude Opus via Venice (anonymized)
haoclaw agent --model venice/claude-opus-4-6 --message "Summarize this task"

# Use uncensored model
haoclaw agent --model venice/venice-uncensored --message "Draft options"

# Use vision model with image
haoclaw agent --model venice/qwen3-vl-235b-a22b --message "Review attached image"

# Use coding model
haoclaw agent --model venice/qwen3-coder-480b-a35b-instruct --message "Refactor this function"
```
...
The Venice model catalog updates dynamically. Run `haoclaw models list` to see currently available models. Some models may be temporarily offline.
