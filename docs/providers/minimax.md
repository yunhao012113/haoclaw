---
summary: "Use MiniMax M2.5 in Haoclaw"
read_when:
  - You want MiniMax models in Haoclaw
...
```bash
haoclaw plugins enable minimax-portal-auth  # skip if already loaded.
haoclaw gateway restart  # restart if gateway is already running
haoclaw onboard --auth-choice minimax-portal
```
...
See [MiniMax OAuth plugin README](https://github.com/haoclaw/haoclaw/tree/main/extensions/minimax-portal-auth) for details.
...
Configure manually via `haoclaw.json`:
...
- Use `haoclaw models list` and `haoclaw models set minimax/MiniMax-M2.5` to switch.
...
- Running `haoclaw configure` and selecting **MiniMax M2.5**, or
