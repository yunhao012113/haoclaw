...
Matrix is an open, decentralized messaging protocol. Haoclaw connects as a Matrix **user**
...
```bash
haoclaw plugins install @haoclaw/matrix
```

Local checkout (when running from a git repo):

```bash
haoclaw plugins install ./extensions/matrix
```
...
1. Install the Matrix plugin:
   - From npm: `haoclaw plugins install @haoclaw/matrix`
   - From a local checkout: `haoclaw plugins install ./extensions/matrix`
...
3. Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: Haoclaw calls the same
     login endpoint, stores the access token in `~/.haoclaw/credentials/matrix/credentials.json`,
...
- On first connection, Haoclaw requests device verification from your other sessions.
- Verify the device in another Matrix client (Element, etc.) to enable key sharing.
- If the crypto module cannot be loaded, E2EE is disabled and encrypted rooms will not decrypt;
  Haoclaw logs a warning.
...
`~/.haoclaw/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/crypto/`
...
- Approve via:
  - `haoclaw pairing list matrix`
  - `haoclaw pairing approve matrix <CODE>`
...
- On startup, Haoclaw resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are ignored for allowlist matching.
