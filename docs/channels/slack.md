...
```
  ...
  <Tab title="HTTP Events API mode">
    <Steps>
      ...
      <Step title="Configure Haoclaw HTTP mode">
...
<Tabs>
  <Tab title="DM policy">
    ...
    Pairing in DMs uses `haoclaw pairing approve slack <code>`.
...
`typingReaction` adds a temporary reaction to the inbound Slack message while Haoclaw is processing a reply, then removes it when the run finishes. This is a useful fallback when Slack native assistant typing is unavailable, especially in DMs.
...
Haoclaw supports Slack native text streaming via the Agents and AI Apps API.
...
- If streaming fails mid-reply, Haoclaw falls back to normal delivery for remaining payloads.
