...
Feishu (Lark) is a team chat platform used by companies for messaging and collaboration. This plugin connects Haoclaw to a Feishu/Lark bot using the platform’s WebSocket event subscription so messages can be received without exposing a public webhook URL.

---

## Bundled plugin

Feishu ships bundled with current Haoclaw releases, so no separate plugin install
is required.
...
```bash
haoclaw plugins install @haoclaw/feishu
```
...
If you just installed Haoclaw, run the wizard:
...
2. Configuring app credentials in Haoclaw
...
1. You already ran `haoclaw channels add` for Feishu
2. The gateway is running (`haoclaw gateway status`)
...
```bash
haoclaw pairing approve feishu <CODE>
```
...
2. Run `haoclaw logs --follow` and look for `chat_id`
...
2. Run `haoclaw logs --follow` and look for `open_id`
...
| `haoclaw gateway restart` | Restart gateway service       |
| `haoclaw logs --follow`   | Tail gateway logs             |
...
4. Check logs: `haoclaw logs --follow`
...
5. Ensure the gateway is running: `haoclaw gateway status`
6. Check logs: `haoclaw logs --follow`
  ...
  agents: {
    list: [
      { id: "main" },
      {
        ...
        agentDir: "/home/user/.haoclaw/agents/clawd-fan/agent",
      },
      {
        id: "clawd-xi",
        workspace: "/home/user/clawd-xi",
        agentDir: "/home/user/.haoclaw/agents/clawd-xi/agent",
      },
    ],
  },
  bindings: [
