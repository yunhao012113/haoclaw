...
read_when:
  - Setting up Twitch chat integration for Haoclaw
title: "Twitch"
---

# Twitch (plugin)

Twitch chat support via IRC connection. Haoclaw connects as a Twitch user (bot account) to receive and send messages in channels.
...
```bash
haoclaw plugins install @haoclaw/twitch
```

Local checkout (when running from a git repo):

```bash
haoclaw plugins install ./extensions/twitch
```
...
4. Configure the token:
   - Env: `HAOCLAW_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "haoclaw", // Bot's Twitch account
      accessToken: "oauth:abc123...", // OAuth Access Token (or use HAOCLAW_TWITCH_ACCESS_TOKEN env var)
...
```bash
HAOCLAW_TWITCH_ACCESS_TOKEN=oauth:abc123...
```
