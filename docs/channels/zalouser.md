...
read_when:
  - Setting up Zalo Personal for Haoclaw
...
Status: experimental. This integration automates a **personal Zalo account** via native `zca-js` inside Haoclaw.
...
- Install via CLI: `haoclaw plugins install @haoclaw/zalouser`
- Or from a source checkout: `haoclaw plugins install ./extensions/zalouser`
...
- `haoclaw pairing list zalouser`
- `haoclaw pairing approve zalouser <code>`
...
- On startup, Haoclaw resolves group/user names in allowlists to IDs and logs the mapping; unresolved entries are kept as typed.
...
- When a group message is skipped because mention is required, Haoclaw stores it as pending group history and includes it on the next processed group message.
...
Accounts map to `zalouser` profiles in Haoclaw state. Example:
...
- Haoclaw sends a typing event before dispatching a reply (best-effort).
...
- For inbound messages that include event metadata, Haoclaw sends delivered + seen acknowledgements (best-effort).
...
- Re-login: `haoclaw channels logout --channel zalouser && haoclaw channels login --channel zalouser`
...
- The channel now runs fully in Haoclaw without external CLI binaries.
