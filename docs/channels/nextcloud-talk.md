...
```bash
haoclaw plugins install @haoclaw/nextcloud-talk
```

Local checkout (when running from a git repo):

```bash
haoclaw plugins install ./extensions/nextcloud-talk
```
...
2. On your Nextcloud server, create a bot:
   ...
   ```bash
   ./occ talk:bot:install "Haoclaw" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `haoclaw pairing list nextcloud-talk`
  - `haoclaw pairing approve nextcloud-talk <CODE>`
