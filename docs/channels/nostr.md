...
read_when:
  - You want Haoclaw to receive DMs via Nostr
...
Nostr is a decentralized protocol for social networking. This channel enables Haoclaw to receive and respond to encrypted direct messages (DMs) via NIP-04.

## Install (on demand)

### Onboarding (recommended)

- The onboarding wizard (`haoclaw onboard`) and `haoclaw channels add` list optional channel plugins.
...
```bash
haoclaw plugins install @haoclaw/nostr
```

Use a local checkout (dev workflows):

```bash
haoclaw plugins install --link <path-to-haoclaw>/extensions/nostr
```
  ...
  "channels": {
    "nostr": {
      ...
      "profile": {
        ...
        "nip05": "haoclaw@example.com",
        "lud16": "haoclaw@example.com"
      }
    }
  }
}
