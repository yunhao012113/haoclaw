...
```
    ...
    Telegram does **not** use `haoclaw channels login telegram`; configure token in config/env, then start gateway.
...
<Tabs>
  <Tab title="DM policy">
    ...
    If you upgraded and your config contains `@username` allowlist entries, run `haoclaw doctor --fix` to resolve them (best-effort; requires a Telegram bot token).
    If you previously relied on pairing-store allowlist files, `haoclaw doctor --fix` can recover entries into `channels.telegram.allowFrom` in allowlist flows (for example when `dmPolicy: "allowlist"` has no explicit IDs yet).
...
```
    ...
    - or read `chat.id` from `haoclaw logs --follow`
...
- DM messages can carry `message_thread_id`; Haoclaw routes them with thread-aware session keys and preserves thread ID for replies.
...
<AccordionGroup>
  <Accordion title="Live stream preview (message edits)">
    Haoclaw can stream partial replies in real time:
    ...
    - DM: Haoclaw keeps the same preview message and performs a final edit in place (no second message)
    - group/topic: Haoclaw keeps the same preview message and performs a final edit in place (no second message)

    For complex replies (for example media payloads), Haoclaw falls back to normal final delivery and then cleans up the preview message.

    Preview streaming is separate from block streaming. When block streaming is explicitly enabled for Telegram, Haoclaw skips the preview stream to avoid double-streaming.

    If native draft transport is unavailable/rejected, Haoclaw automatically falls back to `sendMessage` + `editMessageText`.
  ...
  <Accordion title="Formatting and HTML fallback">
    ...
    - If Telegram rejects parsed HTML, Haoclaw retries as plain text.
...
```
  ...
  <Accordion title="Forum topics and thread behavior">
    ...
    **Thread-bound ACP spawn from chat**:
    ...
    - Haoclaw pins the spawn confirmation message in-topic after a successful bind.
...
```
    ...
    - `~/.haoclaw/telegram/sticker-cache.json`
...
```
  ...
  <Accordion title="Reaction notifications">
    ...
    When enabled, Haoclaw enqueues system events like:
...
```bash
haoclaw message send --channel telegram --target 123456789 --message "hi"
haoclaw message send --channel telegram --target @name --message "hi"
```

    Telegram polls use `haoclaw message poll` and support forum topics:

```bash
haoclaw message poll --channel telegram --target 123456789 \
  --poll-question "Ship it?" --poll-option "Yes" --poll-option "No"
haoclaw message poll --channel telegram --target -1001234567890:topic:42 \
...
<AccordionGroup>
  <Accordion title="Bot does not respond to non mention group messages">
    ...
    - `haoclaw channels status` warns when config expects unmentioned group messages.
    - `haoclaw channels status --probe` can check explicit numeric group IDs; wildcard `"*"` cannot be membership-probed.
  ...
  <Accordion title="Bot not seeing group messages at all">
    ...
    - review logs: `haoclaw logs --follow` for skip reasons
  ...
  <Accordion title="Polling or network instability">
    ...
    - If logs include `TypeError: fetch failed` or `Network request for 'getUpdates' failed!`, Haoclaw now retries these as recoverable network errors.
...
```
    ...
    - Environment overrides (temporary):
      - `HAOCLAW_TELEGRAM_DISABLE_AUTO_SELECT_FAMILY=1`
      - `HAOCLAW_TELEGRAM_ENABLE_AUTO_SELECT_FAMILY=1`
      - `HAOCLAW_TELEGRAM_DNS_RESULT_ORDER=ipv4first`
...
- `channels.telegram.allowFrom`: DM allowlist (numeric Telegram user IDs). `allowlist` requires at least one sender ID. `open` requires `"*"`. `haoclaw doctor --fix` can resolve legacy `@username` entries to IDs and can recover allowlist entries from pairing-store files in allowlist migration flows.
...
- `channels.telegram.groupAllowFrom`: group sender allowlist (numeric Telegram user IDs). `haoclaw doctor --fix` can resolve legacy `@username` entries to IDs. Non-numeric entries are ignored at auth time. Group auth does not use DM pairing-store fallback (`2026.2.25+`).
- Multi-account precedence:
  ...
  - If neither is set, Haoclaw falls back to the first normalized account ID and `haoclaw doctor` warns.
