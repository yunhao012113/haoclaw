---
summary: "Synology Chat webhook setup and Haoclaw config"
read_when:
  - Setting up Synology Chat with Haoclaw
...
```bash
haoclaw plugins install ./extensions/synology-chat
```
...
3. Point the outgoing webhook URL to your Haoclaw gateway:
   - `https://gateway-host/webhook/synology` by default.
   - Or your custom `channels.synology-chat.webhookPath`.
4. Configure `channels.synology-chat` in Haoclaw.
...
- Pairing approvals work with:
  - `haoclaw pairing list synology-chat`
  - `haoclaw pairing approve synology-chat <CODE>`
...
```bash
haoclaw message send --channel synology-chat --target 123456 --text "Hello from Haoclaw"
haoclaw message send --channel synology-chat --target synology-chat:123456 --text "Hello again"
```
