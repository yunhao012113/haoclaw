...
Tlon is a decentralized messenger built on Urbit. Haoclaw connects to your Urbit ship and can
...
```bash
haoclaw plugins install @haoclaw/tlon
```

Local checkout (when running from a git repo):

```bash
haoclaw plugins install ./extensions/tlon
```
...
By default, Haoclaw blocks private/internal hostnames and IP ranges for SSRF protection.
...
Use these with `haoclaw message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, Haoclaw replies in-thread.
