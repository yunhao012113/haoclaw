...
- Publish as `haoclaw/plugin-sdk` (or export from core under `haoclaw/plugin-sdk`).
- Semver with explicit stability guarantees.

### 2) Plugin Runtime (execution surface, injected)

Scope: everything that touches core runtime behavior.
Accessed via `HaoclawPluginApi.runtime` so plugins never import `src/**`.
...
export type PluginRuntime = {
  channel: {
    text: {
      ...
      resolveTextChunkLimit(cfg: HaoclawConfig, channel: string, accountId?: string): number;
      hasControlCommand(text: string, cfg: HaoclawConfig): boolean;
    };
    reply: {
    ...
    mentions: {
      buildMentionRegexes(cfg: HaoclawConfig, agentId?: string): RegExp[];
    ...
    debounce: {
      ...
      resolveInboundDebounceMs(cfg: HaoclawConfig, channel: string): number;
    };
    commands: {
  ...
  state: {
    resolveStateDir(cfg: HaoclawConfig): string;
  };
};
```
...
- Introduce `haoclaw/plugin-sdk`.
- Add `api.runtime` to `HaoclawPluginApi` with the surface above.
...
- Plugins declare a required runtime range (e.g., `haoclawRuntime: ">=2026.2.0"`).
