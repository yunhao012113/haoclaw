...
The onboarding wizard is the **recommended** way to set up Haoclaw on macOS,
...
`haoclaw dashboard` and chat in the browser. Docs: [Dashboard](/web/dashboard).
</Info>
...
`haoclaw configure --section web`. Docs: [Web tools](/tools/web).
</Tip>
...
2. **Workspace** — Location for agent files (default `~/.haoclaw/workspace`). Seeds bootstrap files.
...
If the config is invalid or contains legacy keys, the wizard asks you to run `haoclaw doctor` first.
</Note>
...
Use `haoclaw agents add <name>` to create a separate agent with its own workspace,
...
- Default workspaces follow `~/.haoclaw/workspace-<agentId>`.
...
- CLI command reference: [`haoclaw onboard`](/cli/onboard)
