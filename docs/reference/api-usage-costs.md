...
Haoclaw features that can generate provider usage or paid API calls.
...
- `haoclaw status --usage` and `haoclaw channels list` show provider **usage windows**
...
Haoclaw can pick up credentials from:
...
- `haoclaw status --usage`
- `haoclaw models status --json`
...
`haoclaw models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
