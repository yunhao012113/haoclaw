...
This document explains how Haoclaw manages sessions end-to-end:
...
Haoclaw is designed around a single **Gateway process** that owns session state.
...
Haoclaw persists sessions in two layers:
...
- Transcripts: `~/.haoclaw/agents/<agentId>/sessions/<sessionId>.jsonl`
  - Telegram topic sessions: `.../<sessionId>-topic-<threadId>.jsonl`

Haoclaw resolves these via `src/config/sessions.ts`.
...
In `mode: "warn"`, Haoclaw reports potential evictions but does not mutate the store/files.
...
- `cron.runLog.maxBytes` + `cron.runLog.keepLines` prune `~/.haoclaw/cron/runs/<jobId>.jsonl` files (defaults: `2_000_000` bytes and `2000` lines).
...
Haoclaw intentionally does **not** “fix up” transcripts; the Gateway uses `SessionManager` to read/write them.
...
These are Pi runtime semantics (Haoclaw consumes the events, but Pi decides when to compact).
...
Haoclaw also enforces a safety floor for embedded runs:

- If `compaction.reserveTokens < reserveTokensFloor`, Haoclaw bumps it.
- Default floor is `20000` tokens.
- Set `agents.defaults.compaction.reserveTokensFloor: 0` to disable the floor.
- If it’s already higher, Haoclaw leaves it alone.
...
- `haoclaw status` (CLI)
- `haoclaw sessions` / `sessions --json`
...
Haoclaw supports “silent” turns for background tasks where the user should not see intermediate output.

Convention:

- The assistant starts its output with `NO_REPLY` to indicate “do not deliver a reply to the user”.
- Haoclaw strips/suppresses this in the delivery layer.

As of `2026.1.10`, Haoclaw also suppresses **draft/typing streaming** when a partial chunk begins with `NO_REPLY`, so silent operations don’t leak partial output mid-turn.
...
Haoclaw uses the **pre-threshold flush** approach:
...
Pi also exposes a `session_before_compact` hook in the extension API, but Haoclaw’s
...
- Store vs transcript mismatch? Confirm the Gateway host and the store path from `haoclaw status`.
