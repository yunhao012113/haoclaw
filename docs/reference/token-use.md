---
summary: "How Haoclaw builds prompt context and reports token usage + costs"
read_when:
...
Haoclaw tracks **tokens**, not characters. Tokens are model-specific, but most
OpenAI-style models average ~4 characters per token for English text.

## How the system prompt is built

Haoclaw assembles its own system prompt on every run. It includes:
...
For images, Haoclaw downscales transcript/tool image payloads before provider calls.
...
- `/usage cost` → shows a local cost summary from Haoclaw session logs.

Other surfaces:

- **TUI/Web TUI:** `/status` + `/usage` are supported.
- **CLI:** `haoclaw status --usage` and `haoclaw channels list` show
...
`cacheWrite`. If pricing is missing, Haoclaw shows tokens only. OAuth tokens
never show dollar cost.

## Cache TTL and pruning impact

Provider prompt caching only applies within the cache TTL window. Haoclaw can
...
Anthropic's 1M context window is currently beta-gated. Haoclaw can inject the
...
Haoclaw skips the `context-1m-*` beta header because Anthropic currently
