...
- With Anthropic API-key auth profiles, Haoclaw seeds `cacheRetention: "short"` for Anthropic model refs when unset.
...
For `openrouter/anthropic/*` model refs, Haoclaw injects Anthropic `cache_control` on system/developer prompt blocks to improve prompt-cache reuse.
...
Haoclaw exposes dedicated cache-trace diagnostics for embedded agent runs.
...
diagnostics:
  cacheTrace:
    enabled: true
    filePath: "~/.haoclaw/logs/cache-trace.jsonl" # optional
...
- `filePath`: `$HAOCLAW_STATE_DIR/logs/cache-trace.jsonl`
...
- `HAOCLAW_CACHE_TRACE=1` enables cache tracing.
- `HAOCLAW_CACHE_TRACE_FILE=/path/to/cache-trace.jsonl` overrides output path.
- `HAOCLAW_CACHE_TRACE_MESSAGES=0|1` toggles full message payload capture.
- `HAOCLAW_CACHE_TRACE_PROMPT=0|1` toggles prompt text capture.
- `HAOCLAW_CACHE_TRACE_SYSTEM=0|1` toggles system prompt capture.
