...
This is the full reference for the `haoclaw onboard` CLI wizard.
...
<Steps>
  ...
  <Step title="Model/Auth">
    ...
    - OAuth credentials live in `~/.haoclaw/credentials/oauth.json`; auth profiles live in `~/.haoclaw/agents/<agentId>/agent/auth-profiles.json` (API keys + OAuth).
    - More detail: [/concepts/oauth](/concepts/oauth)
    <Note>
    Headless/server tip: complete OAuth on a machine with a browser, then copy
    `~/.haoclaw/credentials/oauth.json` (or `$HAOCLAW_STATE_DIR/credentials/oauth.json`) to the
    gateway host.
    </Note>
  </Step>
  <Step title="Workspace">
    - Default `~/.haoclaw/workspace` (configurable).
  ...
  <Step title="Channels">
    ...
    - DM security: default is pairing. First DM sends a code; approve via `haoclaw pairing approve <channel> <code>` or use allowlists.
  </Step>
  <Step title="Web search">
    ...
    - Configure later: `haoclaw configure --section web`.
  </Step>
  ...
  <Step title="Health check">
    - Starts the Gateway (if needed) and runs `haoclaw health`.
    - Tip: `haoclaw status --deep` adds gateway health probes to status output (requires a reachable gateway).
  </Step>
...
```bash
export HAOCLAW_GATEWAY_TOKEN="your-token"
haoclaw onboard --non-interactive \
  --mode local \
  --auth-choice skip \
  --gateway-auth token \
  --gateway-token-ref-env HAOCLAW_GATEWAY_TOKEN
```
...
- Stores it under `~/.haoclaw/tools/signal-cli/<version>/`.
...
Typical fields in `~/.haoclaw/haoclaw.json`:
...
`haoclaw agents add` writes `agents.list[]` and optional `bindings`.

WhatsApp credentials go under `~/.haoclaw/credentials/whatsapp/<accountId>/`.
Sessions are stored under `~/.haoclaw/agents/<agentId>/sessions/`.
