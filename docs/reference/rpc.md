...
Haoclaw integrates external CLIs via JSON-RPC. Two patterns are used today.
...
- Haoclaw owns lifecycle when `channels.signal.autoStart=true`.
...
- Haoclaw spawns `imsg rpc` as a child process (legacy iMessage integration).
