---
summary: "CLI reference for `haoclaw configure` (interactive configuration prompts)"
read_when:
...
Tip: `haoclaw config` without a subcommand opens the same wizard. Use
`haoclaw config get|set|unset` for non-interactive edits.
...
```bash
haoclaw configure
haoclaw configure --section model --section channels
```
