---
summary: "CLI reference for `haoclaw onboard` (interactive onboarding wizard)"
read_when:
...
```bash
haoclaw onboard
haoclaw onboard --flow quickstart
haoclaw onboard --flow manual
haoclaw onboard --mode remote --remote-url wss://gateway-host:18789
```

For plaintext private-network `ws://` targets (trusted networks only), set
`HAOCLAW_ALLOW_INSECURE_PRIVATE_WS=1` in the onboarding process environment.
...
- Fastest first chat: `haoclaw dashboard` (Control UI, no channel setup).
