...
read_when:
  ...
  - Wiring external systems into Haoclaw
title: "Webhooks"
---
...
- `hooks.transformsDir` + `transform.module` loads a JS/TS module for custom logic.
  - `hooks.transformsDir` (if set) must stay within the transforms root under your Haoclaw config directory (typically `~/.haoclaw/hooks/transforms`).
...
- `haoclaw webhooks gmail setup` writes `hooks.gmail` config for `haoclaw webhooks gmail run`.
