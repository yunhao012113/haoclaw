...
haoclaw cron add --name "Morning brief" --cron "0 7 * * *" --session isolated --message "..." --announce

# Weekly project review on Mondays at 9am
haoclaw cron add --name "Weekly review" --cron "0 9 * * 1" --session isolated --message "..." --model opus

# One-shot reminder
haoclaw cron add --name "Call back" --at "2h" --session main --system-event "Call back the client" --wake now
```

## Octopus Workflow: Deterministic workflows with approvals

Octopus Workflow is the workflow runtime for **multi-step tool pipelines** that need deterministic execution and explicit approvals.
Use it when the task is more than a single agent turn, and you want a resumable workflow with human checkpoints.

### When Octopus Workflow fits
...
- **Octopus Workflow** defines _what steps_ happen once the run starts.

For scheduled workflows, use cron or heartbeat to trigger an agent turn with deterministic tool execution.
For ad-hoc workflows, call the relevant tool or agent flow directly.

### Operational notes (from the code)

- Keep scheduled work deterministic when the same action must repeat reliably.
- Prefer explicit approval points before external side effects.
- Use additive tool allowlists so core tools remain available unless you intentionally restrict them.
