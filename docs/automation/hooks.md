...
Hooks provide an extensible event-driven system for automating actions in response to agent commands and events. Hooks are automatically discovered from directories and can be managed via CLI commands, similar to how skills work in Haoclaw.
...
- **Webhooks**: external HTTP webhooks that let other systems trigger work in Haoclaw. See [Webhook Hooks](/automation/webhook) or use `haoclaw webhooks` for Gmail helper commands.
...
- Extend Haoclaw's behavior without modifying core code

## Getting Started

### Bundled Hooks

Haoclaw ships with four bundled hooks that are automatically discovered:

- **💾 session-memory**: Saves session context to your agent workspace (default `~/.haoclaw/workspace/memory/`) when you issue `/new`
- **📎 bootstrap-extra-files**: Injects additional workspace bootstrap files from configured glob/path patterns during `agent:bootstrap`
- **📝 command-logger**: Logs all command events to `~/.haoclaw/logs/commands.log`
...
During onboarding (`haoclaw onboard`), you'll be prompted to enable recommended hooks. The wizard automatically discovers eligible hooks and presents them for selection.
...
2. **Managed hooks**: `~/.haoclaw/hooks/` (user-installed, shared across workspaces)
3. **Bundled hooks**: `<haoclaw>/dist/hooks/bundled/` (shipped with Haoclaw)
...
Hook packs are standard npm packages that export one or more hooks via `haoclaw.hooks` in
...
Hook packs can ship dependencies; they will be installed under `~/.haoclaw/hooks/<id>`.
Each `haoclaw.hooks` entry must stay inside the package directory after symlink
resolution; entries that escape are rejected.

Security note: `haoclaw hooks install` installs dependencies with `npm install --ignore-scripts`
...
homepage: https://docs.haoclaw.ai/automation/hooks#my-hook
metadata:
  { "haoclaw": { "emoji": "🔗", "events": ["command:new"], "requires": { "bins": ["node"] } } }
---
...
The `metadata.haoclaw` object supports:
...
These hooks are not event-stream listeners; they let plugins synchronously adjust tool results before Haoclaw persists them.
...
- **Managed hooks** (`~/.haoclaw/hooks/`): Shared across workspaces

### 2. Create Directory Structure

```bash
mkdir -p ~/.haoclaw/hooks/my-hook
cd ~/.haoclaw/hooks/my-hook
```
...
metadata: { "haoclaw": { "emoji": "🎯", "events": ["command:new"] } }
---
...
# JSON output
haoclaw hooks info session-memory --json
```
...
**Output**: `<workspace>/memory/YYYY-MM-DD-slug.md` (defaults to `~/.haoclaw/workspace`)
...
```yaml
metadata: { "haoclaw": { "events": ["command:new"] } } # Specific
```

Rather than:

```yaml
metadata: { "haoclaw": { "events": ["command"] } } # General - more overhead
```
...
2. Verify HOOK.md format:
   ...
   ```bash
   cat ~/.haoclaw/hooks/my-hook/HOOK.md
...
1. Create hook directory:
   ...
   ```bash
   mkdir -p ~/.haoclaw/hooks/my-hook
   mv ./hooks/handlers/my-handler.ts ~/.haoclaw/hooks/my-hook/handler.ts
   ```
...
- [Bundled Hooks README](https://github.com/haoclaw/haoclaw/tree/main/src/hooks/bundled)
