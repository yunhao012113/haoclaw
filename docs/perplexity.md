...
Haoclaw supports Perplexity Search API as a `web_search` provider.
It returns structured results with `title`, `url`, and `snippet` fields.

For compatibility, Haoclaw also supports legacy Perplexity Sonar/OpenRouter setups.
...
**Via config:** run `haoclaw configure --section web`. It stores the key in
`~/.haoclaw/haoclaw.json` under `tools.web.search.perplexity.apiKey`.
...
`~/.haoclaw/.env` (or your service environment). See [Env vars](/help/faq#how-does-haoclaw-load-environment-variables).
