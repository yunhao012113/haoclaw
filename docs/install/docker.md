---
summary: "Optional Docker-based setup and onboarding for Haoclaw"
read_when:
...
- **Yes**: you want an isolated, throwaway gateway environment or to run Haoclaw on a host without local installs.
- **No**: you’re running on your own machine and just want the fastest dev loop. Use the normal install flow instead.
- **Sandboxing note**: agent sandboxing uses Docker too, but it does **not** require the full gateway to run in Docker. See [Sandboxing](/gateway/sandboxing).

This guide covers:

- Containerized Gateway (full Haoclaw in Docker)
...
- builds the gateway image locally (or pulls a remote image if `HAOCLAW_IMAGE` is set)
...
- `HAOCLAW_IMAGE` — use a remote image instead of building locally (e.g. `ghcr.io/haoclaw/haoclaw:latest`)
- `HAOCLAW_DOCKER_APT_PACKAGES` — install extra apt packages during build
- `HAOCLAW_EXTENSIONS` — pre-install extension dependencies at build time (space-separated extension names, e.g. `diagnostics-otel matrix`)
- `HAOCLAW_EXTRA_MOUNTS` — add extra host bind mounts
- `HAOCLAW_HOME_VOLUME` — persist `/home/node` in a named volume
- `HAOCLAW_SANDBOX` — opt in to Docker gateway sandbox bootstrap. Only explicit truthy values enable it: `1`, `true`, `yes`, `on`
- `HAOCLAW_INSTALL_DOCKER_CLI` — build arg passthrough for local image builds (`1` installs Docker CLI in the image). `docker-setup.sh` sets this automatically when `HAOCLAW_SANDBOX=1` for local builds.
- `HAOCLAW_DOCKER_SOCKET` — override Docker socket path (default: `DOCKER_HOST=unix://...` path, else `/var/run/docker.sock`)
- `HAOCLAW_ALLOW_INSECURE_PRIVATE_WS=1` — break-glass: allow trusted private-network
  `ws://` targets for CLI/onboarding client paths (default is loopback-only)
- `HAOCLAW_BROWSER_DISABLE_GRAPHICS_FLAGS=0` — disable container browser hardening flags
  `--disable-3d-apis`, `--disable-software-rasterizer`, `--disable-gpu` when you need
  WebGL/3D compatibility.
- `HAOCLAW_BROWSER_DISABLE_EXTENSIONS=0` — keep extensions enabled when browser
  flows require them (default keeps extensions disabled in sandbox browser).
- `HAOCLAW_BROWSER_RENDERER_PROCESS_LIMIT=<N>` — set Chromium renderer process
...
- Need the URL again? Run `docker compose run --rm haoclaw-cli dashboard --no-open`.
...
```bash
export HAOCLAW_SANDBOX=1
export HAOCLAW_DOCKER_SOCKET=/run/user/1000/docker.sock
./docker-setup.sh
```
...
- If `Dockerfile.sandbox` is missing, the script prints a warning and continues;
  build `haoclaw-sandbox:bookworm-slim` with `scripts/sandbox-setup.sh` if
  needed.
- For non-local `HAOCLAW_IMAGE` values, the image must already contain Docker
...
```bash
docker compose run -T --rm haoclaw-cli gateway probe
docker compose run -T --rm haoclaw-cli devices list --json
```
...
`haoclaw-cli` uses `network_mode: "service:haoclaw-gateway"` so CLI commands can
...
network path instead of the bundled `haoclaw-cli` service.

To reduce impact if the CLI process is compromised, the compose config drops
`NET_RAW`/`NET_ADMIN` and enables `no-new-privileges` on `haoclaw-cli`.
...
- [GitHub Container Registry package](https://github.com/haoclaw/haoclaw/pkgs/container/haoclaw)

Use image name `ghcr.io/haoclaw/haoclaw` (not similarly named Docker Hub
images).
...
- `org.opencontainers.image.source=https://github.com/haoclaw/haoclaw`
...
image instead, set `HAOCLAW_IMAGE` before running the script:

```bash
export HAOCLAW_IMAGE="ghcr.io/haoclaw/haoclaw:latest"
./docker-setup.sh
```

The script detects that `HAOCLAW_IMAGE` is not the default `haoclaw:local` and
...
`docker-compose.yml` and helper files. `HAOCLAW_IMAGE` skips local image build
...
```bash
mkdir -p ~/.clawdock && curl -sL https://raw.githubusercontent.com/haoclaw/haoclaw/main/scripts/shell-helpers/clawdock-helpers.sh -o ~/.clawdock/clawdock-helpers.sh
```
...
See [`ClawDock` Helper README](https://github.com/haoclaw/haoclaw/blob/main/scripts/shell-helpers/README.md) for details.

### Manual flow (compose)

```bash
docker build -t haoclaw:local -f Dockerfile .
docker compose run --rm haoclaw-cli onboard
docker compose up -d haoclaw-gateway
```

Note: run `docker compose ...` from the repo root. If you enabled
`HAOCLAW_EXTRA_MOUNTS` or `HAOCLAW_HOME_VOLUME`, the setup script writes
...
`HAOCLAW_EXTRA_MOUNTS` before running `docker-setup.sh`. This accepts a
comma-separated list of Docker bind mounts and applies them to both
`haoclaw-gateway` and `haoclaw-cli` by generating `docker-compose.extra.yml`.

Example:

```bash
export HAOCLAW_EXTRA_MOUNTS="$HOME/.codex:/home/node/.codex:ro,$HOME/github:/home/node/github:rw"
./docker-setup.sh
```
...
- If you edit `HAOCLAW_EXTRA_MOUNTS`, rerun `docker-setup.sh` to regenerate the
  extra compose file.
...
volume via `HAOCLAW_HOME_VOLUME`. This creates a Docker volume and mounts it at
...
```bash
export HAOCLAW_HOME_VOLUME="haoclaw_home"
./docker-setup.sh
```
...
- If you change `HAOCLAW_HOME_VOLUME`, rerun `docker-setup.sh` to regenerate the
  extra compose file.
...
libraries), set `HAOCLAW_DOCKER_APT_PACKAGES` before running `docker-setup.sh`.
...
```bash
export HAOCLAW_DOCKER_APT_PACKAGES="ffmpeg build-essential"
./docker-setup.sh
```
...
- If you change `HAOCLAW_DOCKER_APT_PACKAGES`, rerun `docker-setup.sh` to rebuild
  the image.
...
dependencies into the image instead, set `HAOCLAW_EXTENSIONS` before
running `docker-setup.sh`:

```bash
export HAOCLAW_EXTENSIONS="diagnostics-otel matrix"
./docker-setup.sh
```
...
```bash
docker build --build-arg HAOCLAW_EXTENSIONS="diagnostics-otel matrix" .
```
...
- If you change `HAOCLAW_EXTENSIONS`, rerun `docker-setup.sh` to rebuild
  the image.
...
```bash
export HAOCLAW_DOCKER_APT_PACKAGES="git curl jq"
./docker-setup.sh
```
...
`HAOCLAW_DOCKER_APT_PACKAGES` instead of using `--with-deps` at runtime.

4. **Persist Playwright browser downloads**:

- Set `PLAYWRIGHT_BROWSERS_PATH=/home/node/.cache/ms-playwright` in
  `docker-compose.yml`.
- Ensure `/home/node` persists via `HAOCLAW_HOME_VOLUME`, or mount
  `/home/node/.cache/ms-playwright` via `HAOCLAW_EXTRA_MOUNTS`.

### Permissions + EACCES

The image runs as `node` (uid 1000). If you see permission errors on
`/home/node/.haoclaw`, make sure your host bind mounts are owned by uid 1000.

Example (Linux host):

```bash
sudo chown -R 1000:1000 /path/to/haoclaw-config /path/to/haoclaw-workspace
```
...
```bash
docker compose run --rm haoclaw-cli channels login
```

Telegram (bot token):

```bash
docker compose run --rm haoclaw-cli channels add --channel telegram --token "<token>"
```

Discord (bot token):

```bash
docker compose run --rm haoclaw-cli channels add --channel discord --token "<token>"
```
...
background. In plain terms: Docker keeps checking if Haoclaw is still
...
```bash
docker compose exec haoclaw-gateway node dist/index.js health --token "$HAOCLAW_GATEWAY_TOKEN"
```
...
`docker-setup.sh` defaults `HAOCLAW_GATEWAY_BIND=lan` so host access to
...
```bash
docker compose run --rm haoclaw-cli config set gateway.mode local
docker compose run --rm haoclaw-cli config set gateway.bind lan
docker compose run --rm haoclaw-cli devices list --url ws://127.0.0.1:18789
```

### Notes

- Gateway bind defaults to `lan` for container use (`HAOCLAW_GATEWAY_BIND`).
- Dockerfile CMD uses `--allow-unconfigured`; mounted config with `gateway.mode` not `local` will still start. Override CMD to enforce the guard.
- The gateway container is the source of truth for sessions (`~/.haoclaw/agents/<agentId>/sessions/`).

### Storage model

- **Persistent host data:** Docker Compose bind-mounts `HAOCLAW_CONFIG_DIR` to `/home/node/.haoclaw` and `HAOCLAW_WORKSPACE_DIR` to `/home/node/.haoclaw/workspace`, so those paths survive container replacement.
...
- **Disk growth hotspots:** watch `media/`, `agents/<agentId>/sessions/sessions.json`, transcript JSONL files, `cron/runs/*.jsonl`, and rolling file logs under `/tmp/haoclaw/` (or your configured `logging.file`). If you also run the macOS app outside Docker, its service logs are separate again: `~/.haoclaw/logs/gateway.log`, `~/.haoclaw/logs/gateway.err.log`, and `/tmp/haoclaw/haoclaw-gateway.log`.
...
- Agent workspace access: `workspaceAccess: "none"` (default) uses `~/.haoclaw/sandboxes`
...
- `user` must be root for `apt-get` (omit `user` or set `user: "0:0"`).
  Haoclaw auto-recreates containers when `setupCommand` (or docker config) changes
  unless the container was **recently used** (within ~5 minutes). Hot containers
  log a warning with the exact `haoclaw sandbox recreate ...` command.
...
This builds `haoclaw-sandbox:bookworm-slim` using `Dockerfile.sandbox`.
...
This builds `haoclaw-sandbox-common:bookworm-slim`. To use it:

```json5
{
  agents: {
    defaults: {
      sandbox: { docker: { image: "haoclaw-sandbox-common:bookworm-slim" } },
    },
  },
}
```
...
This builds `haoclaw-sandbox-browser:bookworm-slim` using
...
- Browser containers default to a dedicated Docker network (`haoclaw-sandbox-browser`) instead of global `bridge`.
...
- noVNC observer access is password-protected by default; Haoclaw provides a short-lived observer token URL that serves a local bootstrap page and keeps the password in URL fragment (instead of URL query).
- Browser container startup defaults are conservative for shared/container workloads, including:
  ...
  - The three graphics hardening flags above are optional. If your workload needs
    WebGL/3D, set `HAOCLAW_BROWSER_DISABLE_GRAPHICS_FLAGS=0` to run without
  ...
  - Extension behavior is controlled by `--disable-extensions` and can be disabled
    (enables extensions) via `HAOCLAW_BROWSER_DISABLE_EXTENSIONS=0` for
    extension-dependent pages or extensions-heavy workflows.
  - `--renderer-process-limit=2` is also configurable with
    `HAOCLAW_BROWSER_RENDERER_PROCESS_LIMIT`; set `0` to let Chromium choose its
...
```json5
{
  agents: {
    defaults: {
      sandbox: { browser: { image: "my-haoclaw-browser" } },
    },
  },
}
```
...
```bash
docker build -t my-haoclaw-sbx -f Dockerfile.sandbox .
```
...
```json5
{
  agents: {
    defaults: {
      sandbox: { docker: { image: "my-haoclaw-sbx" } },
    },
  },
}
```
...
- Image missing: build with [`scripts/sandbox-setup.sh`](https://github.com/haoclaw/haoclaw/blob/main/scripts/sandbox-setup.sh) or set `agents.defaults.sandbox.docker.image`.
- Container not running: it will auto-create per session on demand.
- Permission errors in sandbox: set `docker.user` to a UID:GID that matches your
  mounted workspace ownership (or chown the workspace folder).
- Custom tools not found: Haoclaw runs commands with `sh -lc` (login shell), which
