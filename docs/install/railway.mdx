...
Deploy Haoclaw on Railway with a one-click template and finish setup in your browser.
...
- `https://<your-railway-domain>/haoclaw` — Control UI
...
- `HAOCLAW_STATE_DIR=/data/.haoclaw` (recommended)
- `HAOCLAW_WORKSPACE_DIR=/data/workspace` (recommended)
- `HAOCLAW_GATEWAY_TOKEN` (recommended; treat as an admin secret)
...
This exports your Haoclaw state + workspace so you can migrate to another host without losing config or memory.
