...
Deploy Haoclaw on Render using Infrastructure as Code. The included `render.yaml` Blueprint defines your entire stack declaratively, service, disk, environment variables, so you can deploy with a single click and version your infrastructure alongside your code.
...
[Deploy to Render](https://render.com/deploy?repo=https://github.com/haoclaw/haoclaw)
...
repository configures everything needed to run Haoclaw:
...
services:
  - type: web
    ...
    envVars:
      ...
      - key: HAOCLAW_STATE_DIR
        value: /data/.haoclaw
      - key: HAOCLAW_WORKSPACE_DIR
        value: /data/workspace
      - key: HAOCLAW_GATEWAY_TOKEN
...
The web dashboard is available at `https://<your-service>.onrender.com/haoclaw`.
...
If you use the original Haoclaw repository, Render will not auto-deploy your Haoclaw. To update it, run a manual Blueprint sync from the dashboard.
...
For Haoclaw, vertical scaling is usually sufficient. Horizontal scaling requires sticky sessions or external state management.
...
This downloads a portable backup you can restore on any Haoclaw host.
