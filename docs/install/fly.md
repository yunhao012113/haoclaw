---
title: Fly.io
description: Deploy Haoclaw on Fly.io
summary: "Step-by-step Fly.io deployment for Haoclaw with persistent storage and HTTPS"
read_when:
  - Deploying Haoclaw on Fly.io
  - Setting up Fly volumes, secrets, and first-run config
---

# Fly.io Deployment

**Goal:** Haoclaw Gateway running on a [Fly.io](https://fly.io) machine with persistent storage, automatic HTTPS, and Discord/channel access.
...
fly apps create my-haoclaw

# Create a persistent volume (1GB is usually enough)
fly volumes create haoclaw_data --size 1 --region iad
```
...
[env]
  NODE_ENV = "production"
  HAOCLAW_PREFER_PNPM = "1"
  HAOCLAW_STATE_DIR = "/data"
...
fly secrets set HAOCLAW_GATEWAY_TOKEN=$(openssl rand -hex 32)
...
- Non-loopback binds (`--bind lan`) require `HAOCLAW_GATEWAY_TOKEN` for security.
- Treat these tokens like passwords.
- **Prefer env vars over config file** for all API keys and tokens. This keeps secrets out of `haoclaw.json` where they could be accidentally exposed or logged.
...
**Note:** With `HAOCLAW_STATE_DIR=/data`, the config path is `/data/haoclaw.json`.
...
Or visit `https://my-haoclaw.fly.dev/`

Paste your gateway token (the one from `HAOCLAW_GATEWAY_TOKEN`) to authenticate.
...
**Fix:** Ensure `internal_port` matches the gateway port (set `--port 3000` or `HAOCLAW_GATEWAY_PORT=3000`).
...
If using `--allow-unconfigured`, the gateway creates a minimal config. Your custom config at `/data/haoclaw.json` should be read on restart.

Verify the config exists:

```bash
fly ssh console --command "cat /data/haoclaw.json"
```
...
echo '{"your":"config"}' | fly ssh console -C "tee /data/haoclaw.json"

# Or use sftp
fly sftp shell
> put /local/path/config.json /data/haoclaw.json
```

**Note:** `fly sftp` may fail if the file already exists. Delete first:

```bash
fly ssh console --command "rm /data/haoclaw.json"
```
...
**Fix:** Ensure `HAOCLAW_STATE_DIR=/data` is set in `fly.toml` and redeploy.
...
```bash
# List current IPs
fly ips list -a my-haoclaw

# Release public IPs
fly ips release <public-ipv4> -a my-haoclaw
fly ips release <public-ipv6> -a my-haoclaw
...
fly ips allocate-v6 --private -a my-haoclaw
```
...
fly proxy 3000:3000 -a my-haoclaw
