...
- `/usage cost` in chats: local cost summary aggregated from Haoclaw session logs.
- CLI: `haoclaw status --usage` prints a full per-provider breakdown.
- CLI: `haoclaw channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
