...
read_when:
  - You want to understand what “context” means in Haoclaw
...
“Context” is **everything Haoclaw sends to the model for a run**. It is bounded by the model’s **context window** (token limit).

Beginner mental model:

- **System prompt** (Haoclaw-built): rules, tools, skills list, time/runtime, and injected workspace files.
...
## How Haoclaw builds the system prompt

The system prompt is **Haoclaw-owned** and rebuilt each run. It includes:
...
By default, Haoclaw injects a fixed set of workspace files (if present):
...
Large files are truncated per-file using `agents.defaults.bootstrapMaxChars` (default `20000` chars). Haoclaw also enforces a total bootstrap injection cap across files with `agents.defaults.bootstrapTotalMaxChars` (default `150000` chars). `/context` shows **raw vs injected** sizes and whether truncation happened.
...
By default, Haoclaw uses the built-in `legacy` context engine for assembly and
compaction. If you install a plugin that provides `kind: "context-engine"` and
select it with `plugins.slots.contextEngine`, Haoclaw delegates context
