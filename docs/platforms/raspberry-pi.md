---
summary: "Haoclaw on Raspberry Pi (budget self-hosted setup)"
read_when:
  - Setting up Haoclaw on a Raspberry Pi
  - Running Haoclaw on ARM devices
  - Building a cheap always-on personal AI
title: "Raspberry Pi"
---

# Haoclaw on Raspberry Pi

## Goal

Run a persistent, always-on Haoclaw Gateway on a Raspberry Pi for **~$35-80** one-time cost (no monthly fees).
...
# Update config
haoclaw config set gateway.bind tailnet
...
```bash
grep -q 'NODE_COMPILE_CACHE=/var/tmp/haoclaw-compile-cache' ~/.bashrc || cat >> ~/.bashrc <<'EOF' # pragma: allowlist secret
export NODE_COMPILE_CACHE=/var/tmp/haoclaw-compile-cache
mkdir -p /var/tmp/haoclaw-compile-cache
export HAOCLAW_NO_RESPAWN=1
EOF
source ~/.bashrc
```
...
- `HAOCLAW_NO_RESPAWN=1` avoids extra startup cost from CLI self-respawn.
- First run warms the cache; later runs benefit most.

### systemd startup tuning (optional)

If this Pi is mostly running Haoclaw, add a service drop-in to reduce restart
jitter and keep startup env stable:

```bash
sudo systemctl edit haoclaw
```

```ini
[Service]
Environment=HAOCLAW_NO_RESPAWN=1
Environment=NODE_COMPILE_CACHE=/var/tmp/haoclaw-compile-cache
Restart=always
RestartSec=2
TimeoutStartSec=90
```
...
If possible, keep Haoclaw state/cache on SSD-backed storage to avoid SD-card
...
Most Haoclaw features work on ARM64, but some external binaries may need ARM builds:
...
sudo systemctl is-enabled haoclaw

# Enable if not
sudo systemctl enable haoclaw

# Start on boot
sudo systemctl start haoclaw
```
...
```bash
# Check logs
journalctl -u haoclaw --no-pager -n 100

# Common fix: rebuild
cd ~/haoclaw  # if using hackable install
npm run build
sudo systemctl restart haoclaw
```
