...
- Same path: run the app from a fixed location (for Haoclaw, `dist/Haoclaw.app`).
...
```bash
sudo tccutil reset Accessibility ai.haoclaw.mac
sudo tccutil reset ScreenCapture ai.haoclaw.mac
...
Workaround: move files into the Haoclaw workspace (`~/.haoclaw/workspace`) if you want to avoid per-folder grants.
