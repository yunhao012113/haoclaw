...
service via the external `haoclaw` CLI (no embedded runtime). This gives you
...
- The app installs a per‑user LaunchAgent labeled `ai.haoclaw.gateway`
  (or `ai.haoclaw.<profile>` when using `--profile`/`HAOCLAW_PROFILE`; legacy `com.haoclaw.*` is supported).
