...
Haoclaw on Windows is recommended **via WSL2** (Ubuntu recommended). The
...
### 2) Install the Haoclaw gateway user service
...
```bash
systemctl --user is-enabled haoclaw-gateway
systemctl --user status haoclaw-gateway --no-pager
```
