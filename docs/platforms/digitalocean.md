---
summary: "Haoclaw on DigitalOcean (simple paid VPS option)"
read_when:
  - Setting up Haoclaw on DigitalOcean
  - Looking for cheap VPS hosting for Haoclaw
title: "DigitalOcean"
---

# Haoclaw on DigitalOcean

## Goal

Run a persistent Haoclaw Gateway on DigitalOcean for **$6/month** (or $4/mo with reserved pricing).
...
# Check service
systemctl --user status haoclaw-gateway.service

# View logs
journalctl --user -u haoclaw-gateway.service -f
```
...
```bash
haoclaw channels login whatsapp
# Scan QR code
```
...
- `~/.haoclaw/workspace/` — workspace (SOUL.md, memory, etc.)
...
journalctl -u haoclaw --no-pager -n 50
```
