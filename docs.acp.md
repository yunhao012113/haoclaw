...
This document describes how the Haoclaw ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`haoclaw acp` exposes an ACP agent over stdio and forwards prompts to a running
Haoclaw Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
- ACP client spawns `haoclaw acp` and speaks ACP messages over stdio.
...
`haoclaw acp` resolves the Gateway URL and auth from CLI flags or config:
